...
We call \defname{input-variables nodes} the elements of $N_{\sf var}^{r\vdash}$ i.e.\
...
represent this pointer in the sequence as follows \Pstr[0.4cm]{
...
%\begin{definition}
%We define the \defname{subterm filtering} operator on traversals of $\tau(M)$ as follows. Let $\Delta \vdash \kappa(n) : A$ be some subterm of $\elnf{M}$ for some $n\in N_\lambda$ and $t \in \travset(M)$. Then $t\subtermfilter \kappa(n)$ is the subsequence of $t$ consisting of nodes in $\tau(\kappa(n))$ ({\it i.e.}~nodes that are descendants of $n$ in $\tau(M)$).
%\end{definition}
%In particular we have $ t \subtermfilter M = t$.

\begin{lemma}[Traversal filtering lemma]
\label{lem:trav_filtering}
Let $\Gamma(r') \vdash \kappa(r') : T(r')$ be some subterm of $\elnf{M}$ for some $r'\in N_\lambda$. Let $t \in \travset(M)$ and $r_0$ be an occurrence of $r'$ in $t$, then
$$(t-@)\filter r_0 \in \travset^{-@}(\kappa(r'))\ .$$
{\it i.e.}\ there is a traversal $t'$ of $\travset(\kappa(r'))$ verifying
$t'-@ = (t-@)\filter r_0$.
\end{lemma}
\begin{proof}
  Let $M'$ denote the term $\kappa(r')$ and 
  $N'$ be the set of nodes of the tree $\tau(\kappa(r'))$.

  \notetoself{
  By induction on the traversal. The base cases \rulenamet{Empty} and \rulenamet{Root} are trivial.
  \begin{itemize}
    \item \rulenamet{Lam}
    Suppose $t = u \cdot \lambda \overline{\xi} \cdot n \in \travset(M)$.
    By the induction hypothesis, for some traversal $u' \in \travset(\kappa(r'))$ we have
    $u'-@ = ((u \cdot \lambda \overline{\xi}) -@)\filter r_0$.
    W.l.o.g. we can assume that $u'^\omega \neq @$.

    Suppose that $n$ is her. just. by $r_0$ in $t-@$ then its parent node
    $\lambda \overline{\xi}$ is also her. just. by $r_0$ therefore
    $u'$'s last node is $\lambda \overline{\xi}$ and we can use the rule \rulenamet{Lam} in $\tau(M')$ to produce the traversal $u' \cdot n$ of $M'$ verifying 
    
    $(u' \cdot n) - @  = ((u \cdot \lambda \overline{\xi} \cdot n) -@)\filter r_0$.

    Suppose that $n \not\in N'$ then since $n$ is not a lambda node, its parent node is not in $N'$ either therefore we can conclude by applying the induction hypothesis on $u$.
