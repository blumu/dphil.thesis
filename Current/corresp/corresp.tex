...
All the operations on strategies except composition are pointer-preserving: Let $u$ be a position of the game $\intersem{T}$, $T'$ a subtree of $T$, $A$ be a sub-game of $\sem{type(T)}$ and $A'$ be a sub-game of $\sem{type(T')}$ such that $A$ and $A'$ are $\sim$-equivalent, we say that  \defname{$u$'s justification is preserved from $A$ to $A'$} with respect to $\sim$ just if an occurrence $m$ is justified by $n$ in $u\filter A$ if and only if it is justified by the same occurrence $n$ in $u \filter A'$. This is formally expressed by the equality:
\begin{equation*}
