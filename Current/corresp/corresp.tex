...
\emph{Notations:} We write $N$ for the set of nodes of $\tau(M)$,
$N_\Sigma$ for the set of $\Sigma$-labelled nodes, $N_@$ for the set
of @-labelled nodes, $N_{var}$ for the set of variable nodes,
$N_{fv}$ for the subset of $N_{var}$ constituted of free-variable
nodes and $r$ for the root of $\tau(M)$.
...
\item[\rulenamet{Answer-$\lambda$}] If \Pstr[0.4cm]{t \cdot \lambda \overline{\xi} \cdot (x){x}  \ldots   (xv-x,50:v){v}_x}
...
\item[\rulenamet{Answer-$var$}] If \Pstr[0.4cm]{t \cdot y \cdot (lmd){\lambda \overline{\xi}}
...
\begin{remark}
\label{rem:interstrat} We observe that for all strategy operator
except composition, pointers associated to moves are preserved.
Moreover for strategy composition, additional pointers are
...
\begin{eqnarray*}
 ...
 \varphi_M  &:& \travset(\Gamma \vdash M : T)^{\upharpoonright r} \stackrel{\cong}{\longrightarrow} \sem{\Gamma \vdash M :T} \ .
\end{eqnarray*}

\end{theorem}
Because of corollary \ref{cor:varphi_bij}, we just need to show that
$\varphi_M$ defines \emph{surjections}, that is to
say:
\begin{eqnarray*}
\mbox{i'.}& \quad \varphi_M(\travset(M)^{-@}) &= \intersem{\Gamma \vdash M : T} \\
\mbox{ii'.}& \quad \varphi_M(\travset(M)^{\upharpoonright r}) &= \sem{\Gamma \vdash M :
T}
\end{eqnarray*}
But ii' is an immediate consequence of i' since:
\begin{align*}
...
\end{align*}
therefore we just need to show i'.
...
\begin{remark} Let us give a brief overview of the proof before going into the
technical details.
    We proceed by induction on the structure of the computation tree.
...
$\eta$-long normal form. We proceed by induction on the structure of
$M$:
\begin{enumerate}[-]
...
Hence we can conclude using the induction hypothesis.
  ...
  \item (variable) $M \equiv x_i$. Since $M$ is in $\eta$-long normal form, $x$ must be of ground
    ...
    $$ \tree[levelsep=6ex]{\lambda^{[0]}}
        { \tree[levelsep=6ex]{@^{[1]}}
            {
            \tree[levelsep=6ex]{\lambda y_1 \ldots y_p}{\TR{\ldots}}
            \TR{\tau(N_1)} \TR{\ldots} \TR{\tau(N_p)}}}
    $$
    ...
    \intersem{\Gamma \vdash M : o}
            &=  \langle \intersem{\Gamma \vdash N_0 : B_0}, \intersem{\Gamma \vdash N_1 : B_1} \rangle^\dagger\ \| \ ev
...
%            & \mbox{($\varphi_M = \psi_{\sem{\Gamma\rightarrow T}}^{r,q_0} \union \varphi_{N_0} \union \varphi_{N_1}$)} %\\
    \end{align*}
    and since $\varphi_M = \psi_M \union \varphi_{N_0} \union
    ...
    C^1$ where $C = o$. Thus, plays of $ev$ do not contain
    uncovered moves.
    ...
    proof that there exists a sequence of nodes $t$ in $N$ such
    that $\varphi_M(t-@) = u$ by induction on the length of $u$.
    Let $q_o$ and $q_1$ be the initial question of the arena $C$
    and $B_0$ respectively.

    \emph{Base cases}:
    \begin{compactitem}[-]
    \item $u=\epsilon$ then $\varphi(\epsilon) = u$ where the traversal $\epsilon$ is formed with the rule \rulenamet{Empty}.
    \item If $|u|=1$ then $u=q_0$ is the initial move in $C$ and $\varphi(\lambda \overline{\xi}) = u$. The traversal
    $\lambda \overline{\xi}$ is formed with the rule
     \rulenamet{Root}.
    \end{compactitem}

    \emph{Step cases}: Suppose that $u = \overbrace{\ldots \cdot
    m^1}^{u'} \cdot\ m \in \intersem{\Gamma \vdash M : T}$ and
    $u' = \varphi_M(t'-@)$ for some traversal $t'$ of $\tau(M)$
    for some moves $m, m^1 \in M_T$.


    By unraveling the definition of $u \in \intersem{\Gamma
    \vdash M : T}$ we have:
    \begin{eqnarray*}
      &&      \left\{
            \begin{array}{ll}
    ...
    \begin{array}{ll}
        u \in L_T \\
        \hbox{for any initial $m$ in $!B_0^0 \otimes !B_1^0$ there is $j \in 0..p$ such that } \\
        \left\{\begin{array}{ll}
            u \filter m \filter T^{00j} \in \intersem{B_j} \label{eq:def_z} \\
    ...
    Thus we can use the rule \rulenamet{Answer-$\lambda$} to produce
    ...
    move in $t'$.
        ...
        case: we use the rule (Answer-$\lambda$) or
        (Answer-$\lambda$') to extend the traversal
        $t'$.
        ...
        We can therefore use the rule (Answer-var) or (Answer-@) to extend the traversal $t'$.
        \end{enumerate}
