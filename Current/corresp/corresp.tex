...
$\sim_T : \mathcal{M}_T \times \mathcal{M}_T$ defined as follows:
...
The set of \defname{internal moves} of the game $\intersem{T}$,
written $M_T^-$, consists of all moves from $M_T$ that are not
...
moves obtained from $s$ by keeping only the moves that are
$\sim$-equivalent to some move in $M_T$. Also for every preserved
equivalence class-move $m$ we only keep the pointers associated to
class-citizens that are present in $\mathcal{M}_{T'}$.
...
of sets of uncovered positions. We define it inductively on the
