        ...
        \begin{compactitem}
            ...
            \item If $n \in N_{\sf var}$ then it has auxiliary links pointing to each of the prime $\lambda$-nodes in $\pview{t\cdot
                ...
                \begin{compactitem}
                    ...
                    P-view. In particular, if $n$ is a free variable node then its
                    justifier is the root node occurring initially in $\pview{t
                    \cdot \lambda \overline{\xi}}$.} of its enabler in
...
\cdot t_2)^\omega=x$) then so is \Pstr[0.4cm]{t_1 \cdot
(x){x} \cdot t_2 \cdot (xv-x,38:v){v_x} } for all $v \in
\mathcal{D}$.

\item[\rulenamet{InputVar}] If $t$ is a traversal with a pending node
...
traversal for some variable $y\not\in N_{\sf var}^{\theroot\vdash}$ then so is \Pstr[0.5cm]{t \cdot (y){y} \cdot (lmd){\lambda
...
If \Pstr[0.5cm]{t' \cdot (n){n} \cdot (lx){\lambda \overline{x}}
    ...
    $x_i$ not in $N_{\sf var}^{\theroot\vdash}$ then
...
\begin{remark}
    The definition of well-behavior is in fact derived from the definition of the rules \rulenamet{InputVar$^{val}$} and \rulenamet{InputVar}.  In fact while we can regard uninterpreted constants as free variables, interpreted constants can be seen as a generalization of the notion of free variable. The extra power of the well-behaved constant rules
...
If $n$ occurs in $t \filter^\star r_0$ then $n$ is in $N^{(r')}$.
\end{lemma}
\begin{proof}
Justification pointers attached to a node $n$ in $t^\star$ can only point to
 ...
 at $r'$ and  therefore $n$ must belong to $N^{(r')}$.
\end{proof}
...
$M^{(r')}$ for the term $\kappa(r')$ and $N^{(r')}$ for the set of vertices and nodes of the sub computation tree $\tau(M^{(r')})$ (thus $N^{(r')} = E^*(\{r'\})$).
...
\begin{proposition}[Traversal filtering]
    ...
    \begin{itemize}
            \item[(P)] 
            \begin{compactitem}
            \item[(a)] If $t^\omega$ belongs to $N^{(r')}$ for some lambda node $r'\in N_\lambda$
            ({\it i.e.}\ $t^\omega$ is an occurrence of non-leaf node of the subtree of $\tau(M)$ rooted at $r'$) then there is an occurrence $r_0$ of $r'$ in $t$ such that
            \begin{itemize}
            ...
            \item $\pview{t}^M \filter N^{(r')} = r_0\cdot u =  \pview{t\filter^\star r_0}^{M^{(r')}}$;
            \end{itemize}

            \item[(b)] If $t$'s last occurrence is not in $N^{(r')}$ then
            all the occurrences in $\pview{t}$ are not in
            $N^{(r')}$ and do not appear in any sequence $t\filter^\star r_0$ for some 
...
$r'$ in $N_\lambda$. If $t^\omega \in V^{\theroot\vdash}$ and $t^\omega$ appears in $t \filter^\star r_0$ then $\oview{t}_M\filter^\star r_0 = \oview{t\filter^\star r_0}_{M^{(r')}}$.
            \end{compactitem}

       \item[(T)]
            For any occurrence $r_0$ in $t$ of some lambda node
             $r'\in N_\lambda$ we have $t\filter^\star r_0 \in
             \travset(M^{(r')})$.
    \end{itemize}
\end{proposition}
...
\proof
We prove (P), (O) and (T) simultaneously by induction on the
traversal rules. The base cases \rulenamet{Empty} and
\rulenamet{Root} are trivial. \emph{Step case:} Take a traversal $t
\in \travset(M)$ such that $|t|>1$. Suppose that the properties are
...
We do a case analysis on the type of the last node in $t$:
    ...
    (a) Suppose $n$ is in $N^{(r')}$. Since $n$ is not a lambda
    node, its parent node $t'^\omega$ necessarily belong to
    $N^{(r')}$. Then by the induction hypothesis,
    ...
    to $N^{(r')}$ since they appear on the path from $r'$ to
    ...
    &= \pview{t' \cdot n}^M \filter  N^{(r')} & (\mbox{definition of } t)\\
            ...
            &= \pview{t'}^M \filter N^{(r')}  \cdot n             & \parbox[t]{6cm}{(\raggedleft since $n$ is in $N^{(r')}$, where $n$'s justifiers are those from the sequence $t$ that are in $N^{(r')}$)} \\
    ...
    (b) Suppose $n$ is not in $N^{(r')}$ then necessarily its parent node $t'^\omega$ does not belong to $N^{(r')}$ either.
    ...
    (a) Suppose that $n\in N^{(r')}$. Since $m$ is the parent of
    the node $n$ in $N^{(r')}$ which is not the the root of
    $\tau(M^{(r')})$, necessarily $m$ also belongs to
    $N^{(r')}$. Therefore by the induction hypothesis, there
    is some occurrence $r_0$ of $r'$ in $t$ such that
    $\pview{r' \cdot m} = \ldots r_0 \cdot u$ and all the
    nodes in $u$ belong to $N^{(r')}$, and all the
    occurrences in $u$ appear in $t\filter^\star r_0$
    and not in any $t\filter^\star r_1$ for any occurrence $r_1\neq r_0$ of $r'$.
    Since $n$'s only justifier points in $\pview{r' \cdot
    m}$, $n$ must also verifies the second point of (a).

    Moreover:
            \begin{align*}
            \pview{t}^M \filter  N^{(r')}            &= \pview{\Pstr{t' \cdot (m){m} \cdot u \cdot (n-m,40){n}}}^M \filter  N^{(r')}
                                                                  & (\mbox{definition of } t)\\
            &= (\pstr{\pview{t' \cdot \nd(m){m} }^M \cdot \nd(lmd-m,60){n}}) \filter  N^{(r')}                                                                  & (\mbox{P-view in $M$}) \\
            &= \pstr[0.4cm]{ \pview{t' \cdot \nd(m){m} }^M \filter N^{(r')} \cdot \nd(lmd-m,30){n} }
                                                                  & (\mbox{$n \in N^{(r')}$}) \\
            ...
            &= \pview{ \Pstr{t' \filter^\star r_0 \cdot (m){m} \cdot {(u \filter^\star r_0)} \cdot (lmd-m,35){n}}}^{M^{(r')}}
                                                               & \parbox[t]{6cm}{(P-view in $M^{(r')}$: nodes in $(u \filter^\star r_0) \cdot n$ are all in $N^{(r')}$
            ...
            &= \pview{ t \filter^\star r_0 }^{M^{(r')}}        & \mbox{(definition of $t$).}
            \end{align*}

    (b) Suppose $n$ is not in $N^{(r')}$ then necessarily its parent node $m$ does not belong to $N^{(r')}$ either.     Therefore by the induction hypothesis, all the nodes in $\pview{t' \cdot m}$ do not belong to
    ...
    \item If $t =  t' \cdot r_0$ where $r_0$ is an occurrence of $r'$, then $r_0 \in N^{(r')}$, $\pview{t}^M = \pview{t'}^M \cdot r_0$. Since $\pview{t}$ is the path from the root to $r'$, all the nodes occurring before $r_0$ in $\pview{t'}$ do not belong to $N^{(r')}$. Hence:
        \begin{align*}
        \pview{t}^M \filter N^{(r')}
            &=  \pview{t'}^M \filter N^{(r')} \cdot r_0 & (\mbox{P-view in $M$})\\
            &=  r_0                                     & (\mbox{nodes in $\pview{t'}_{\prefixof r_0}$ do not belong to $N^{(r')}$}) \\
            &=  \pview{r_0 }^{M^{(r')}}                 & (\mbox{P-view in $M^{(r')}$})\\
            &= \pview{t \filter^\star  r_0 }^{M^{(r')}} & \mbox{($t \filter^\star  r_0 = r_0$)}.
        \end{align*}

    \end{compactitem}


\item (O)  We first show (a), (b) and (c) by induction on the last traversal rule used to form $t$.
The results trivially hold for the case \rulenamet{Empty} and \rulenamet{Root}.
\emph{Step case:}
Let us first consider the rules \rulenamet{Lam}, \rulenamet{App}, \rulenamet{Answer-$\lambda$-var} and
\rulenamet{Answer-$\lambda$-@}. In these cases, $t$'s last node is not in $N_\lambda\union L_{\sf var} \union L_\Sigma \union L_@$ which does not correspond to the hypothesis of (a) therefore we do not need to show (a).
...
\begin{compactitem}
    \item[\rulenamet{Lam}] Take  $t = t' \cdot \lambda \overline{\xi} \cdot n$ where $n$ denotes $\lambda \overline{\xi}$'s child.
        ...
        If $n$ does not appear in $t \filter^\star r_0$ then necessarily its binder does not either hence we can conclude using the induction hypothesis on $t_{\prefixof \lambda \overline{xi}}$.

        If $n$ appears in $t \filter^\star r_0$ then by (P),
        $r_0$ necessarily occurs in the P-view $\pview{t}$ and all the nodes and leaves occurring after
        $r_0$ in $\pview{t}$ are her.\ just.\ by $r_0$ in $t^\star$.
        Hence since all the justifiers of $n$ are in the P-view, they must all be her.\ just.\ by $r_0$ in $t^\star$.
        Consequently $\lambda \overline{x}$ appears in $t \filter^\star r_0$.
    ...
    \item[\rulenamet{Answer-$\lambda$-@}] Take \Pstr[0.5cm]{t = t' \cdot (lmd){\lambda
    \overline{\xi}} \cdot (x){@}  \ldots  (xv-x,50:v){v}_@  \cdot
    (lmdv-lmd,30:v){v}_{\lambda \overline{\xi}} }.

    Clearly since $v_{\lambda \overline{\xi}}$ is a value-leaf, it appears in $t \filter^\star r_0$ if and only its justifier does.

    \item[\rulenamet{Answer-$\lambda$-var}] Take \Pstr[0.5cm]{t = t' \cdot (lmd){\lambda
    \overline{\xi}} \cdot (x){x}  \ldots  (xv-x,50:v){v}_x  \cdot
    (lmdv-lmd,30:v){v}_{\lambda \overline{\xi}} }.
...
For these cases, (b) is a direct consequence of (a).
For (c), it suffices to show that if $t^\omega \in V^{\theroot\vdash}$ then
$t^\omega$ appears in $t \filter^\star r_0$ if and only if $t^{\omega^2}$ does.
We show (a) and (c) by case analysis:
\begin{compactitem}
\item[\rulenamet{InputVar$^{val}$}] Take \Pstr[0.4cm]{t =
    ...
    which contradicts the hypothesis that $t^\omega$ is in $V^{\theroot\vdash}$.
     ...
     \item[\rulenamet{Answer-var-$\lambda$}] Take \Pstr[0.7cm]{t = t' \cdot (y){y} \cdot
    ...
    \item[(Constant rules)] For (a) the proof is similar to the cases \rulenamet{InputVar} and
    \rulenamet{InputVar$^{val}$} (using the fact that constant
    rules are well-behaved).
    For (b) the proof is is similar to the cases \rulenamet{Var} and
    \rulenamet{Answer-@-$\lambda$}.
    \end{compactitem}
    \bigskip

    Now we show (d) by case analysis on the type of the last occurrence in $t$:
    \begin{itemize}
    ...
    By (c), $t'$'s last occurrence appears in $t \filter^\star r_0$ hence we can apply
    the induction hypothesis (O) on $t'$ which gives us $\oview{t}_M = \oview{t' \filter^\star  r_0}_{M^{(r')}}$.  Thus:
        \begin{align*}
    ...
    &= \oview{t' \cdot n}_M \filter^\star r_0 & (\mbox{definition of } t)\\
            ...
            &= \oview{t' \filter^\star  r_0 \cdot n}_{M^{(r')}} & \parbox[t]{7cm}{\raggedleft (O-view in $M^{(r')}$:
             $n$ is indeed a node of in $\tau(M^{(r')})$ by Lemma \ref{lem:instarfilter_imp_insubtree})} \\
...
By (b) the nodes in the O-view are all in the same thread as $n$ and
by (c) all the nodes and leaves occurring after $r_0$ in the O-view appear in
$t \filter^\star r_0$. In particular since $\oview{t} = \oview{t' \cdot m} \cdot n$, $m$ appears in $t \filter^\star r_0$ and we can apply the induction hypothesis (O) on $t' \cdot m$ which gives us $\oview{t' \cdot m}_M \filter^\star r_0 = \oview{(t' \cdot m)\filter^\star r_0 }_{M^{(r')}}$.
Hence:
    \begin{align*}
            \oview{t}_M \filter^\star r_0
            ...
            &= \oview{ \Pstr[0.4cm]{t' \filter^\star r_0 \cdot (m){m} \cdot {(u \filter^\star r_0)} \cdot (lmd-m,30){n}}}_{M^{(r')}}
                    & \parbox[t]{5cm}{\raggedleft(O-view in $M^{(r')}$, nodes in $m\cdot (u \filter^\star r_0) \cdot n$ are all in $N^{(r')}$)} \\
...
\item (T) Suppose $t = t' \cdot n$.
  ...
  \begin{itemize}
    ...
    The last move of $t'$ is in the P-view $\pview{t}$ therefore by (P),
    $t'^\omega$ must occur in $t\filter^\star r_0$. Therefore
    $ \lambda \overline{\xi}$ is the last node in $t'\filter^\star r_0$.
    ...
    We have $t \filter^\star r_0 = (t'\filter^\star r_0) \cdot n$, but in order
     ...
     By (P) we have that $\pview{t'\filter^\star r_0} = \pview{t}_{\suffixof r_0} = r_0 \cdot w$ where all the nodes in
    ...
    \item \rulenamet{App} $t' = \ldots \cdot \lambda \overline{\xi} \cdot @$: the proof is identical to the previous case.
...
\end{corollary}
\begin{proof}
It follows immediately from the fact that, by the previous proposition (O)(b), all the occurrence in $\oview{t}$ belong to the same thread and therefore are all hereditarily justified by $r$.
\end{proof}
...
\rulenamet{Answer-$\lambda$-@} which again violates the second
hypothesis.
...
\rulenamet{Answer-$\lambda$-var} in which case the second hypothesis
is violated; or they were formed with \rulenamet{InputVar$^{val}$}
...
\item If  $n_1,n_2\in L_\lambda$ then either the traversals $t \cdot n_1$
    ...
    \rulenamet{Answer-var-$\lambda$} or they were formed with
    \rulenamet{Answer-@-$\lambda$} but this is impossible since these two rules are
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    Suppose that $n \in N^{(r')}$ then since $n$ is not a lambda node, its parent node
    $\lambda \overline{\xi}$ is necessarily also in $N^{(r')}$ therefore
    ...
    Suppose that $n \not\in N^{(r')}$ then since $n$ is not a lambda node, its parent node is not in $N^{(r')}$ either therefore
...
\end{remark}
    ...
    the jump is permitted by one of the four ``copycat'' rules \rulenamet{Var}, \rulenamet{Answer-@-$\lambda$}, \rulenamet{Answer-$\lambda$-var} or \rulenamet{Answer-var}.
