...
We call \defname{input-variables nodes} the elements of $V_{\sf var}^{r\vdash}$ i.e.\
...
\begin{description}
    \item[\rulenamet{Lam}] If $t \cdot \lambda \overline{\xi}$ is a traversal then so is
        ...
        \begin{compactitem}
            \item If $n \in N_@$ then it has no justifier;
...
where $x \in N_{\sf var}^{r\vdash}$ is the pending node ($?(t_1 \cdot x
...
traversal for some variable $y\not\in N_{\sf var}^{r\vdash}$ then so is \Pstr[0.5cm]{t \cdot (y){y} \cdot (lmd){\lambda
...
Since traversals are well-bracketed, so are the sequences in
...
value-leaves. We introduce the notation $\travset(M)^\star = \{
t^\star \ | \  t \in \travset(M) \}$.

Note that in the safe lambda calculus without interpreted constants,
for $\beta$-normal terms we have $\travset(M) = \travset(M)^{\filter
r } = \travset(M)^\star$.
...
Justification pointers attached to a node $n$ in $t^\star$ can only point to
 ...
 at $r'$. In other words, $n$ must belong to $N^{(r')}$.
\end{proof}
...
For $r'$ a lambda node in $N_\lambda$,
...
\begin{proposition}[Traversal filtering]
    ...
    \begin{itemize}
            ...
            \item all the occurrences in $u$ that are not @-nodes or $\Sigma$-nodes are her.\ just.\ in $t^\star$ by $r_0$
            ...
            If $t$'s last occurrence is not in $N^{(r')}$ then
            all the occurrences in $\pview{t}$ are not in
            $N^{(r')}$ and are not her.\ just.\ by any other
            occurrence of $r'$ in $t^\star$.
        ...
        \item[(O)]
            ...
            Suppose that $t^\omega\in V^{r\vdash}$ and $t^\omega$ is not the occurrence $r_0$.
...
$r'$ in $N_\lambda$. If $t^\omega \in V^{r\vdash}$ and $t^\omega$ appears in $t \filter^\star r_0$ then $\oview{t}_M\filter^\star r_0 = \oview{t\filter^\star r_0}_{M^{(r')}}$
            where $\oview{t}_M \filter^\star r_0$ denotes the
            subsequence of $\oview{t}_M$ consisting of occurrences that
            are her.\ just.\ by $r_0$ in $t^\star$.
            \end{compactitem}
    \end{itemize}
\end{proposition}
...
Property (T) will justify a {\it posteriori} that the sequence $t \filter^\star r_0$ is indeed a
traversal of $\tau(M^{(r')})$ and therefore that the expression $\pview{t
\filter^\star  r_0}^{M{(r')}}$ is well-defined.
...
We do a case analysis on the type of the last node in $t$:
    ...
    Suppose $n$ is in $N^{(r')}$. Since $n$ is not a lambda
    ...
    $u$ are her.\ just.\ by $r_0$ in $t^\star$ and not by
    any other occurrence of $r'$. By P-visibility, $n$'s
    justifiers all appear in $\pview{t'^\omega}$.
    Furthermore, those occurring before $r_j$ do not belong
    to $N^{(r')}$ since they appear on the path from $r'$ to
    the root $r$. Moreover those occurring in $u$ are all
    her.\ just.\ by $r_0$ in $t^\star$. Consequently, $n$ is
    also her.\ just.\ by $r_0$ in $t^\star$, and not by any
    other occurrence of $r_0$.
    ...
    Suppose that $n\in N^{(r')}$. Since $m$ is the parent of
    ...
    occurrences that are not @-node or $\Sigma$-nodes in $u$
    are her.\ just.\ by $r_0$ in $t^\star$ and not by any
    other occurrence of $r'$.

    Since $n$'s only justifier points in $\pview{r' \cdot
    m}$, $n$ is also her.\ just.\ by $r_0$ in $t^\star$ and
    not by any other occurrence of $r'$.
    ...
    Moreover:
            ...
            &= \Pstr{ \pview{t'}^M \filter N^{(r')} \cdot (m){m} \cdot  (lmd-m,60){n} }
                                                                  & (\mbox{$m, n \in N^{(r')}$}) \\
            &= \Pstr{ \pview{t'\filter^\star r_0}^{M^{(r')}}  \cdot (m){m} \cdot  (lmd-m,60){n} }
                                                                  & \mbox{(induction hypothesis)} \\
            &= \pview{ \Pstr{t' \filter^\star r_0 \cdot (m){m} \cdot {(u \filter^\star r_0)} \cdot (lmd-m,35){n}}}^{M^{(r')}}
                                                               & \parbox[t]{6cm}{(P-view in $M^{(r')}$, nodes in $m\cdot (u \filter^\star r_0) \cdot n$ are all in $N^{(r')}$)} \\
    ...
    \item If $t =  t' \cdot r_0$ where $r_0$ is an occurrence of $r'$, then $r_0 \in N^{(r')}$, $\pview{t} = \pview{t'} \cdot r_0$ and:
        ...
        \pview{t}^M \filter N^{(r')}
            ...
            &=  \pview{(t'\filter^\star r_0) \cdot r_0 }^{M^{(r')}}    & (\mbox{induction hypothesis})\\
            &=  \pview{(t' \cdot r_0)  \filter^\star r_0 }^{M^{(r')}}    & (\mbox{$r_0$ is her. just. by itself})\\
            &= \pview{t \filter^\star  r_0 }^{M^{(r')}}                & (\mbox{def. of } t).
        \end{align*}

    \end{itemize}


\item (O) Property (T) will justify a {\it posteriori} that the sequence $t \filter^\star r_0$ is indeed a
traversal of $\tau(M^{(r')})$ and therefore that the expression $\oview{t
\filter^\star  r_0}_{M{(r')}}$ is well-defined.

 We first show (a), (b) and (c) by induction on the last traversal rule used to form $t$.
...
For (c), suppose that $t^\omega\in V^{r\vdash}$ and $t^\omega$ is not the occurrence $r_0$.
...
\begin{compactitem}
    \item[\rulenamet{Lam}] Take  $t = t' \cdot \lambda \overline{\xi} \cdot n$ where $n$ denotes $\lambda \overline{\xi}$'s child.
        ...
        Since $n$ is in $N^{r \vdash}$, $n$ cannot be an application node nor a $\Sigma$-node.
    ...
    therefore it cannot be in $N^{r\vdash}$. Hence this case does not happen.
...
For (c), it suffices to show that if $t^\omega \in V^{r\vdash}$ then
...
\item[\rulenamet{InputVar$^{val}$}] Take \Pstr[0.4cm]{t =
    ...
    $x$ and $v_x$.
        ...
        (c) Suppose that $v_x \in V^{r\vdash}$.
    ...
    \overline{\xi}}}}.
        ...
        (c) Suppose that $v_x$ is in $V^{r\vdash}$, then by (a) so is $v_{\lambda \overline{\xi}}$ and in turn so is its parent node $\lambda
    ...
    which contradicts the hypothesis that $t^\omega$ is in $V^{r\vdash}$.
    ...
    (vy-y,50:v){v}_y } for some variable $y$ not in $N_{\sf var}^{r\vdash}$.
    ...
    are all in the same thread.
       ...
       (c) By (a), the last two nodes in $t$ do not belong to $N_{\sf var}^{r\vdash}$.
           But by assumption $t$'s last node is in $V^{r\vdash}$ therefore this rules cannot be used.
    ...
    Take \Pstr[0.6cm]{ t = t' \cdot (p){p} \cdot (lx){\lambda
        ...
        variable $x_i$ not in $N_{\sf var}^{r\vdash}$.

        (a)Again the induction hypothesis permits us to conclude.

        (c) By (a), the last two nodes in $t$ are both not in $N_{\sf var}^{r\vdash}$.
            But by assumption $t$'s last node is in $V^{r\vdash}$ therefore this rules cannot be used.
    ...
    the induction hypothesis (O) on $t'$ which gives us $\oview{t}_M = \oview{t' \filter r_0}_{M^{(r')}} = \oview{t' \filter^\star  r_0}_{M^{(r')}}$.  Thus:
        \begin{align*}
        \oview{t}_M \filter^\star r_0
    &= \oview{t' \cdot n}_M \filter^\star r_0 & (\mbox{definition of } t)\\
            &= \oview{t'}_M \filter^\star r_0 \cdot n  & (\mbox{O-view computation}) \\
            &= \oview{t' \filter^\star  r_0 }_{M^{(r')}} \cdot n            & (\mbox{induction hypothesis}) \\
            &= \oview{t' \filter^\star  r_0 \cdot n}_{M^{(r')}} & \parbox[t]{7cm}{(O-view in $M^{(r')}$. $n$ is necessarily in $N^{(r')}$ since it is her.\ just.\ by $r_0$ in $t^\star$)} \\
            &= \oview{(t' \cdot n ) \filter^\star  r_0  }_{M^{(r')}}           & (\mbox{$n$ is her.\ just.\ by $r_0$ in $t-@$}) \\
...
by (c) all the nodes and leaves occurring after $r_0$ in the O-view are her.\ just.\ by
$r_0$ in $t$. Thus since $\oview{t} = \oview{t'} \cdot m \cdot n$ we can use the induction hypothesis (O) on $t'$ which gives us $\oview{t'}_M \filter^\star r_0 = \oview{t' \filter^\star r_0 }_{M^{(r')}}$.
Hence: \begin{align*}
            \oview{t}_M \filter^\star r_0
            &= \oview{\Pstr{t' \cdot (m){m} \cdot u \cdot (n-m,40){n}}}_M \filter^\star r_0
                                                                  & (\mbox{definition of } t)\\
            &= (\Pstr{\oview{t'}_M \cdot (m){m} \cdot  (lmd-m,60){n}}) \filter^\star r_0
                                                                  & (\mbox{O-view computation}) \\
            &= \Pstr{ \oview{t'}_M \filter^\star r_0 \cdot (m){m} \cdot  (lmd-m,60){n} }
                                                                  & (\mbox{$m, n$ appear in $t \filter^\star r_0$}) \\
            &= \Pstr{ \oview{t'\filter^\star r_0}_{M^{(r')}}  \cdot (m){m} \cdot  (lmd-m,60){n} }
                                                                  & \mbox{(induction hypothesis)} \\
            &= \oview{ \Pstr{t' \filter^\star r_0 \cdot (m){m} \cdot {(u \filter^\star r_0)} \cdot (lmd-m,35){n}}}_{M^{(r')}}
                                                               & \parbox[t]{6cm}{(O-view in $M^{(r')}$, nodes in $m\cdot (u \filter^\star r_0) \cdot n$ are all in $N^{(r')}$)} \\
            &= \oview{ (\Pstr{t' \cdot (m){m} \cdot u \cdot (lmd-m,35){n}}) \filter^\star r_0 }_{M^{(r')}}
                                                               & (m, n \mbox{ appear in } t \filter^\star r_0) \\
            &= \oview{ t \filter^\star r_0 }_{M^{(r')}}                & \mbox{(definition of $t$).} \qed
            \end{align*}
    \end{itemize}
...
root in $t$. If $t^\omega \in V^{r\vdash}$ then $$\oview{t\filter r} = \oview{t}\filter r = \oview{t} \ .$$
\end{corollary}
\begin{proof}
...
\begin{remark}
\label{rem:inputvar} In the rule \rulenamet{InputVar}, the visited node $n$ is necessarily in $N_\lambda^{r\vdash}$. Indeed since $x$ is in $N^{r\vdash}$, by
the previous proposition all the nodes and leaves in 
$\oview{t_1 \cdot x}$ must be in the same thread as $x$ thus they are all hereditarily justified
...
%%%%%%%%%
    ...
    %where $x \in N^{r\vdash}$ is the pending node in $t$. But then
    %by the induction hypothesis ($ii$), all the nodes in
    %$\oview{t_{\prefixof x}}$ are hereditarily justified by $r$
    %therefore we have $m\in N^{r\vdash}$ which contradicts the case
    %hypothesis $n\not\in N^{r\vdash}$.
    %
...
\begin{lemma}[Filtering lemma for @-free traversals]
...
Conversely, suppose that a node $n \in N\setminus N_@$ is mapped by
...
\item $\psi_M$ preserves hereditary enabling: a node $n \in V^{r\vdash}$ is hereditarily
 enabled by some node $n' \in V^{r\vdash}$ in $\tau(M)$ if and only if
...
\item If  $n_1,n_2\in N_\lambda$ then the traversals $t \cdot n_1$
    ...
    By definition of \rulenamet{InputVar}, $n_1,n_2\in N_\lambda^{r\vdash}$ and the parent node of $n_1$ and the parent node of $n_2$  occur in  $\oview{t_{\prefixof x}}$ where $x \in N^{r\vdash}_{\sf var}$
...
If $n_1 = n_2$ then $ptr(n_1) \neq ptr(n_2)$. An analysis of the traversal rules shows that \rulenamet{InputVar} is the only rule that can visit the same node with two different pointers, thus $n_1,n_2 \in V_\lambda^{r\vdash}$
...
Suppose that $m \in T^{000}$ and $m^1 \in T^{001}$.
    ...
    move in $t'$.
        ...
        Therefore since $m$ and $n$ point to the same position we have $\varphi(t-@) = u$.
                ...
                Suppose $z\not\in N^{r\vdash}$. we have to use the rule $\rulename{InputVar^{val}}$...
}
