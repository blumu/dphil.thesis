...
Hence we can conclude using the induction hypothesis.
  ...
  \item (variable) $M \equiv x_i$. Since $M$ is in $\eta$-long normal form, $x$ must be of ground
    ...
    We refer the reader to Figure \ref{fig:interaction_strategy_denotations} from the previous section for a tree-representation of $\syntrevsem{M}$ which fixes the names of the different games involved in the interaction strategy.
...
\end{enumerate}
    ...
    The proof then follows the same pattern as in the @-application case. In the step case we have $u = u' \cdot m  \in \revsem{\Gamma \vdash M : o}$ for some move $m \in M_T$ with $u' = \varphi_M(t'^\star)$ for some traversal $t'$ of $\tau(M)$. We proceed by case analysis on $m$. It is either played in $A$ or $C$ or it is a profound internal moves in some $B_k$ for $k\in \{0..p\}$:
