...
\begin{compactitem}
    \item[\rulenamet{InputVar}]  Take $t = t' \cdot n$ where $n$ points in $\oview{t'}$.
            Since $n$ points in the $\oview{t'}$, by the I.H., it must belong to the same thread
            as all the nodes in $\oview{t'}$ and in particular to the thread of $t'^\omega$. Therefore both (i) and (ii) hold.

    \item[\rulenamet{InputValue}] Take \Pstr[0.4cm]{t =
        t_1 \cdot (x){x} \cdot t_2 \cdot (xv-x,38:v){v_x} } for some
        $v \in \mathcal{D}$. By remark \ref{rem:traversal_rules}, $x$ must occur in the O-view $\oview{t}$ hence again both (i) and (ii) hold.
