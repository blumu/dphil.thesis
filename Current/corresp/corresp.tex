...
\overline{\xi}$'s child and:
\begin{compactitem}[-]
\item If $n \in N_@$ then it has no justifier;
\item If $n \in N_{\sf var}$ then its main link
points to the only occurrence\footnote{We will show in Prop. \ref{prop:pviewtrav_is_path} that P-views correspond to paths in the tree thus $n$'s enabler occurs exactly once in the P-view. In particular, if $n$ is a free variable node then its justifier is
the root node occurring initially in $\pview{t \cdot \lambda \overline{\xi}}$.} of its enabler in $\pview{t\cdot \lambda \overline{\xi}}$;
\item If $n\in N_{\sf fv}$ then it has auxiliary links
pointing to each of the prime $\lambda$-nodes in $\pview{t\cdot \lambda \overline{\xi}}$.
\end{compactitem}
...
%We can extend the definition of subterm filtering to sequences of $\travset(M)^{-@}$ as follows: for $t \in \travset(M)$ we define
%          $$ (t - @) \subtermfilter \kappa(n) = (t \subtermfilter \kappa(n))-@ \ . $$
%Note that this is well-defined (since $t-@ = t'-@$ implies $t\subtermfilter \kappa(n) = t'\subtermfilter \kappa(n)$).

%Similarly
We can however extend hereditary filtering to sequences of $\travset(M)^{-@}$.
Note that $(t\filter n_0)-@ \subseqof (t-@) \filter n_0$
but $(t\filter n_0)-@ \neq (t-@) \filter n_0$.
%as follows: for $t \in \travset(M)$, for any occurrence $n_0$ of $n$ in $t$ we define
%$$(t-@)\filter n_0 = (t\filter n_0)-@ \ .$$
%It is well-defined (since $t-@ = t'-@$ implies $t\filter n_0 = t'\filter n_0$).
...
%            & \mbox{($\varphi_M = \psi_M \union \Union_{k\in \{0..p\}} \varphi_{N_k}$ by definition).}
    ...
    \begin{enumerate}[(a)]
    \item  Suppose $t_j$'s last move is \emph{not} visited by the rule \rulenamet{InputVar} nor $\rulename{InputVar^{val}}$. Since $\ip( t_j) \subseqof t'$, $t_j$ is a traversal of the subterm $N_j$ of $M$ and the last move of $t$ and $t_j$ coincide,
