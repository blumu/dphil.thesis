...
of the tree, $V$ for the set of vertices (nodes and leaves) of the tree, $N$ for the set of nodes
and $VL$ for the set of value-leaves. Thus $V = N \union VL$.
...
The order of a value-leaf $v \in VL$ is $0$.
\end{definition}
...
$$\begin{array}{rcll}
 ...
 \pview{s \cdot n }  &=&  \pview{s} \cdot n
    & \mbox{for $n \in N_{\sf var} \union N_\Sigma \union N_@ \union VL_\lambda$;}
    \\
 \pview{\Pstr[10pt]{ s \cdot (m){m} \cdot \ldots \cdot (lmd-m,25){n}}} &=&
        \Pstr{ \pview{s} \cdot (m2){m} \cdot (lmd2-m2,60){n} }
    & \mbox{for $n \in VL_{\sf var} \union VL_\Sigma \union VL_@ \union N_\lambda$;}
    \\
 \pview{s \cdot r }  &=&  r
...
$$\begin{array}{rcll}
 ...
 \oview{\Pstr[10pt]{s \cdot (m){m} \cdot \ldots \cdot (x-m,30){n}}} &=&
    ...
    & \parbox[t]{10cm}{for $n \in N_{\sf var} \union VL_\lambda$ (if $n \in N_{\sf var}$ then the represented pointer is its main link);}
    \\
 \oview{s \cdot n }  &=&  n
...
\begin{lemma}
\label{lem:view_of_appfreefilter}
Let $\Gamma(r') \vdash \kappa(r') : T(r')$ be some subterm of $\elnf{M}$ for some $r'\in N_\lambda$. Let $t \in \travset(M)$ and $r_0$ be an occurrence of $r'$ in $t$.
Let $q$ denote $(t-@)$'s last node.
\begin{enumerate}[(i)]
\item If $q$ is her. just. by $r_0$ then all the nodes from $t-@$ occurring in
$\oview{t}$ or $\pview{t}$ are her. just. by $r_0$ in $t-@$.

\item If $q$ is not her. just. by $r_0$ then all the nodes from $t-@$ occurring in $\oview{t}$ or $\pview{t}$ are not her. just. by $r_0$ either.
\end{enumerate}
\end{lemma}
\begin{proof}
...
\item  Suppose $t = t' \cdot @$ then $t-@ = t'-@$,
   $\oview{t} = @$ and $\pview{t'}$ so we can conclude using the induction hypothesis.

\item  Suppose $t = \Pstr[12pt]{ t_1 \cdot (app){@} \cdot (a){\lambda \overline{\xi}}
        \cdot t_2 \cdot (va-a){v_{\lambda \overline{\xi}}} \cdot (vapp-app){v_@}}$ then
          $\pview{t} = \pview{t_1} \cdot @ \cdot v_@$, $\oview{t} = @ \cdot \lambda \overline{\xi}
  \cdot v_{\lambda \overline{\xi}} \cdot v_@$ and
  $t-@ = \Pstr[12pt]{ {(t_1-@)} \cdot (a){\lambda \overline{\xi}}
        \cdot {(t_2-@)} \cdot (va-a){v_{\lambda \overline{\xi}}} }$.

  Let $m$ be a node in $t-@$. If $m$ occurs in $\oview{t}$ then either $m=v_{\lambda \overline{\xi}}$ or $m=\lambda \overline{\xi}$. These two moves are hereditarily justified by the same nodes in $t-@$ therefore
  ($i$) and ($ii$) hold.

  If $m$ occurs in $\pview{t}$ then it also occur in $\pview{t_1 \cdot @ \cdot \lambda \overline{\xi}}$ and in $(t_1\cdot @ \cdot \lambda \overline{\xi})-@$, thus by the induction hypothesis
  on $t_1\cdot @ \cdot \lambda \overline{\xi}$, ($i$) and ($ii$) hold.

  \item  Suppose $t = \Pstr[12pt]{ t_1 \cdot (m){m} \cdot t_2 \cdot (n-m){n}}$
  where $n \in N_{\sf var} \union N_\Sigma \union VL_\lambda$. We have $\pview{t} = \pview{t_1} \cdot m \cdot \pview{t_2} \cdot n$ (by P-visibility of $t$)
  and $\oview{t} = \oview{t_1} \cdot m \cdot n$.
    Since $t$ is P-visible, $n$'s justifiers\footnote{We recall that if $n$ is a variable node then it may have several auxiliary justifiers.} are all in $\pview{t}$.

  Let $p$ be a node occurring in $t-@$.
  ($i$) If $n$ is her. just. by $r_0$ then so is its main justifier $m$.
  Suppose $p\in \pview{t}$.

   therefore by the induction hypothesis,
  all the nodes in $\pview{t_1 \cdot m}$ that are in $t-@$ are her. just by $r_0$.


thus by the induction hypothesis, all $n$'s justifiers occurring in $t-@$ are not her. just. by $r_0$. Consequently neither is $n$. Thus
...
\begin{enumerate}[(i)]
\item $(t-@)\filter r_0 \in \travset^{-@}(\kappa(r'))$,
{\it i.e.}\ $t'-@ = (t-@)\filter r_0$ for some traversal $t' \in \travset(\kappa(r'))$.

\item If $t = \ldots \cdot y \cdot \lambda \overline{\xi}$ where $y$ is a variable node
her. just. by $r_0$ in $t-@$ then either $y$ is a free variable in $M'$ or
$\lambda \overline{\xi}$ is also her. just. by $r_0$ in $t-@$.

\item If $t = \ldots \cdot n \cdot \lambda \overline{\xi}$ where $\lambda \overline{\xi}$ is 
her. just. by $r_0$ in $t-@$ then $n$ is also her. just. by $r_0$ in $t-@$.

\item If $t = \ldots \cdot \lambda \overline{\xi} \cdot y$, then
$\lambda \overline{\xi}$ is her. just. by $r_0$ in $t-@$ if and only if $y$ is.

\end{enumerate}
\end{lemma}
\begin{proof}
  Let $M'$ denote the term $\kappa(r')$, $N'$ be the set of nodes of the tree $\tau(\kappa(r'))$.

  We prove the result by induction on the traversal rules. The base cases \rulenamet{Empty} and \rulenamet{Root} are trivial. Take a traversal $t = u \cdot n \in  \travset(M)$ formed using another rule.
  ...
  By Lemma \ref{lem:view_of_appfreefilter}, $n$ is not in
  Furthermore we have $(t-@)\filter r_0 = (u-@)\filter r_0$ therefore by induction hypothesis on $u$
  property $(i)$ holds.

  Suppose that $u$'s last node is in $O$ then again by visibility and property
  $(iv)$ $n$ is also in $O$. Let us do a case analysis:

  \notetoself{
  \begin{itemize}
    \item \rulenamet{Lam}
    Suppose $t = w \cdot \lambda \overline{\xi} \cdot n \in \travset(M)$.
    By the induction hypothesis, for some traversal $w' \in \travset(\kappa(r'))$ we have
    $w'-@ = ((w \cdot \lambda \overline{\xi}) -@)\filter r_0$.
    W.l.o.g. we can assume that $w'^\omega \neq @$.

    Since $\lambda \overline{\xi} = u^\omega$ is in $O$,
     $w'$'s last node must be $\lambda \overline{\xi}$. Thus we can use the rule \rulenamet{Lam} in $\tau(M')$ to produce the traversal $w' \cdot n$ of $M'$ verifying
    $(w' \cdot n) - @  = ((w \cdot \lambda \overline{\xi} \cdot n) -@)\filter r_0$. Hence ($i$) holds.

    \item \rulenamet{App}
     Suppose $t = w \cdot \lambda \overline{\xi} \cdot @ \cdot n \in \travset(M)$.

     Since $n$ is her. just. by $r_0$ in $t-@$, so is $\lambda \overline{\xi}$ and
     we have $(t-@) \filter r_0 = (((w\cdot \lambda \overline{\xi})-@) \cdot n)\filter r_0 =
                                    ((w\cdot \lambda \overline{\xi})-@)\filter r_0 \cdot n$.
    By the induction hypothesis $((w\cdot \lambda \overline{\xi}) -@)\filter r_0 = w'-@$ for some traversal $w'$ of $M$. Since $\lambda \overline{\xi}$ is her. just. by $r_0$ w.l.o.g. we can assume that the last move of $w'$ is
    $\lambda \overline{\xi}$. Thus we can use the rule \rulenamet{Lam} and \rulenamet{App} in $\tau(M')$ to obtain
    the traversal $w' \cdot @ \cdot n$ of $M'$ and we have:
    $(t-@) \filter r_0 = (w'-@) \cdot n = (w' \cdot @ \cdot n) - @$.

    \item \rulenamet{Answer-@-$\lambda$}, \rulenamet{Answer-$\lambda$-@},
    \rulenamet{Answer-var-$\lambda$} and \rulenamet{Answer-$\lambda$-var}. These cases are all trivial application
...
Conversely, suppose that a node $n \in N\setminus N_@$ is mapped by $\varphi^*(M)$
