...
\subsubsection{Modeling the $\lambda$-calculus in $\mathcal{I}$: examples of revealed semantics}
...
\begin{definition}[The canonical revealed semantics]
\label{dfn:canonical_revealed_semantics} 
The \defname{canonical revealed game denotation} of $M$ written $\revsem{\Gamma \vdash M : A}$ is defined by structural induction on the \emph{$\eta$-long normal form of $M$}:
\begin{eqnarray*}
...
%    } }
%$$
Its canonical revealed denotation is $$\Lambda ( \langle \sem{ x:X \vdash \lambda f . f
x : (o\typear o) \typear o} , \sem{ x:X \vdash \lambda y . y
: o \typear o} \rangle \| ev_2 ) \ .$$
\end{example}


\begin{definition}[Syntactic revealed semantics]
\label{dfn:syntactic_revealed_semantics} The \defname{syntactic revealed game denotation} of $M$ written $\syntrevsem{\Gamma \vdash M : A}$ is defined by structural induction on the \emph{$\eta$-long normal form of $M$}. The equations are the same as in definition \ref{dfn:canonical_revealed_semantics} except for the third case:
\begin{eqnarray*}
...
The syntactic revealed semantics differs from the canonical revealed semantics in that only certain internal moves are preserved during composition: when computing the denotation of an application joint by an @-node in the computation tree, all the internal moves are preserved.
...
\notetoself{ Relate the two denotations: the two semantics are isomorphic: there is a transformation that takes $\revsem{\Gamma \vdash M : A}$ into $\syntrevsem{\Gamma \vdash M : A}$ 
and another one taking $\syntrevsem{\Gamma \vdash M : A}$ into
$\revsem{\Gamma \vdash M : A}$.

Let us denote $\delta$ for the first transformation: what it does is to duplicate certain external moves into internal moves according to the copy-cat strategies $\pi_i$ and $ev$ in the case $y_i N_1 \ldots N_p$ of the definition.}
...
Conversely, the standard semantics can be obtained from the interaction semantics by filtering out all the internal moves:
\begin{eqnarray}
...
$\travset(M)^\star$ to $\revsem{\Gamma \vdash M : T}$:
\begin{eqnarray*}
 \varphi_M  &:& \travset(\Gamma \vdash M : T)^\star \stackrel{\cong}{\longrightarrow} \revsem{\Gamma \vdash M :T} \\
...
\label{rem:corresp_proofreduction}
    ...
    say: $\varphi_M(\travset(M)^\star) = \revsem{\Gamma \vdash M : T}$
    and $\psi_M(\travset(M)^{\filter r}) = \sem{\Gamma \vdash M : T}$.
    Moreover the former implies the latter, indeed:
    \begin{align*}
    \sem{\Gamma \vdash M : T} &= \revsem{\Gamma \vdash M : T} \filter \sem{\Gamma \gamear T} & \mbox{by Eq.\ \ref{eqn:int_std_gamsem}} \\
    ...
    \end{align*}
    therefore we just need to prove $\varphi_M(\travset(M)^\star) = \revsem{\Gamma \vdash M : T}$.
\end{remark}
\smallskip
...
show that $\varphi_M(\travset(M)^\star) = \revsem{\Gamma \vdash M : T}$.
...
\begin{eqnarray*}
\revsem{\Gamma \vdash \lambda \overline{\xi}. N:T} &=& \Lambda^n( \revsem{\overline{\xi}, \Gamma \vdash N: B } ) \\
        &\simeq& \revsem{\overline{\xi}, \Gamma \vdash N: B } \ .
\end{eqnarray*}
...
Hence we can conclude using the induction hypothesis.
  ...
  \item (variable) $M \equiv x_i$. Since $M$ is in $\eta$-long normal form, $x$ must be of ground
      type. The computation tree $\tau(M)$ and the arena $\revsem{\Gamma \gamear o}$ are represented below
      (value leaves and answer moves are not represented):
        ...
        semantics. We have:
        \begin{align*}
        \revsem{M} &= \pi_i = \prefset(\{ \Pstr{(q0){q_0} \cdot (qi){q^i} \cdot (vqi-qi){v_{q^i}} \cdot (vq0-q0){v_{q_0}} } \ | \ v\in \mathcal{D} \})\ .
        \end{align*}
    ...
    We have:
    $$
    \revsem{\Gamma \vdash M : o}
            =  \underbrace{\langle \revsem{\Gamma \vdash N_0 : B_0}, \ldots, \revsem{\Gamma \vdash N_p : B_p} \rangle}_{\Sigma}\ \| \ ev
    $$
    ...
    a tree-representation of $\revsem{M}$ which fixes the names of the different games involved in the interaction strategy.
...
%    \begin{align}
%    \varphi_{M} (\travset(N_0)^\star) &= \revsem{\Gamma \vdash N_0 : B_0} \label{eqn:ih_1} \\
%    \varphi_{M}(\travset(N_1)^\star) &= \revsem{\Gamma \vdash N_1 : B_1} \label{eqn:ih_2}
%    \end{align}
\begin{enumerate}
\item[$\subseteq$]
    We first prove that $\revsem{\Gamma \vdash M : T}
    \subseteq \varphi_{M}( \travset(M)^\star )$. Suppose $u
    \in \revsem{\Gamma \vdash M : T}$. We give a
    ...
    \revsem{\Gamma \vdash M : T}$ for some move $m \in
    M_T$ where $u' = \varphi_M(t'^\star)$ for some traversal
    $t'$ of $\tau(M)$.

    By unraveling the definition of $u \in \revsem{\Gamma \vdash M : T}$ we have:
    \begin{eqnarray*}
    && \left\{
    \begin{array}{ll}
        u \in L_T \\
        ...
        \left\{\begin{array}{ll}
            u \filter T^{0k} \filter b  \in \revsem{N_k} \\
...
$\varphi_M(t^\star) = u$.
    ...
    \item Suppose $m\in T^0$ and $m$ is not initial in $B$. In $u \filter T^0$, $m$ must be hereditarily justified by some initial move $b$ in $B_k$ for some $k\in \{0..p\}$. Since $u \filter T^{0k} \filter b \in \revsem{N_k}$, the outermost induction hypothesis gives us:
        \begin{equation}
...
%        (and therefore so is $\psi_{N_k}(t_k^\omega)$).
    ...
    $$
    \caption{Example of a sequence $u\filter A,B,C$ for $u\in\revsem{M}$ and $l=1$.}
...
\item[$\supseteq$]
  The converse, $\varphi_{M}( \travset(M)^\star) \subseteq \revsem{M}$, is the easy part of the proof.

  Let $u$ be as sequence of $\varphi_{M}( \travset(M)^\star)$. Then
  $u = \varphi_{M}(t^\star)$ for some traversal $t$ of $\tau(M)$. We need to show that
  $u$ is a position of $\revsem{\Gamma \vdash M : T}$ that is to say:
    \begin{eqnarray*}
    && \left\{
    \begin{array}{ll}
        u \in L_T \\
    ...
    By definition, the function $\varphi_{M}$ maps nodes from the subtree of $\tau(M)$ rooted at $\theroot_{k'}$, for any $k'\in \{0..p\}$, to moves of the game $\revsem{\Gamma \vdash B_{k'}}$ that are hereditarily justified by some occurrence of $\varphi_M(\theroot_{k'})$.
    ...
    \begin{align*}
        u \filter T^{0k}\filter b &= \varphi_{M}(t^\star) \filter T^{0k}\filter b \\
         ...
         &\quad = \revsem{N_k} & \mbox{by the induction hypothesis.}
    \end{align*}
...
\end{enumerate}
    ...
    $\revsem{\Gamma \vdash M : o} =  \underbrace{\langle
            \pi_i, \revsem{\Gamma \vdash N_1 : B_1}, \ldots,
            \revsem{\Gamma \vdash N_p : B_p} \rangle}_{\Sigma}
            ;^{\emptyset,\{1..p\}} \ ev$
    is represented on Figure \ref{fig:interaction_strategy_denotations}.

    \begin{description}
    \item[$\subseteq$] 
    Take the revealed strategy $\Sigma \| ev$ where all the internal moves are uncovered:
    ...
    We will use the notation $X_i.B''_k$ to denote the sub-component $B''_k$ of $X_i$.
    
    
    The revealed denotation $\revsem{\Gamma \vdash M : o}$ is obtained from $\Sigma \| ev$ by hiding all the moves from $B_0$ as well as all the superficial internal moves in $B_k$ for $k\in \{1..p\}$ (\ie the ones that take part in the composition of $\Sigma$ with $ev$).
    
    For every interaction play $u$ of $\revsem{\Gamma \vdash M : o}$ we will write $u^\diamond$ to denote its corresponding entirely-uncovered interaction play in $\Sigma \|\ ev$ ending with the same move. Observe that all the moves in $u^\diamond$ that are hidden in $u$ are in fact moves played by some copycat strategy. Indeed, the $\opmove$-moves from $B_0$ are precisely the copies by $ev$ of the O-moves from $C$ and the $\pomove$-moves from $B_k$ for $k \in \{1..p\}$; the $\pomove$-moves from $B_0$ are the copies by $\pi_i$ of the O-move from $X_i$; and for every $k \in \{1..p\}$, the $\opmove$-move from $B_k$ are the copies by $ev$ of the $\pomove$-move from the components $B_k'$ of $B_0$.
    ...
    The proof then follows the same pattern as in the @-application case. In the step case we have $u = u' \cdot m  \in \revsem{\Gamma \vdash M : o}$ for some move $m \in M_T$ with $u' = \varphi_M(t'^\star)$ for some traversal $t'$ of $\tau(M)$.
    
    We proceed by case analysis on $m$.
    It is either played in $A$ or $C$ or it is a profound internal moves in some $B_k$ for $k\in \{0..p\}$:
    ...
    \begin{itemize}
        ...
        \item $m = q_0''$ is the first occurrence in $u$ of the initial $o''$-move. Then we have $u^\diamond = q_0 \cdot q_0' \cdot q_0''$ where $q_0'$ is the initial move of $B_0'$. Therefore we have $u = q_0 \cdot q_0''$ and we can form the traversal $t = \lambda^{[\theroot]} \cdot x_i$ using the rules \rulenamet{Root} and \rulenamet{Lam};
