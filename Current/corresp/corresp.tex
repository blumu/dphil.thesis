...
\cite{hylandong_pcf}. Instead of redoing the proof in our setting,
we will show how the proof of the previous proposition can be
adapted to our setting to give us Proposition
\ref{lem:oview_trav_filtering}. The argument used in
\cite{hylandong_pcf} relies on several properties on the legal
position $s$:
\begin{itemize}
...
O-visibility.
\end{itemize}
Furthermore it relies on the fact that legal positions of the game
$A\rightarrow B$ verify the following properties:
\begin{itemize}
...
Take a traversal $t$. We consider the sequence of
nodes-with-pointers $\hat{t}$ obtained from $\diamond \cdot t$
(where $\diamond$ is a dummy node) by adding justification pointers
going from occurrences of the root $\theroot$, @-nodes and
$\Sigma$-nodes to their predecessor in $t$. In other words,
$\hat{t}$ is $(\diamond \cdot t)^+$ where $t$'s first node points to
$\diamond$ -- the operation $\_^+$ being defined in section
\ref{sec:tstar}.
...
Since @/$\Sigma$-nodes have pointers, $\hat{t}$ is not a proper
justified sequence of nodes as we defined in Def.
\ref{dfn:justseqnode}. Nevertheless, it can be treated as such since
almost all the operations on justified sequences defined up to nwo
can be trivially extended to such sequences. In particular, the
notion of (hereditarily) filtering is defined identically. Let us
now define the O-view and P-view for such sequence. We are going to
make use the notion of ``long O-view'' introduced in
\cite{Harmer2005}:
...
$\longoview{t}$:
\begin{equation}
  \longoview{t} = w \cdot \oview{t} \quad \mbox{for some sequence $w$.}
\end{equation}

In $\hat{t}$, the dummy $\diamond$ node causes a change in the
parity of the alternation between nodes in $N_{\sf var} \union
L_\lambda \union N_@ \union N_\Sigma$ and $N_\lambda \union L_{\sf
var} \union L_\Sigma \union L_@$. To compensate this side-effect,
the notion of O-view and P-view get swapped for $\hat{t}$: we define
the O-view of $\hat{t}$ to be $\hat{s}$ where $s$ is the P-view of
$t$ and the P-view of $\hat{t}$ is defined as $\hat{u}$ where $u$ is
the \emph{long O-view} of $t$:
\begin{equation}
\oview{\hat{s}} = \widehat{\pview{s}} \qquad
\mbox{and}
\qquad  \pview{\hat{s}} =
\widehat{\longoview{s}}
\end{equation}
...
a different correspondence that we will establish in the following
sections between plays of game semantics and traversals of the
computation tree. (In particular the ``coloring'' of nodes in term
of P-move/O-move is the opposite of the one used in the
...
P-view $\pview{\hat{t}}$  & P-view $\pview{s}$\\
O-view $\oview{\hat{t}}$  & O-view $\oview{s}$\\
...
No notion of initiality: all nodes are considered to be non-initial. & Distinction between initial/non-initial move
\end{tabular}
\end{center}

If we ignore auxiliary pointers then the sequence $\hat{t}$ verifies
all the requirement (w1) to (w5): For (w1), the initial node becomes
$\diamond$. Explicit justification (w4) holds provided that we
ignore auxiliary pointers. Finally, alternation (w3),
...
Using the analogy of the preceding table (since all nodes are
considered ``non-initial'' $\hat{t}$) (w8) becomes:
\begin{quote}
 (w8) Suppose $m$ justifies $n$ in $\hat t$. Then $n \in t\filterplus r_0$ if and only if $m\in t\filterplus r_0$.
\end{quote}
Clearly, this condition (more precisely the direct implication) does
not always hold! Indeed, since variable nodes can have several
justifiers, they can very well appear in $t\filterplus r_0$ even
though their main justifier does not. However we know by Lemma
\ref{lem:ifvar_in_tfilterstar_sodoes_certain_of_its_justifier} that
if $r_0$ occurs before $n$'s main justifier then $n$ appears in
$t\filterplus r_0$ if and only if its main justifier does.

Hence Proposition \ref{prop:hylandong_pviewprojection} cannot be
directly reused in our setting. Instead we will show that for
