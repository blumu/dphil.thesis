...
\begin{lemma}[Traversal filtering lemma]
    ...
    Let $t \in \travset(M)$.
    \begin{itemize}
        \item Let $r'\in N_\lambda$.

            \begin{enumerate}
            \item[(T)] If $t$'s last occurrence is her.\ just.\ by
            some occurrence $r_0$ of $r'$ in $t-@$ then
             $(t-@)\filter r_0 \in \travset^{-@}(\kappa(r'))$,
            {\it i.e.}\ $t'-@ = (t-@)\filter r_0$ for some traversal $t' \in
            \travset(\kappa(r'))$.

            \item[(P)] \{DONE\} If $t$'s last occurrence belongs to $N^{(r')}$ ({\it i.e.}\ it is a non-leaf node of the tree $\tau(M^{(r')})$) then there is an occurrence $r_0$ of $r'$ in $t$ such that
            \begin{itemize}
            \item $\pview{t} = \ldots r_0 \cdot u$;
            \item all the occurrences in $u$ that are not @-nodes are her.\ just.\ by $r_0$
            and not her.\ just.\ by any other occurrence of $r'$ in $t-@$;
            ...
            %\item[(*)] Suppose $t = \ldots \cdot m \cdot n$ where
            %$m\in N_{\sf var} \union N_\Sigma \union N_@ \union L_\lambda$
            %and $n\in L_{\sf var} \union L_\Sigma \union L_@ \union
            %N_\lambda$:
            %
            %If $n$ is in $V'$ and is her.\ just.\ by $r_0$ in $t-@$ then so is $m$;
            %
            %If $m$ is in $V'$ and is her.\ just.\ by $r_0$ in $t-@$ then
            %so is $n$.
...
\begin{proof}
We prove (P), (*), (O) and (T) simultaneously by induction on the traversal rules. The base cases \rulenamet{Empty} and \rulenamet{Root} are trivial. \emph{Step case:} Take a traversal $t = u \cdot n \in  \travset(M)$ formed
...
nodes preceding $r_0$ are not in $N^{(r')}$, and by the third point all the nodes in $u$ are in $N^{(r')}$.
    ...
    The node $n$ was necessarily visited with the rule \rulenamet{Lam} since this is the only rule
...
\item (O) We do a case analysis on the type of $t$'s last occurence:
    ...
    Moreover:
            \begin{align*}
            \oview{t}^M \filter  N^{(r')}
            &= \oview{\Pstr{t' \cdot (m){m} \cdot u \cdot (n-m,40){n}}}^M \filter  N^{(r')}
                                                                  & (\mbox{definition of } t)\\
            &= (\Pstr{\oview{t'}^M \cdot (m){m} \cdot  (lmd-m,60){n}}) \filter  N^{(r')}
                                                                  & (\mbox{O-view computation}) \\
            &= \Pstr{ \oview{t'}^M \filter N^{(r')} \cdot (m){m} \cdot  (lmd-m,60){n} }
                                                                  & (\mbox{$m, n \in N^{(r')}$}) \\
            &= \Pstr{ \oview{t'\filter^@ r_0}^{M^{(r')}}  \cdot (m){m} \cdot  (lmd-m,60){n} }
                                                                  & \mbox{(induction hypothesis)} \\
            &= \oview{ \Pstr{t' \filter^@ r_0 \cdot (m){m} \cdot {(u \filter^@ r_0)} \cdot (lmd-m,35){n}}}^{M^{(r')}}
                                                               & \parbox[t]{6cm}{(O-view in $M^{(r')}$, nodes in $m\cdot (u \filter^@ r_0) \cdot n$ are all in $N^{(r')}$)} \\
            &= \oview{ (\Pstr{t' \cdot (m){m} \cdot u \cdot (lmd-m,35){n}}) \filter^@ r_0 }^{M^{(r')}}
                                                               & (m, n \in N^{(r')}) \\
            &= \oview{ t \filter^@ r_0 }^{M^{(r')}}                & \mbox{(definition of $t$).}
            \end{align*}
