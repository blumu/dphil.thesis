...
\begin{definition}[Type-tree]
We call \emph{type decomposition tree} or \emph{type-tree}, a tree
whose leaves are labelled with linear simple types and nodes are
labelled with symbol in $\{ ;, \langle \_\ ,\_ \rangle, \otimes,
\dagger, \Lambda \}$.

Nodes labelled $;$, $\langle \_\ ,\_ \rangle$ or $\otimes$ are binary nodes and nodes labelled $\dagger$ or $\Lambda$ are unary nodes.
...
\item a $\langle \_\ ,\_ \rangle$-node with two children nodes of type $C \multimap A$
and $C \multimap B$ has type $C \multimap A \times B$;

\item a $\otimes$-node with two children nodes of type $A \multimap B$
and $C \multimap D$ has type $(A \otimes C) \multimap (B \otimes D)$;

\item a $;$-node with two children nodes of type $A\multimap B$
and $B \multimap C$ has type $A \multimap C$.
\end{itemize}

Moreover, for the type-tree to be well-defined, it is required that types of children nodes are consistent with the meaning of the parent node, for instance the two children nodes of a ;-node must be of type
...
If $T_1$ and $T_2$ are type-trees we write $\langle T_1, T_2
\rangle$ to denote the tree obtained by attaching $T_1$ and $T_2$ to
a $\langle \_\ ,\_ \rangle$-node. Similarly we use the notations
$T_1 \otimes T_2$, $T_1 ; T_2$, $\Lambda(T_1)$ and  $T_1^\dagger$.
\end{definition}


Let $T$ be a type-tree. Each leaf or node of type $A$ in $T$ can be mapped to the
(standard) game $\sem{A}$. By taking the image of $T$ across this mapping we obtain a tree whose leaves and nodes are labelled by games.
This tree, written $\intersem{T}$, is called an \defname{interaction game}.
We write $root(\intersem{T})$ to denote the game located at the root of the interaction game $\intersem{T}$.

A \emph{revealed strategy} $\Sigma$ on the interaction game $\intersem{T}$ is made of compositions of several standard strategies in which certain internal moves are not hidden. Formally this can be defined as follows:
\begin{definition}[Revealed strategy]
A revealed strategy $\Sigma$ on an interaction game $\intersem{T}$, written
$\Sigma: \intersem{T}$, is an annotated type-tree $T$ where
\begin{itemize}
...
\begin{definition}[Valid strategy]
...
of the arena of which node/leaf-game the move belongs to. Note that a given move may belong to multiple node/leaf-game arena.
The \defname{internal moves} of an interaction play on the game $\intersem{T}$ are those which do not belong to the arena of the game $root(\intersem{T})$.

For any uncovered play $s$ and any interaction game $\intersem{T}$
...
belonging to a node/leaf-game of $\intersem{T}$.

Revealed strategies can alternatively be represented as by means of
sets of uncovered plays. It is defined inductively on the structure of the annotated type-tree defining the interaction strategy:
\begin{itemize}[-]
\item (Leaf $\sem{A}$ annotated by $\sigma :\sem{A}$) The set of plays of the revealed strategy is precisely the set of plays of the standard strategy $\sigma$.
\item ($\otimes$-node with two children strategies $\Sigma_1$ and $\Sigma_2$) $\Sigma$ is the tensor product written $\Sigma_1 \otimes \Sigma_2$ and defines as follows:
$$ $$
\item ($\langle \_ \ , \_ \rangle$-node
with two children strategies $\Sigma_1 : \intersem{T_1}$ and $\Sigma_2 : \intersem{T_2}$ with $root(\intersem{T_1})=C\multimap A$ and  $root(\intersem{T_2})=C\multimap B$:
\begin{eqnarray*}
\langle \Sigma_1, \Sigma_2 \rangle &=& \Sigma_1 [A\times B/A] \\
&\union& \Sigma_2 [A\times B/B]
\end{eqnarray*}
\begin{eqnarray*}
\langle \Sigma_1, \Sigma_2 \rangle &=& \{ s \in \intersem{\langle T_1, T_2 \rangle} \ | \ s \filter \in L_{C\multimap A\&B} \wedge  s \upharpoonright \intersem{T_1} \in \Sigma_1 \wedge s \upharpoonright \intersem{T_2}^-, B = \epsilon  \} \\
&\union& \{ s \in L_{C\multimap A\&B} \ | \ s \upharpoonright \intersem{T_2} \in \Sigma_2 \wedge s \upharpoonright \intersem{T_1}^-, A = \epsilon  \}
\end{eqnarray*}

where $\intersem{T_2}^-$ denotes the set of internal moves of the game $\intersem{T_2}$ {\it i.e.} moves that do not belong to $root(\intersem{T_2})$.

\item ($\dagger$-node with a child strategy $\Pi$) $\Sigma$ is the promotion written $\Pi^\dagger$;
\item ($\Lambda$-node with a child strategy $\Pi$) $\Sigma$ is obtained from  $\Pi$ by retagging the moves appropriately;

\item ($;^U$-node with children nodes $\Sigma_1 : \intersem{T_1}$ and $\Sigma_2 :\intersem{T_2}$)
...
where $int(\intersem{T})$ denotes the set of justified sequences of moves
in (some game of) $\intersem{T}$ and $cover(u,U)$ denotes the
subsequence of $u$ obtained by removing nodes that are in $B_j$
for some $j \in ( 0..l ) \setminus U$ (hence
...
\emph{Remark:} $\mathcal{I}$ is not a category: composition is not associative
and there is no identity interaction strategy.
...
\intersem{\Gamma \vdash f N_1 \ldots N_p : o} &=& \langle \intersem{\Gamma \vdash N_1 : A_1}, \ldots, \intersem{\Gamma \vdash N_p : A_p} \rangle \fatsemi^{\{0..p-1\}} \sem{f} \\
\intersem{\Gamma \vdash N_0 \ldots N_p : o} &=& \langle \intersem{\Gamma \vdash N_0 : A_0}, \ldots, \intersem{\Gamma \vdash N_p : A_p}  \rangle \fatsemi^{\{0..p\}} ev^p
\end{eqnarray*}
  ...
  x$.  Since $M$ is in $\eta$-long normal form, $x$ must be of ground type and since $M$ is
    ...
    \begin{eqnarray*}
      &&      \left\{
            \begin{array}{ll}
                u \in int(\intersem{T})\\
                u \upharpoonright \intersem{T_1}  \in \sigma^\dagger \\
