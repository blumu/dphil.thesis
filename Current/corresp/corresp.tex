...
If we consider the sequence n some more specific cases the equality $\pview{t\filter r} = \pview{t}\filter r$ also holds:
\begin{lemma}
\label{lem:betanf_orde1const_trav_pview_red}
Suppose that $M$ is in $\beta$-normal form and all the $\Sigma$-constants are of order $1$ at most.
Let $t$ be a non-empty traversal of $M$ and $r$ denote the only occurrence of $\tau(M)$'s
