...
children $\Sigma_1 : \intersem{A\gamear
B}$ and $\Sigma_2 : \intersem{B\gamear C}$ then the two
...
We call \defname{internal move} of the game $\intersem{T}$, any $\sim$-class from $M_T$ that does not contain any move from $M_{type(T)}$. We denote the set of all internal moves by $M_T^{\sf int}$. The complement of $M_T^{\sf int}$ in $M_T$, called the set of \defname{external moves}, is denoted by
$M_T^{\sf ext}$.
...
A \defname{justified interaction sequence} of moves on the interaction game $\intersem{T}$ is a sequence of moves from $M_T$ together with pointers where each move in the sequence except the first one has a link attached to it pointing to some preceding move in the sequence.
...
\item Currying: $\Lambda(\Sigma' : \intersem{T'}) : \intersem{T} = \{ u \in L_T \ |  u \filter T' \in \Sigma'  \}$.
...
\item Uncovered composition: Take $\Sigma_1 : \intersem{T_1}$ and $\Sigma_2 :\intersem{T_2}$ then $\Sigma_1\ \| \ \Sigma_2$ is defined on the game $\intersem{T}$ where
...
C_{\sf sup}} \overbrace{M^{\sf ext}_{T_1,B_j} \union M^{\sf ext}_{T_2,B_j})}^{\hbox{superficial moves from $B_j$}}
\union
\Union_{j\in
C_{\sf prof}} \overbrace{M^{\sf int}_{T_1,B_j} \union M^{\sf int}_{T_2,B_j})}^{\hbox{profound moves from $B_j$}} \right)$$
...
\subsubsection{Modeling the $\lambda$-calculus in $\mathcal{I}$: the canonical revealed semantics}

As we have observed in the previous section, depending on the internal moves that we wish to hide, we obtain different possible revealed strategies for a given term. We now present an example of revealed semantics by mapping each term of the
simply-typed lambda calculus to a unique revealed strategy from $\mathcal{I}$ computed from the $\eta$-normal form of the term:

\begin{definition}[Revealed denotation of a term]
\label{dfn:interactionstrategy_ofterms} Let $\pi_i$ denote the
$i^{th}$ projection strategy $\pi_i : \sem{X_1 \gameprod
...
\intersem{\Gamma  \vdash x_i N_1 \ldots N_p :o} &=& \langle \pi_i, \intersem{\Gamma \vdash N_1 : A_1}, \ldots, \intersem{\Gamma \vdash N_p : A_p}  \rangle ; ^{\emptyset,\{1..p\}} ev^p, \quad X_i = A_0 \\
\intersem{\Gamma \vdash f N_1 \ldots N_p : o} &=& \langle \intersem{\Gamma \vdash N_1 : A_1}, \ldots, \intersem{\Gamma \vdash N_p : A_p} \rangle\ \|\ \sem{f}, \quad f : A_0 \in \Sigma \\
...
\begin{remark}
In our revealed semantics only certain internal moves are preserved during composition:
when computing the denotation of an application joint by an @-node in the computation tree, all the internal moves are preserved.
When computing the denotation of
$\intersem{y_i N_1 \ldots N_p}$ for some variable $y_i$, however, we only preserve the internal moves of
...
Observe that the two set of plays $\intersem{\Gamma \vdash M : T}$ and $\sem{\Gamma \vdash M : T}$ are not in bijection.  Indeed, by definition the revealed denotation is prefix-closed therefore it also contains plays ending with an internal move. Thus the revealed denotation contains more plays than the standard denotation.

In fact, the set of plays $\sem{\Gamma \vdash M : T}$ is in bijection with the subset of $\intersem{\Gamma \vdash M : T}$ consisting of plays ending with an external move. Additionally, the set of complete plays of $\sem{\Gamma \vdash M : T}$ is in bijection with the set of complete interaction plays of $\intersem{\Gamma \vdash M : T}$.
...
Since the sequences $t^\star \filter V^{(r')}$ and  $\varphi_M(t^\star) \filter \intersem{\Delta \vdash A }$ have the same justification pointers,
...
\item[$\subseteq$]
    ...
    \begin{array}{ll}
        ...
        \left\{\begin{array}{ll}
            ...
            u \filter T^{0k} \filter b  = \epsilon \quad \mbox{ for every } k\in \{0..p\}\setminus\{j\}
        \end{array}
        \right. \\
...
\smallskip
    ...
    We now show that $t$ is indeed a traversal by case analysis on the rule used to visit the last occurrence of $t_j$ in the tree $\tau(N_j)$:
    \begin{enumerate}[(a)]
    ...
    \item Suppose $t_j^\omega$ is visited with \rulenamet{InputVar}. Then $t_j$ is of the form
...
%        (and therefore so is $\psi_{N_j}(t_j^\omega)$).
    ...
    \item Suppose $t_j$'s last move is visited with the rule $\rulename{InputVar^{val}}$ then the proof is the same as in the previous case but with $\rulename{InputVar^{val}}$ substituted for $\rulename{InputVar}$.
    \end{enumerate}
  ...
  $u$ is a position of $\intersem{\Gamma \vdash M : T}$ that is to say:
    ...
    \begin{array}{ll}
        ...
        \left\{\begin{array}{ll}
            ...
            u \filter T^{0k} \filter b_j  = \epsilon \quad \mbox{ for every } k\in \{0..p\}\setminus\{j\}
        \end{array}
        \right. \\
        u \filter B^1_0 = u \filter B^1_1, \ldots, B^1_p, C
    \end{array}
    ...
    Now for any initial $B$-move $b_j \in B_j$, for some $j\in\{0..p\}$, occurring in $\varphi_{M}(t^\star)$, there is a corresponding occurrence $r_j$
    of a level-$2$ lambda node $\theroot_j$ of $\tau(M)$.

    By definition, for any $k\in \{0..p\}$, the function $\varphi_{M}$ maps nodes from the subtree of $\tau(M)$ rooted at $\theroot_k$ to moves of the game $\intersem{\Gamma \vdash B_k}$ that are hereditarily justified by some occurrence of $\varphi_M(\theroot_k)$.
    Hence for any $k\in \{0..p\}\setminus\{j\}$ we clearly have $\varphi_{M}(t^\star) \filter T^{0k} \filter b_j = \epsilon$.
    Moreover:
    \begin{align*}
...
\end{enumerate}
    ...
    The proof is similar to the previous case with the following differences in the step case of the induction in the ``$\subseteq$'' part of the proof:
    \begin{itemize}
        \item when $m$ is an initial $B_0$-move, the rule \rulenamet{InputVar} is used instead of \rulenamet{App} to visit the node $x$ instead of $@$;
        \item when $m\in C$, the rules \rulenamet{Value$^{\lambda\mapsto{\sf var}}$} and \rulenamet{Value$^{{\sf var}\mapsto\lambda}$} are used instead of \rulenamet{Value$^{\lambda\mapsto@}$} and \rulenamet{Value$^{@\mapsto\lambda}$} respectively;
        \item when $m$ is hereditarily justified by a move $b \in B_j$ for $j\in \{1 .. p\}$ the proof remains unchanged. The case where $m$ is hereditarily justified by a move $b \in B_0$ is treated as follows: $m$ is played by the projection strategy $\pi$ denoting $x$.
