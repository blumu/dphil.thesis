...
\begin{compactitem}
  ...
  \item Suppose that \pstr[0.5cm]{t = \ldots \nd(j0){j_0} \ldots \nd(j1){j_1} \ldots \nd(jk){j_k} \ldots \nd(n-j0,20){y}} where $y\in N_{\sf var}$ and $j_0$, \ldots $j_k$ are the lambda nodes occurring in the $\pview{t}$ (in particular $j_0$ is $y$'s justifier). Then $y$ appears in $t\filterplus r_0$ if and only if for some $i \in \{0..k\}$, $j_i$ appears in $t\filterplus r_0$.
\end{compactitem}
\end{lemma}
\begin{proof}
By remark \ref{rem:tplus} we have $t\filterplus r_0 = (t^+ \filter
r_0)^-$, hence an occurrence appears in $t\filterplus r_0$ if and
only if it is her.\ just.\ by $r_0$ \emph{in $t^+$}. But, provided
that $t^\omega \neq r_0$, $t^\omega$ is her.\ just.\ by $r_0$
\emph{in $t^+$} if and only if one of its justifiers in $t^+$ is. In
particular for the third case, the $j_i$s are precisely (by
definition) the justifiers of $y$ in $t^+$.
\end{proof}
...
\end{lemma}
\proof
Take a prefix $u$ of $t$ ending with a value-leaf $v_n$ of an occurrence $n$.
By Lemma \ref{lem:ifin_projplus_so_does_justifier}, the operation $\_ \filterplus r_0$ removes $v_n$ from $t$ if and only if also removes $n$.
This implies the desired result. \qed
% end of proof
...
If $x$ appears in $t\filterplus r_0$ then by Lemma
\ref{lem:ifin_projplus_so_does_justifier} there must be some lambda
node $j$ occurring (after $m$) in $\pview{t}$ that appears in
$t\filterplus r_0$. Hence we have $m \in \pview{t_{\prefixof j}}$.
By the I.H.\ (ii.a) on $t_{\prefixof j}$, we have that $r_0$ appears
in $\pview{t_{\prefixof j}}$. This P-view is a path to the root
hence it can only contain one occurrence of $r'$. Thus by Lemma
...
Suppose $t^\omega$ has no justifier in $t$ then $t^\omega$ is either the root, an @-node or $\Sigma$-node. If it is the root then the result holds trivially. Otherwise,
by definition of $t\filterplus r_0$, $t^\omega$
appears in $t\filterplus r_0$ if and only if its immediate predecessor does.
Hence we can conclude using the I.H.\ on $\ip(t)$.

Suppose $t^\omega$ is justified by some occurrence $m$. If $m$ is in
$t\filterplus r_0$ then a.\ follows from visibility as well as the
I.H.\ on $t_{\prefixof m}$ whereas b.\ holds trivially. Suppose $m$
is not in $t\filterplus r_0$. Then by Lemma
\ref{lem:ifin_projplus_so_does_justifier} this can only happen if
$n$ is a variable node and some lambda node $j$ occurring in
$\pview{t}$ appears in $t\filterplus r_0$. Hence (a) follows from
the I.H.\ on $t_{\prefixof j}$.

Since $m$ does not appear in $t\filterplus r_0$, by lemma
\ref{lem:pview_trav_projection}, it necessarily precedes $r_0$ in
$t$. Thus $m$ occurs before $r_0$ in the P-view $\pview{t}$. In
other words, $r_0$ lies in the path from $n$ to its binder $m$.
Consequently, $n$ is a free variable node in $\tau(M^{(r')})$.
...
\ref{lem:ifin_projplus_so_does_justifier}} \\
...
\proof
   ...
   \item \rulenamet{InputValue} Take \Pstr[0.4cm]{t =
    ...
    \item \rulenamet{InputVar}  Take $t =  t' \cdot n$ where $n \in N_{\sf var}\union L_\lambda$ points to an occurrence of its parent node $y \in N_{\sf var}^{\theroot\vdash}$ in $\oview{t}$.
...
T} = \varphi_M(t^\star\filter V^{\theroot\vdash})$ which is equal to
...
$\varphi_M(t^\star) = \varphi_M(t'^\star) \cdot m = u$.
    ...
    \item Suppose $m\in T^0$ and $m$ is not initial in $B$. Then $m$ is hereditarily justified by some initial move $b$ in $B_j$ for some $j\in \{0..p\}$.
        Since $u \filter T^{00j} \filter b \in
        \intersem{N_j}$, the outermost induction
        hypothesis gives us:
        \begin{equation}
        u \filter T^{00j} \filter b = \varphi_{N_j}(t_j^\star)  \label{eqn:corresp_outmost_ih}
        \end{equation}
          for some traversal $t_j \in \travset(N_j)$
          where w.l.o.g.\ we can assume that $t_j^\omega
          \not\in V_@$. We have:
        \begin{align*}
