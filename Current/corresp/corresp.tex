...
The function $\psi^{n,q}_A$ from $D_n$ to $A$ is defined as:
        ...
        \psi^{n,q}_A &=& \{ n \mapsto q \} \union  \{ v_n \mapsto v_q \ | \ v \in \mathcal{D} \}\\
         &&\union \left\{
                        \begin{array}{ll}
                          \Union_{m \in N | n \vdash_i m} \psi^{m, q^i}_A, & \hbox{if $n\in N_{\lambda}$\ ;} \\
...
%    for $\psi^{n,m}_{\sem{T(n)}} : V^{n\vdash} \rightarrow
%    \sem{T(n)}$ where $T(n)$ denotes the type of $\kappa(n)$ and
%    $m$ denotes $\sem{T(n)}$'s initial move.
...
& \union& \Union_{n \in N | r \vdash_i n} \psi^{n, q^i}_\sem{\Gamma\rightarrow T} \\
& \union& \Union_{n \in N | n \mbox{ \small labelled } x_j, j \in \{ 1..k \} } \psi^{n, q_{x_j}}_\sem{\Gamma\rightarrow T}
    \end{eqnarray*}
    \end{itemize}
\end{definition}
...
Let $\Delta \vdash \kappa(r') : A$ be some subterm of $\elnf{M}$ for some $r'\in N_\lambda$. Let $t \in \travset(M)$ and $r_0$ be an occurrence of $r'$ in $t$. Then
...
\begin{lemma}[@-free traversal filtering lemma]
...
that $n_1$ and $n_2$ belong to the same category of node i.e.\ they
...
in which case $n_1$ and $n_2$ are two value leaves belonging to
$N^{r\vdash}_\lambda$ and justified by the same input variable node.
...
\item If  $n_1,n_2\in N_\lambda$ then the traversals $t \cdot n_1$
    ...
    Now the rule \rulenamet{Root}, \rulenamet{App} or \rulenamet{Var} are all deterministic
    therefore since $n_1\neq n_2$ the rule used is necessarily \rulenamet{InputVar}.

    By definition of \rulenamet{InputVar}, $n_1$ and $n_2$ are two child
    nodes in $N_\lambda^{r\vdash}$ of two occurrences of some node occurring in
    $\oview{t_{\prefixof x}}$ where $x \in N^{r\vdash}_{\sf var}$
    denotes the pending node at $t$. If $n_1$ and $n_2$ have the same
    parent node in $\tau(M)$ then since $n_1\neq n_2$, by definition of
    $\varphi$, $\varphi(n_1)\neq \varphi(n_2)$. If their parent node is
    different, then $n_1$ and $n_2$ are justified by two different
