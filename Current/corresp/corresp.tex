...
O-view of its reduction (Lemma \ref{lem:trav_filtering}).
\vspace{8pt}
...
If $n$ is a leaf $v_m$ then we represent the link in the sequence as
...
\begin{lemma}[Traversal filtering lemma]
\label{lem:trav_filtering} Let $\Gamma(r') \vdash \kappa(r') :
T(r')$ be some subterm of $\elnf{M}$ for some $r'\in N_\lambda$. Let
$t \in \travset(M)$ and $r_0$ be an occurrence of $r'$ in $t$. Let
$M'$ denote the term $\kappa(r')$ and $N'$ be the set of nodes of
the tree $\tau(\kappa(r'))$.

\begin{enumerate}
\item[(T)] If $t$'s last node is her.\ just.\ by $r_0$ in $t-@$ then
 $(t-@)\filter r_0 \in \travset^{-@}(\kappa(r'))$,
{\it i.e.}\ $t'-@ = (t-@)\filter r_0$ for some traversal $t' \in
\travset(\kappa(r'))$.

\item[(Var$\leftrightarrow\lambda$)] Suppose $t = \ldots \cdot m \cdot n$ where
$m\in N_{\sf var} \union N_\Sigma \union N_@ \union L_\lambda$
and $n\in L_{\sf var} \union L_\Sigma \union L_@ \union
N_\lambda$.

If $n$ is her.\ just.\  by $r_0$ in $t-@$ then so is $m$.

If $m$ is her.\ just.\  by $r_0$ in $t-@$ and $m \in N'$ then
$n$ is her.\ just.\  by $r_0$ in $t-@$ .

\item[(O)]  hence if $t$'s last node is her.\ just.\ in $t-@$ by $r_0$ then all the nodes from $t-@$ occurring in
$\oview{t}$ are also her.\ just.\ by $r_0$ in $t-@$ and
therefore $\oview{t}_M \filter N' = \oview{t\filter^{@}
r_0}_{M'}$.



\item[($\lambda\rightarrow$Var)] \{DONE 3\} Suppose $t = \ldots \cdot \lambda \overline{\xi}\cdot y$.

If $\lambda \overline{\xi}$ is  her.\ just.\ by $r_0$ in $t-@$
then so is $y$.


\item[($\lambda\leftarrow$Var)] Suppose $t = \ldots \cdot \lambda \overline{\xi} \cdot y$.

 If $y$ is her.\ just.\  by $r_0$ in $t-@$ then so is $\lambda
\overline{\xi}$.

\item[(P)] \{DONE 1\} hence if $t$'s last node is not a leaf and it is her.\ just.\ by $r_0$ in $t-@$ then
$\pview{t}^M \filter N' = \pview{t\filter^@ r_0}^{M'}$ and $r_0$
occurs in $\pview{t}$;

\item[(*)] \{DONE 2\} If $n$ is her.\ just.\ by $r_0$ in $t-@$ then $n \in N'$.
\end{enumerate}
\end{lemma}
\begin{proof}
  We prove all the properties simultaneously by induction on the traversal rules. The base cases \rulenamet{Empty} and \rulenamet{Root} are trivial.
    \emph{Step case:} Take a traversal $t = u \cdot n \in  \travset(M)$ formed
    using another rule and suppose that the property is verified for all traversal shorter than $t$:
%  Suppose $u$'s last node is not in $(t-@)\filter r_0$. By visibility of traversals,
%  $n$'s justifiers\footnote{We recall that if $n$ is a variable node then it may have auxiliary justifiers.} are in $\pview{u}$ or $\oview{u}$ (depending on the type of $n$), thus by
%  Lemma \ref{lem:view_of_appfreefilter}, all $n$'s justifiers
%  are not in $(t-@)\filter r_0$. Consequently $n\not\in (t-@)\filter r_0$ and therefore by the induction hypothesis $(v)$ holds for $t$.
%  Furthermore we have $(t-@)\filter r_0 = (u-@)\filter r_0$ therefore by induction hypothesis on $u$
%  property $(i)$ holds.
   Since $n$ is her.\ just.\ by $r_0$ in $t-@$, by ($ii$) and (L$\leftarrow$V),
  $u$'s last node is also her.\ just.\ by $r_0$ in $t-@$.


\indent For $(T)$, $(Var$\leftrightarrow\lambda$)$,
(L$\rightarrow$V), (L$\leftarrow$V) and $(*)$, we do a case analysis
on the last traversal rule used:
  ...
  \begin{itemize}
    ...
    traversal $w' \in \travset(\M')$ we have $w'-@ = ((w \cdot
    \lambda \overline{\xi}) -@)\filter r_0$. W.l.o.g. we can
    assume that $w'^\omega \neq @$.

    Since $\lambda \overline{\xi} = u^\omega$ is in $O$, $w'$'s
     last node must be $\lambda \overline{\xi}$. Thus we can use
     the rule \rulenamet{Lam} in $\tau(M')$ to produce the
     traversal $w' \cdot n$ of $M'$ verifying
    ...
    and \rulenamet{App} in $\tau(M')$ to obtain the traversal
    $w' \cdot @ \cdot n$ of $M'$ and we have: $(t-@) \filter r_0
...
\noindent (P) We do a case analysis on the type of $t$'s last node:
\begin{itemize}
...
N_\Sigma \union N_@$. By Lemma \ref{lem:trav_last_not_leaf},
$t'^\omega \in N_\lambda$ therefore by ($\lambda\leftarrow$Var),
$t'^\omega$ is also her. just. by $r_0$ in $t-@$ and $t'^\omega$
is not a leaf, thus we can use the induction hypothesis on $t'$:
    \begin{align*}
    \pview{t}^M \filter N'
&= \pview{t' \cdot n}^M \filter  N' & (\mbox{definition of } t)\\
        &= (\pview{t'}^M \cdot n) \filter  N'  & (\mbox{P-view computation}) \\
        &= \pview{t'}^M \filter  N'  \cdot n             & (\mbox{$n\in N'$ by (*)}) \\
        &= \pview{t' \filter^@  r_0 }^{M'} \cdot n            & (\mbox{induction hypothesis}) \\
        &= \pview{t' \filter^@  r_0 \cdot n }^{M'} & (\mbox{P-view in $M'$}) \\
        &= \pview{(t' \cdot n ) \filter^@  r_0  }^{M'}           & (\mbox{$n$ is her.\ just.\ by $r_0$ in $t-@$}) \\
        &= \pview{t \filter^@  r_0  }^{M'}
 & (\mbox{definition of } t).
    \end{align*}
    Furthermore $\pview{t} = \pview{t'} \cdot n$. By the
    induction hypothesis, $r_0 \in \pview{t'}$ therefore $r_0
    \in \pview{t}$.
...
where $n \in N^{r'\vdash}_\lambda$ is her.\ just.\ by $r_0$ in
$t-@$ and $n\neq r'$, then necessarily $m\in N_{\sf var} \union
N_\Sigma \union N_@$ and $m$ is also her.\ just.\ by $r_0$ in
$t-@$. By Lemma \ref{lem:trav_last_not_leaf}, $t'^\omega$ is not
a leaf, and by ($\lambda\leftarrow$Var), $t'^\omega$ is her.\
just.\ by $r_0$ in $t-@$, thus we can apply the induction
hypothesis on $t'$:
        \begin{align*}
        \pview{t}^M \filter  N'
        &= \pview{\Pstr{t' \cdot (m){m} \cdot u \cdot (n-m,40){n}}}^M \filter  N'
                                                              & (\mbox{definition of } t)\\
        &= (\Pstr{\pview{t'}^M \cdot (m){m} \cdot  (lmd-m,60){n}}) \filter  N'
                                                              & (\mbox{P-view computation}) \\
        &= \Pstr{ \pview{t'}^M \filter N' \cdot (m){m} \cdot  (lmd-m,60){n} }
                                                              & (\mbox{$m, n \in N'$ by (*)}) \\
        &= \Pstr{ \pview{t'\filter^@ r_0}^{M'}  \cdot (m){m} \cdot  (lmd-m,60){n} }
                                                              & \mbox{(induction hypothesis)} \\
        &= \pview{ \Pstr{t' \filter^@ r_0 \cdot (m){m} \cdot {(u \filter^@ r_0)} \cdot (lmd-m,35){n}}}^{M'}
                                                           & \parbox[t]{6cm}{(P-view in $M'$, nodes in $m\cdot (u \filter^@ r_0) \cdot n$ are all in $N'$ by (*))} \\
        &= \pview{ (\Pstr{t' \cdot (m){m} \cdot u \cdot (lmd-m,35){n}}) \filter^@ r_0 }^{M'}
                                                           & (m, n \in N') \\
        &= \pview{ t \filter^@ r_0 }^{M'}                & \mbox{(definition of $t$).}
        \end{align*}
    Furthermore $\pview{t} = \pview{t' \cdot m} \cdot n$. By the
    induction hypothesis, $r_0 \in \pview{t' \cdot m}$ therefore
    $r_0 \in \pview{t}$.

\item If $t =  t' \cdot r_0$ then clearly $r_0 \in \pview{t}$ and:
    \begin{align*}
    \pview{t}^M \filter N'
        &=  \pview{t' \cdot r_0 }^M \filter N'  & (\mbox{def. of } t)\\
        &=  \pview{t'}^M \filter N' \cdot r_0 & (\mbox{def. P-view})\\
        &=  \pview{(t'\filter^@ r_0) \cdot r_0 }^{M'}    & (\mbox{induction hypothesis})\\
        &=  \pview{(t' \cdot r_0)  \filter^@ r_0 }^{M'}    & (\mbox{$r_0$ is her. just. by itself})\\
        &= \pview{t \filter^@  r }^{M'}                & (\mbox{def. of } t).
    \end{align*}
...
\indent ($\lambda\rightarrow$Var) The node $y$ was necessarily
visited with the rule \rulenamet{Lam} since this is the only rule
which can visit variable nodes. Thus $y$ points to all the prime
lambda-nodes in $\pview{t_{\prefixof \lambda \overline{\xi}}}$ as
well as the first $\lambda$-node in the P-view. But by (P), $r_0$
must appear in $\pview{t_{\prefixof \lambda \overline{\xi}}}$ and
the node $r_0$ is precisely either a prime node or the root node $r$
in which case it must be the first node of $\pview{t_{\prefixof
\lambda \overline{\xi}}}$. Hence $y$ necessarily points to $r_0$ and
thus $y \in O$.
\smallskip

\indent ($\lambda\leftarrow$Var) By (P), $r_0$ appears in
$\pview{t_{\prefixof \lambda \overline{\xi}}}$. Therefore since the
P-view is the path to

By P-visibility, $y$ points in $\pview{t_{\prefixof \lambda
\overline{\xi}}}$, and
\smallskip

\indent (O) By case analysis similarly to (P).

\end{proof}


\begin{proof}
  By induction on the length of the traversal. It is trivial for the empty traversal.
\begin{itemize}
\item  Suppose $t = t' \cdot @$ then $t-@ = t'-@$,
   $\oview{t} = @$ and $\pview{t'}$ so we can conclude using the
   induction hypothesis.

\item  Suppose $t = \Pstr[12pt]{ t_1 \cdot (app){@} \cdot (a){\lambda \overline{\xi}}
        \cdot t_2 \cdot (va-a){v_{\lambda \overline{\xi}}} \cdot
          (vapp-app){v_@}}$ then $\pview{t} = \pview{t_1} \cdot
          @ \cdot v_@$, $\oview{t} = @ \cdot \lambda
          \overline{\xi}
  ...
  If $m$ occurs in $\pview{t}$ then it also occur in $\pview{t_1
  \cdot @ \cdot \lambda \overline{\xi}}$ and in $(t_1\cdot @
  \cdot \lambda \overline{\xi})-@$, thus by the induction
  hypothesis on $t_1\cdot @ \cdot \lambda \overline{\xi}$, ($i$)
  and ($ii$) hold.

  \item  Suppose $t = \Pstr[12pt]{ t_1 \cdot (m){m} \cdot t_2 \cdot (n-m){n}}$
  where $n \in N_{\sf var} \union N_\Sigma \union L_\lambda$. We
  have $\pview{t} = \pview{t_1} \cdot m \cdot \pview{t_2} \cdot
  n$ (by P-visibility of $t$) and $\oview{t} = \oview{t_1} \cdot
  m \cdot n$.
    Since $t$ is P-visible, $n$'s justifiers\footnote{We recall
    that if $n$ is a variable node then it may have several
    auxiliary justifiers.} are all in $\pview{t}$.

  Let $p$ be a node occurring in $t-@$. ($i$) If $n$ is her.
  just. by $r_0$ then so is its main justifier $m$. Suppose
  $p\in \pview{t}$.

   therefore by the induction hypothesis, all the nodes in
  $\pview{t_1 \cdot m}$ that are in $t-@$ are her. just by
  $r_0$.


thus by the induction hypothesis, all $n$'s justifiers occurring
in $t-@$ are not her.\ just.\  by $r_0$. Consequently neither is
$n$. Thus

   or $\oview{t_{<n}}$ (depending on the type of $n$),
  \end{itemize}
\end{proof}
...
only occurrence of the root in $t$.

If $t^\omega \in V^{r\vdash}$ ({\it i.e.}~$t$'s last node is
hereditarily justified by $r$) then
\begin{enumerate}[(i)]
\item if $t^\omega \in N$ ({\it i.e.}~$t$'s last node is not a leaf) then $ \pview{t \filter  r } = \pview{t}$ \ ;
\item $\oview{t \filter r } = \oview{t}$\ .
\end{enumerate}
\end{corollary}
\begin{proof}
...
\label{lem:varphi_filter} Let $r'\in N_\lambda$, $\Delta \vdash M' :
...
$$
\end{lemma}
\proof ($i$) Firstly, by Lemma \ref{lem:trav_filtering},
$(t-@)\filter r'_0$ is a traversal of $\travset(M')$ therefore
$\varphi_{M'}((t-@)\filter r'_0)$ is well-defined. By Definition
\ref{dfn:phi_for_justsequ} we must have $\varphi_{M}((t-@)\filter
r'_0) = \varphi_M(t-@) \filter m_0$. Furthermore, because of the
inductive definition of $\varphi_{M}$, $\varphi_{M'}$ coincides with
...
If we can form a traversal $t' \cdot n$ of $\tau(M')$ using a rule different from \rulenamet{InputVar} and $\rulename{InputVar^{val}}$ then
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    $t' \cdot \lambda \overline{\xi} \filter M' \in \travset(M')$.

    Suppose that $n \in N'$ then since $n$ is not a lambda node, its parent node
    $\lambda \overline{\xi}$ is necessarily also in $N'$ therefore
    $(t'\filter M') \cdot \lambda \overline{\xi} \in \travset(M')$ so we can use the rule \rulenamet{Lam} in $\tau(M')$ to produce the traversal
    $(t'\filter M') \cdot \lambda \overline{\xi} \cdot n =
    (t' \cdot \lambda \overline{\xi} \cdot n ) \filter M'$ of $M'$.


    Suppose that $n \not\in N'$ then since $n$ is not a lambda node, its parent node is not in $N'$ either therefore
    $(t' \cdot \lambda \overline{\xi} \cdot n) \filter M'
    = t' \filter M' \in \travset(M')$.

    \item \rulenamet{App}
     Suppose $t = t' \cdot @ \cdot n \in \travset(M)$.
    By the induction hypothesis,
    $( t' \cdot @ ) \filter M' \in \travset(M')$.
...
%            & \mbox{($\varphi_M = \psi_M \union \Union_{k\in \{0..p\}} \varphi_{N_k}$ by definition).}
    ...
    \begin{enumerate}[(a)]
    \item  Suppose $t_j$'s last node is \emph{not} visited by the rule \rulenamet{InputVar} nor $\rulename{InputVar^{val}}$. Since $\ip( t_j) \subseqof t'$, $t_j$ is a traversal of the subterm $N_j$ of $M$ and the last node
