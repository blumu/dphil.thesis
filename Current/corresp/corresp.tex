...
\emph{Notations:} We write $\theroot$ to denote the root of $\tau(M)$.
We write $E$ to denote the parent-child relation of the tree, $V$ for the set
...
A \defname{justified sequence of nodes} is a sequence of nodes $s$
...
$$\begin{array}{rcll}
 ...
 \oview{\Pstr[10pt]{s \cdot (m){m} \cdot \ldots \cdot (x-m,30){n}}} &=&
    ...
    & \parbox[t]{10cm}{for $n \in N_{\sf var} \union L_\lambda$ (if $n \in N_{\sf var}$ then the pointer considerd is its main link);}
    \\
 \oview{s \cdot n }  &=&  n
...
where $x \in N_{\sf var}^{\theroot\vdash}$ is the pending node ($?(t_1 \cdot x
\cdot t_2)^\omega=x$) then so is \Pstr[0.5cm]{t_1 \cdot
(x){x} \cdot t_2 \cdot (xv-x,38:v){v_x} } for all $v \in
\mathcal{D}$.

\item[\rulenamet{InputVar}] If $t$ is a traversal with
...
%%%%%%%%%%%%%
% The following formulation is equivalent to the preceding one:
%
...
traversal where $y\in N_{\sf var} \setminus N_{\sf var}^{\theroot\vdash}$ then so is \Pstr[0.5cm]{t \cdot (y){y} \cdot (lmd){\lambda
...
If \Pstr[0.5cm]{t' \cdot (n){n} \cdot (lx){\lambda \overline{x}}
    ...
    $x_i \in N_{\sf var} \setminus N_{\sf var}^{\theroot\vdash}$ then
    ...
    $$\rulef{t = t_1\cdot c \cdot t_2 \in \travset(M) \quad ?(t)^\omega = c \quad P(t)}
      {
        \stackrel{\rule{0pt}{3pt} }{
            \Pstr[8pt]{ t' = t_1\cdot (c){c} \cdot t_2 \cdot (v-c,35){v(t)} \in {\travset(M)}}
            }
       }
       $$
    where:
    \begin{compactitem}
      \item $c$ is a constant node ($c \in N_{\Sigma}$);
...
where:
    \begin{compactitem}
      \item $x$ is a variable node her.\ just.\ by some constant node ($x \in N^{N_\Sigma\vdash}_{\sf var}$);
      \item $P(t)$ is a predicate expressing some condition on $t$;
      \item where $n(t)$ is a lambda-node determined by $t$, and its link, also determined by $t$, points to some occurrence of its parent node in $\oview{t_{\prefixof x}}$.
    \end{compactitem}
\end{enumerate}
...
\begin{remark}
    The definition of well-behavior is in fact derived from the definition of the rules \rulenamet{InputValue} and \rulenamet{InputVar}.  In fact while we can regard uninterpreted constants as free variables, interpreted constants can be seen as a generalization of the notion of free variable. The extra power of the well-behaved constant rules
...
(t-\Sigma)-@$. We write $t^\star$ to denote $(t-@)-\Sigma$ {\it
i.e.}\ the sequence obtained from $t$ by removing all the @-nodes as
well as the constant nodes together with their associated
value-leaves. We introduce the set
...
Let us write $t^+$ for the sequence-with-pointers obtained from $t$ by adding
pointers further pointers as follows: for every occurrence of
a $@$ or $\Sigma$ node $m$ in $t$ we add a pointer going from $m$ to its predecessor in $t$ (which is necessarily an occurrence of its parent node).

For any traversal $t\in \travset(M)$ and node $n$ occurring in $t$,
we write $t \filter^\star n$ to denote the sequence $t^+ \filter n$: the
sequence of nodes in $t$ that are hereditarily justified by $n$ \emph{in $t^+$}.
In essence, the effect of this operation is to compute the sequence $((t-@-\Sigma)\filter n)+\Sigma+@$ (provided that $t$ does not end with an @/$\Sigma$-node).

We extend this definition to any subsequence of $t$: if $s$ is a justified subsequence of $t$
then $s \filter^\star n$ denotes the sequence of nodes in $s$ that are hereditarily justified by $n$ \emph{in $t^+$}.


If $n$ is an occurrence of a lambda node $r' \in N_\lambda$ then we will call $t \filter^\star n$
a \defname{sub-traversal of the computation tree $\tau(M)$}. This appellation will be justified by Proposition
\ref{prop:trav_filtering} stating that $t \filter^\star n$ is a traversal of the sub-computation tree
of $\tau(M)$ rooted at $r'$.
\bigskip
...
\begin{compactitem}
  \item Suppose that \Pstr[0.5cm]{t = \ldots (j){j} \ldots (vj-j){v_j}} with $v_j \in L = L_@ \union L_\Sigma \union L_{\sf var} \union L_\lambda$.
      Then $v_j$ appears in $t\filter^\star r_0$ if and only if
      $j$ appears in $t\filter^\star r_0$.
  \item Suppose that \Pstr[0.5cm]{t = \ldots (j){j} \ldots (n-j){\lambda \overline{\xi}}} where $\lambda \overline{\xi} \in N_\lambda$
          ...
          Then $\lambda \overline{\xi}$ appears in $t\filter^\star r_0$ if and only if $j$ appears in $t\filter^\star r_0$.
  ...
  \item Suppose that \pstr[0.6cm]{t =
    ...
    Then $y$ appears in $t\filter^\star r_0$ if and only if 
      for some $i \in \{0..k\}$, $j_i$ appears in $t\filter^\star r_0$.
\end{compactitem}
\end{lemma}
\begin{proof}
  It is a direct consequence of the definition of $t\filter^\star r_0$: if $t$'s last occurrence is not $r_0$ then it appears in
  $t\filter^\star r_0$ if and only if one of its justifiers appears in $t\filter^\star r_0$.
\end{proof}
For the third case, we will see that in fact
all $y$'s auxiliary justifiers occurring after $j_i$ must also appear in $t\filter^\star r_0$. (This is due to the fact that the auxiliary justifiers all appear in the P-view and that by Lemma \ref{lem:pview_trav_filtering} (to be stated later on) all the nodes in the P-view occurring after $r_0$ also appear in $t\filter^\star r_0$).
...
\begin{lemma}
 ...
 We have:
  $$?(t\filter^\star r_0) = ?(t) \filter^\star r_0 \ .$$
\end{lemma}
\proof
 By definition, the operation $t \filter^\star r_0$ removes an occurrence of a value-leaf in $t$
...
\begin{lemma}
\label{lem:instarfilter_imp_insubtree}
Let $r_0$ be an occurrence in $t$ of a lambda-node $r' \in N_\lambda$.
If $n$ occurs in $t \filter^\star r_0$ then $n$ is in $V^{(r')}$.
\end{lemma}
\begin{proof}
...
\label{lem:pview_trav_filtering}
   Let $t \in \travset(M)$ such that $t^\omega \not\in L_\lambda$.
        ...
        \item all the occurrences in $u$ must appear in $t\filter^\star r_0$ and do not appear
        in any sequence $t\filter^\star r_1$ for any occurrence $r_1$ of $r'$ different from $r'$;
        \item $\pview{t}^M \filter V^{(r')} = r_0\cdot u =  \pview{t\filter^\star r_0}^{M^{(r')}}$;
        \end{itemize}

        \item[(b)] If $t$'s last occurrence is not in $V^{(r')}$ then
        all the occurrences in $\pview{t}$ are not in
        $V^{(r')}$ and do not appear in any sequence $t\filter^\star r_0$ for some
...
\in \travset(M)$ such that $|t|>1$. We do a case analysis on the type of the last occurrence in $t$:
    ...
    $u$ appear in $t\filter^\star r_0$ and not in any
    other sequence $t\filter^\star r_1$ for any occurrence $r_1\neq r_0$ of $r'$. By P-visibility, $n$'s
    justifiers all appear in $\pview{t'}$:
    those occurring before $r_j$ do not belong
    to $V^{(r')}$ since they appear on the path from $r'$ to
    the root $r$; and those occurring in $u$ all appear in $t\filter^\star r_0$.
    Consequently, $n$ must also appear in $t\filter^\star r_0$ and not in any
    other sequence $t\filter^\star r_1$ for any occurrence $r_1\neq r_0$ of $r'$.
    ...
    &= \pview{t' \cdot n}^M \filter  V^{(r')} & (\mbox{definition of } t)\\
            ...
            &= \pview{t' \filter^\star  r_0 }^{M^{(r')}} \cdot n            & (\mbox{induction hypothesis}) \\
            &= \pview{t' \filter^\star  r_0 \cdot n }^{M^{(r')}} & (\mbox{P-view in $M^{(r')}$}) \\
            &= \pview{(t' \cdot n ) \filter^\star  r_0  }^{M^{(r')}}           & (\mbox{$n$ is her.\ just.\ by $r_0$ in $t^\star$}) \\
            &= \pview{t \filter^\star  r_0  }^{M^{(r')}}
    ...
    $t\filter^\star r_0$ for any occurrence $r_0$ of $r'$. By P-visibility, $n$'s justifiers all appear in $\pview{t'}$ therefore, $n$ does not belong to $t\filter^\star r_0$ for any occurrence $r_0$ of $r'$.
    ...
    occurrences in $w$ appear in $(t'\cdot m)\filter^\star r_0$
    and not in any $(t'\cdot m)\filter^\star r_1$ for any occurrence $r_1\neq r_0$ of $r'$.
    ...
    Moreover:
            ...
            &= \pstr[0.4cm]{ \pview{t' \cdot \nd(m){m} }^M \filter V^{(r')} \cdot \nd(lmd-m,30){n} }
                & (\mbox{$n \in V^{(r')}$}) \\
            &= \pstr[0.4cm]{ \pview{(t' \cdot \nd(m){m})\filter^\star r_0}^{M^{(r')}}  \cdot  \nd(lmd-m,30){n} }
                & \mbox{(induction hypothesis)} \\
            &= \pstr[0.4cm]{ \pview{t' \cdot \filter^\star r_0 \cdot \nd(m){m}}^{M^{(r')}}  \cdot  \nd(lmd-m,35){n} }
                & \mbox{($m$ appears in $t\filter^\star r_0$)} \\
            &= \pview{ \Pstr{t' \filter^\star r_0 \cdot (m){m} \cdot {(u \filter^\star r_0)} \cdot (lmd-m,35){n}}}^{M^{(r')}}
                & \parbox[t]{6cm}{(P-view in $M^{(r')}$: nodes and leaves in $(u \filter^\star r_0) \cdot n$ are all in $V^{(r')}$
                    by Lemma \ref{lem:instarfilter_imp_insubtree})} \\
            &= \pview{ (\Pstr{t' \cdot (m){m} \cdot u \cdot (lmd-m,35){n}}) \filter^\star r_0 }^{M^{(r')}}
                & (\mbox{$m$ and $n$ appear in $t\filter^\star r_0$}) \\
            &= \pview{ t \filter^\star r_0 }^{M^{(r')}}
                & \mbox{(definition of $t$).}
            \end{align*}
    ...
    $t\filter^\star r_0$ for any occurrence $r_0$ of $r'$. Hence since the only justifier of $n$ is $m$, $n$ must also not appear in $t\filter^\star r_0$ for any occurrence $r_0$ of $r'$
    ...
    \item If $t =  t' \cdot r_0$ where $r_0$ is an occurrence of $r'$, then $r_0 \in V^{(r')}$, $\pview{t}^M = \pview{t'}^M \cdot r_0$. Since $\pview{t}$ is the path from the root to $r'$, all the nodes occurring before $r_0$ in $\pview{t'}$ do not belong to $V^{(r')}$. Hence:
        ...
        \pview{t}^M \filter V^{(r')}
            ...
            &= \pview{t \filter^\star  r_0 }^{M^{(r')}} & \mbox{($t \filter^\star  r_0 = r_0$)}. \qed
        \end{align*}
    \end{compactitem}
%end of proof
...
\label{lem:oview_trav_filtering}
   Let $t \in \travset(M)$.
    ...
    \item[(c)]   Let $r_0$ be an occurrence in $t$ of a lambda-node $r' \in N_\lambda$.
    Suppose that $t^\omega\in V^{\theroot\vdash}$ and $t^\omega$ is not the occurrence $r_0$.

     If $t^\omega$ appears in $t \filter^\star r_0$ then all the nodes and leaves occurring after $r_0$ in $\oview{t}$ also appear in $t \filter^\star r_0$.

     If $t^\omega$ does not appear in $t \filter^\star r_0$ then all the nodes and leaves occurring after $r_0$ in $\oview{t}$ do not appear in $t \filter^\star r_0$.

    \item[(d)] Suppose $|t|>0$. Let $r_0$ be an occurrence in $t$ of a lambda node
    $r'$ in $N_\lambda$. Supposet $t^\omega \in V^{\theroot\vdash}$ and $t^\omega$ appears in $t \filter^\star r_0$.
     
     If $r_0$ appears in $\oview{t}$ then $\oview{t\filter^\star r_0}_{M^{(r')}} = \oview{t}_M\filter^\star r_0$.
     Otherwise $\oview{t\filter^\star r_0}_{M^{(r')}} = r_0 \cdot (\oview{t}_M\filter^\star r_0)$.
    \end{compactitem}
\end{lemma}
\proof
We show (a), (b), (c) simultaneously by induction on the last traversal rule used to form $t$.
The results trivially hold for the case \rulenamet{Empty} and \rulenamet{Root}.

\emph{Step case:}
Let us first consider the rules \rulenamet{Lam}, \rulenamet{App}, \rulenamet{Value$^{{\sf var}\mapsto\lambda}$} and
\rulenamet{Value$^{@\mapsto\lambda}$}. In these cases, $t$'s last node is not in $N_\lambda\union L_{\sf var} \union L_\Sigma \union L_@$ which does not correspond to the hypothesis of (a) therefore we do not need to show (a).
For (b) the result holds trivially: we have $\oview{t} = \jp(t) \cdot t^\omega$ where $t^\omega$ points in $\jp(t)$
so we conclude by using the induction hypothesis on $\jp(t)$.

For (c), suppose that $t^\omega\in V^{\theroot\vdash}$ and $t^\omega$ is not the occurrence $r_0$.
We just need to show that $t^\omega$ appears in $t \filter^\star r_0$ if and only if its main justifier
does; the induction hypothesis then permits us to conclude. We do a case analysis:
\begin{compactitem}
    \item[\rulenamet{Lam}] Take  $t = t' \cdot \lambda \overline{\xi} \cdot n$ where $n$ denotes $\lambda \overline{\xi}$'s child.
        ...
        Suppose that $n$ is a free variable node then its justifier is $r$, the first node of $t$. Thus
        $\oview{t} = r \cdot n$ and (c) holds trivially.
        Suppose that $n$ is a bound variable node. If $r_0$ occurs after $x$'s binder then (c) holds trivially.

        If $r_0$ occurs before $x$'s binder then
        \Pstr[0.6cm]{t = \ldots r_0 \ldots (l){\lambda \overline{x}} \cdot \ldots \cdot (n-l,40:0) n}
        where $\lambda \overline{x}$ is $n$'s binder.

        If $n$ does not appear in $t \filter^\star r_0$ then necessarily its binder does not either hence we can conclude using the induction hypothesis on $t_{\prefixof \lambda \overline{x}}$.

        If $n$ appears in $t \filter^\star r_0$ then by Lemma \ref{lem:pview_trav_filtering},
        $r_0$ necessarily occurs in the P-view $\pview{t}$ and all the nodes and leaves occurring after
        $r_0$ in $\pview{t}$ also appear in $t\filter^\star r_0$.
        Thus, since $n$'s justifiers are all in the P-view, they must all appear in $t\filter^\star r_0$.
        In particular its main justifier $\lambda \overline{x}$ must appear in $t \filter^\star r_0$.


    \item[\rulenamet{App}] Take \Pstr[0.4cm]{t \cdot (m) @  \cdot (n-m,40:0) n}. $n$ is justified by an @-node
    therefore it cannot be in $N^{\theroot\vdash}$. Hence this case does not happen.

    \item[\rulenamet{Value$^{@\mapsto\lambda}$}] Take \Pstr[0.5cm]{t = t' \cdot (lmd){\lambda
    \overline{\xi}} \cdot (x){@}  \ldots  (xv-x,50:v){v}_@  \cdot
    (lmdv-lmd,30:v){v}_{\lambda \overline{\xi}} }.

    Clearly since $v_{\lambda \overline{\xi}}$ is a value-leaf, it appears in $t \filter^\star r_0$ if and only its justifier does.
...
\end{compactitem}
\smallskip
We now consider the remaining rules.
For these rules, (b) is a direct consequence of (a).
For (c), it suffices to show that whenever $t^\omega \in V^{\theroot\vdash}$,
$t^\omega$ appears in $t \filter^\star r_0$ if and only if $t^{\omega^2}$ does.
Let us show (a) and (c):
\begin{compactitem}
\item[\rulenamet{InputValue}] Take \Pstr[0.4cm]{t =
    ...
    $x$ and $v_x$.
        ...
        (a) Since $v_x$ is justified by $x$ in $t$ it
        must also be the case in $t^\star$. Thus $x$ and $v_x$ are in the same thread.

        (c) Suppose that $v_x \in V^{\theroot\vdash}$.
        Since $x$ justifies $v_x$ we clearly have
        $v_x \in t \filter^\star r_0 \iff x \in t \filter^\star r_0$.
    ...
    \overline{\xi}}}}.
        ...
        (a) By the induction hypothesis  $x$ and $\lambda \overline{\xi}$ are in the same thread. Thus since $v_x$
            is justified by $x$ and $v_{\lambda \overline{\xi}}$ is
            justified by $\lambda  \overline{\xi}$, $v_x$ and $v_{\lambda \overline{\xi}}$ must
            be in the same thread.

        (c) Suppose that $v_x$ is in $V^{\theroot\vdash}$, then by (a) so is $v_{\lambda \overline{\xi}}$ and in turn so is its parent node $\lambda
        \overline{\xi}$. Thus by the induction hypothesis.
        $\lambda \overline{\xi} \in t \filter^\star r_0 \iff x \in t \filter^\star r_0$.
        Moreover since $x$ justifies $v_x$ and $\lambda
        \overline{\xi}$ justifies $v_{\lambda
        \overline{\xi}}$  we have   $x \in t \filter^\star r_0 \iff v_x \in t \filter^\star r_0$
        and $\lambda \overline{\xi} \in t \filter^\star r_0 \iff v_{\lambda
        \overline{\xi}} \in t \filter^\star r_0$.
    \end{compactitem}
    ...
    (and $x$ and $y$ are possibly the same occurrence).
        ...
        (a) $n$ is in the same thread as its justifier $y$ which
        by the induction hypothesis, is in the same thread as all the nodes
        in the O-view $\oview{t_{\prefixof \lambda
        \overline{\xi}}}$, and in particular $\lambda
        \overline{\xi}$. In turn, since $\lambda
        \overline{\xi}$ justifies $v_{\lambda
        \overline{\xi}}$, this is the same thread as the thread of $v_{\lambda
        \overline{\xi}}$.

        (c) Suppose that $n \in V^{\theroot\vdash}$.
        Since $n$ is not the occurrence $r_0$, we have that $n$ apperas in $t \filter^\star r_0$ if and only if its justifier $y$ does. But then by the induction hypothesis, since $y$ occurs in $\oview{t_{\prefixof \lambda \overline{\xi}}}$, it appears $t \filter^\star r_0$ if and only if $\lambda \overline{\xi}$ does. Which in turn happens if and only if
        $v_{\lambda \overline{\xi}}$ does.
    ...
    \Pstr[0.4cm]{t = \ldots (y){y} \ldots x \cdot (n){n} } where $y$ is a variable node occurring in $\oview{t_{\prefixof x}}$ (and $x$ and $y$ are possibly the same occurrence).
        ...
        (a) Again since $n$ points in the O-view $\oview{t_{\prefixof x}}$, it must belong to the same thread
        as all the nodes in $\oview{t_{\prefixof x}}$ and in particular to the thread of $x$.

        (c) Suppose that $n \in V^{\theroot\vdash}$.
          Again $n$ appears in $t \filter^\star r_0$ if and only if its justifier $y$ does. Moreover since $y$ occurs in $\oview{t_{\prefixof x}}$ by the induction hypothesis, $y$ appears in  $t \filter^\star r_0$ if and only if $x$ does.
    ...
    (lz-app,60){\lambda \overline{z}} \ldots
    (lzv-lz,60:v){v}_{\lambda \overline{z}} \cdot
    (appv-app,45:v){v}_@}. (a) Clearly the last two nodes are
    both in the same thread initiated by the @-node.

    (c) This case does not happen since $t$'s last node is in $V_@$
    ...
    (vy-y,50:v){v}_y } for some variable $y$ not in $N_{\sf var}^{\theroot\vdash}$.
        ...
        (a) Again, by the induction hypothesis we must
        have that $y$, $\lambda \overline{\xi}$, $v_{\lambda \overline{\xi}}$ and $v_y$
    are all in the same thread.

       (c) By (a), the last two nodes in $t$ do not belong to $N_{\sf var}^{\theroot\vdash}$.
    ...
    Take \Pstr[0.6cm]{ t = t' \cdot (p){p} \cdot (lx){\lambda
        ...
        (a) Again the induction hypothesis permits us to conclude.

        (c) By (a), the last two nodes in $t$ are both not in $N_{\sf var}^{\theroot\vdash}$.
            But by assumption $t$'s last node is in $V^{\theroot\vdash}$ therefore this rules cannot be used.

    \item[(Constant rules)] 
    (a) the proof is similar to the cases \rulenamet{InputVar} and
    \rulenamet{InputValue} (using the fact that constant
    rules are well-behaved).

    (c) the proof is similar to the cases \rulenamet{Var} and
    \rulenamet{Value$^{\lambda\mapsto@}$}.
    \end{compactitem}
    \bigskip

    We now show (d) by induction. The result is trivial for the empty traversal. Consider a traversal $t$ with $|t|>0$.
    We proceed by case analysis on the type of $t$'s last occurrence:
    \begin{itemize}
    \item If $t =  t' \cdot r_0$ then $t \filter^\star r_0 = r_0$ and  $\oview{t}_M \filter^\star r_0 = r_0 = \oview{t \filter^\star r_0}_{M^{(r')}}$.

    \item If $t = t' \cdot n$ with $n \in N_\lambda \union L_{\sf var} \union
    L_\Sigma \union L_@$ where $n$ is not the occurrence $r_0$.

    Since $n$ appears in $t\filter^\star r_0$, its only justifier (which is in $\oview{t'}$ by O-visibility) must also appear in $t\filter^\star r_0$.

    By (b), $t'$'s last occurrence is in $V^{(r')}$ and by (c), it appears in $t \filter^\star r_0$. Hence we can apply
    the I.H. on $t'$ which gives us $\oview{t}_M = \oview{t' \filter^\star  r_0}_{M^{(r')}}$.  Thus:
        \begin{align*}
        \oview{t}_M \filter^\star r_0
    &= \oview{t' \cdot n}_M \filter^\star r_0 & (\mbox{definition of } t)\\
            &= \oview{t'}_M \filter^\star r_0 \cdot n  & \parbox[t]{7cm}{(\raggedleft O-view in $M$, $n$'s justifier appears in $\oview{t'}\filter^\star r_0$ by the previous remark)} \\
            &= \oview{t' \filter^\star  r_0 }_{M^{(r')}} \cdot n            & (\mbox{induction hypothesis}) \\
            &= \oview{t' \filter^\star  r_0 \cdot n}_{M^{(r')}} & \parbox[t]{7cm}{\raggedleft (O-view in $M^{(r')}$:
             $n$ is indeed a node of $\tau(M^{(r')})$ by Lemma \ref{lem:instarfilter_imp_insubtree})} \\
            &= \oview{(t' \cdot n ) \filter^\star  r_0  }_{M^{(r')}}           & (\mbox{$n$ occurs in $t\filter^\star r_0$}) \\
            &= \oview{t \filter^\star  r_0  }_{M^{(r')}}
     & (\mbox{definition of } t).
        \end{align*}

    \item If $\Pstr{ t =  t' \cdot (m){m} \cdot u \cdot (lmd-m,30){n} }$
    where $n \in N_{\sf var} \union N_\Sigma \union N_@ \union L_\lambda$ then necessarily
    $m\in N_\lambda \union L_{\sf var} \union L_\Sigma \union L_@$.
    


By (b) the nodes in the O-view are all in the same thread as $n$ therefore $m$ is in $V^{(r')}$.

        \begin{itemize}
        \item Suppose that $r_0$ occurs before $m$ in $t$.

            By (c) all the nodes and leaves occurring after $r_0$ in the O-view appear in
            $t \filter^\star r_0$, in particular since $m$ is in the O-view $\oview{t}$ and $r_0$ occurs before $m$, $m$ must appear in $t \filter^\star r_0$ and we can apply the I.H. on $t' \cdot m$ which gives us $\oview{t' \cdot m}_M \filter^\star r_0 = \oview{(t' \cdot m)\filter^\star r_0 }_{M^{(r')}}$.
            Hence:
                \begin{align*}
                        \oview{t}_M \filter^\star r_0
                        &= \oview{\Pstr{t' \cdot (m){m} \cdot u \cdot (n-m,40){n}}}_M \filter^\star r_0
                                & (\mbox{definition of } t)\\
                        &= (\pstr{\oview{t' \cdot \nd(m){m}}_M  \cdot \nd(lmd-m,60){n}}) \filter^\star r_0
                                & (\mbox{O-view in $M$ }) \\
                        &= \pstr[0.4cm]{ \oview{t' \cdot \nd(m){m}}_M \filter^\star r_0  \cdot  \nd(lmd-m,30){n} }
                                & (\mbox{$n$ appears in $t \filter^\star r_0$}) \\
                        &= \pstr[0.4cm]{ \oview{(t' \cdot \nd(m){m} )\filter^\star r_0}_{M^{(r')}} \cdot \nd(lmd-m,25){n} }
                                & \mbox{(induction hypothesis)} \\
                        &= \pstr[0.4cm]{ \oview{t'\filter^\star r_0 \cdot \nd(m){m}}_{M^{(r')} } \cdot \nd(lmd-m,25){n} }
                                & \mbox{($m$ appears in $t \filter^\star r_0$)} \\
                        &= \oview{ \Pstr[0.4cm]{t' \filter^\star r_0 \cdot (m){m} \cdot {(u \filter^\star r_0)} \cdot (lmd-m,30){n}}}_{M^{(r')}}
                                & \parbox[t]{5cm}{\raggedleft(O-view in $M^{(r')}$, nodes in $m\cdot (u \filter^\star r_0) \cdot n$ are all in $V^{(r')}$)} \\
                        &= \oview{ (\Pstr{t' \cdot (m){m} \cdot u \cdot (lmd-m,35){n}}) \filter^\star r_0 }_{M^{(r')}}
                                & \mbox{($m$ and $n$ both appear in } t \filter^\star r_0) \\
                        &= \oview{ t \filter^\star r_0 }_{M^{(r')}}
                                & \mbox{(definition of $t$).}
            \end{align*}
...
\begin{proposition}[Traversal filtering]
    ...
    $r'\in N_\lambda$ we have $t\filter^\star r_0 \in \travset(M^{(r')})$.
\end{proposition}
\proof
    ...
    Suppose that $n$ does not appear in $t \filter^\star r_0$ then
    the result follows by applying the induction hypothesis on $t'$.
    Suppose that $n$ appears in $t \filter^\star r_0$.
    We do a case analysis on the last traversal rule used to form $t$:

  \begin{itemize}
    \item \rulenamet{Lam}
    We have $t' = \ldots \cdot \lambda \overline{\xi}$.
    By the induction hypothesis, $t' \filter^\star r_0 \in \travset(M^{(r')})$.

    Since $\lambda \overline{\xi}$ is in the P-view $\pview{t}$, by Lemma \ref{lem:pview_trav_filtering}
    it must occur in $t\filter^\star r_0$ and therefore
    it is the last occurrence in $t'\filter^\star r_0$.
    Thus we can use the rule \rulenamet{Lam} in $\tau(M^{(r')})$ to produce the
    traversal $u = (t'\filter^\star r_0) \cdot n$ of $M^{(r')}$.

    We have $t \filter^\star r_0 = (t'\filter^\star r_0) \cdot n$, but in order
     to state the equality $u = t \filter^\star r_0$ it remains to prove that $n$'s justifiers in $t \filter^\star r_0$ are the same as $n$'s justifiers in $u$.

     The justifiers of $n$ in $u$ are defined as follows:
      \begin{compactitem}
            \item If $n \in N_@ \union N_\Sigma$ then it has no justifier;
            \item If $n \in N_{\sf var}$ then it has auxiliary links pointing to each of the prime $\lambda$-nodes in $\pview{t'\filter^\star r_0}$. Furthermore,
                if  $n \not\in N_{\sf fv}$ then its main link points to the only occurrence of its enabler in $\pview{t'\filter^\star r_0}$ and if  $n \in N_{\sf fv}$ then its main link points to $r_0$.
        \end{compactitem}

     On the other hand in $t \filter^\star r_0$,
     $n$ points to all its justifiers in $\pview{t'}$ that also appear in $t'\filter^\star r_0$.

     By Lemma \ref{lem:pview_trav_filtering} we have that $\pview{t'\filter^\star r_0} = \pview{t}_{\suffixof r_0} = r_0 \cdot w$ where all the nodes in
     $w$ occur in $t \filter^\star r_0$. Hence $n$'s justifiers in $u$ correspond precisely to
     the justifiers of $n$ in $t \filter^\star r_0$ and therefore $u = t \filter^\star r_0$.

    \item \rulenamet{App} $t = \ldots \cdot \lambda \overline{\xi} \cdot @$.
    Again, since $\lambda \overline{\xi}$ is in the P-view $\pview{t}$, by Lemma \ref{lem:pview_trav_filtering}
    it must be the last occurrence in $t'\filter^\star r_0$.
    By the induction hypothesis, $t' \filter^\star r_0$ is a traversal of $\tau(M^{(r')})$
    therefore we can use the rule \rulenamet{App} in $\tau(M^{(r')})$ to produce the
    traversal $(t'\filter^\star r_0) \cdot n = t \filter^\star r_0$ of $M^{(r')}$.
    ...
    The occurrence $v_{\lambda \overline{\xi}}$ appears $t\filter^\star r_0$
    therefore since $r_0$ is not a lambda node, its justifier $\lambda \overline{\xi}$
    also appear in $t\filter^\star r_0$. Moreover since $@$ and $v_@$ are her.\ just.\ by
    $\lambda \overline{\xi}$, they must also appear in $t\filter^\star r_0$.

    By the induction hypothesis $t' \filter^\star r_0$ is a traversal of $\tau(M^{(r')})$
    therefore since the occurence $\lambda \overline{\xi}$, @, $v_@$, $v_{\lambda \overline{\xi}}$ all
    appear in $t\filter^\star r_0$ we can use the rule \rulenamet{Value$^{@\mapsto\lambda}$}
    in $M^{(r')}$ to form the traversal $(t'\filter^\star r_0) \cdot n = t \filter^\star r_0$ of $M^{(r')}$.
    ...
    (lzv-lz,60:v){v}_{\lambda \overline{z}} \cdot
    (appv-app,45:v){v}_@}.
    Again, since $v_@$ appears in $t \filter^\star r_0$, necessarily the occurrences @, $\lambda \overline{z}$, $v_{\lambda \overline{z}}$ and $v_@$ must all appear in $t \filter^\star r_0$. Hence using the induction hypothesis and the rule \rulenamet{Value$^{\lambda\mapsto@}$} in $M^{(r')}$ we obtain that $t \filter^\star r_0$ is a traversal of $M^{(r')}$.

    \item \rulenamet{Value$^{{\sf var}\mapsto\lambda}$} Take \Pstr[0.5cm]{t = t' \cdot (lmd){\lambda
    \overline{\xi}} \cdot (x){x}  \ldots  (xv-x,50:v){v}_x  \cdot
    (lmdv-lmd,30:v){v}_{\lambda \overline{\xi}} }.

    Again, since $v_{\lambda \overline{\xi}}$ is in $t \filter^\star r_0$, so must be
    $\lambda \overline{\xi}$, and therefore by applying Lemma \ref{lem:pview_trav_filtering} on $t_{\prefixof x}$, so must
    be $x$ and $v_x$. Hence we can use the induction hypothesis to
    form the traversal $t \filter^\star r_0$ of $M^{(r')}$.
...
%    The proof is identical to the previous case.
   ...
   \item \rulenamet{InputValue} Take \Pstr[0.4cm]{t =
    ...
    If $\lambda \overline{\eta_i}$ is the occurrence $r_0$ then the traversal $t\filter^\star r_0 = r_0$ cam be formed using the rule \rulenamet{Root}.
...
%    Otherwise, $p$ must be a variable node $y$ that does not belong to $N^{\theroot\vdash}$.
    ...
    Then both $\lambda \overline{\eta_i}$ and its justifier $p$ must appear in $t\filter^\star r_0$. However $\lambda \overline{x}$ and $x_i$ do not necessarily appear in $t\filter^\star r_0$.

    We do a case analysis on whether or not $x$ belongs to the sub-computation tree  $\tau(M^{(r')})$ or not:
    \begin{compactitem}
    \item If $x \in V^{(r')}$ then since $p$ appears in $t\filter^\star r_0$, by applying Lemma \ref{lem:pview_trav_filtering}
    on $t_{\prefixof \lambda \overline{x}}$ we have that $x$ and $\lambda \overline{x}$ also appear in $t\filter^\star r_0$. Hence we can use the I.H. together with the rule \rulenamet{Var} in $M^{(r')}$ to form the traversal $t \filter^\star r_0$ of $M^{(r')}$.
    ...
    \item Suppose that $x \not \in V^{(r')}$ then necessarily its binder node $\lambda \overline{x}$ is not in $V^{(r')}$ either. Therefore the two consecutive
        ...
        We can therefore make use of the rule \rulenamet{InputVar} in $M^{(r')}$ to produce the traversal $t \filter^\star r_0$ of $M^{(r')}$.
...
\end{corollary}
\begin{proof}
It follows immediately from the fact that, by the previous Lemma \ref{lem:oview_trav_filtering}(b), all the occurrence in $\oview{t}$ belong to the same thread and therefore are all hereditarily justified by $r$.
\end{proof}
...
%$t\filter r = t \filter^\star r$. We have $t^\omega \not\in L$.
%Applying the previous lemma with $r'=r$ gives $\pview{t
%\filter^\star r} = \pview{t} \filter N^{\theroot\vdash}$. Since there are
...
$t\filter^\star r'_0$ is a traversal of $\travset(M^{(r')})$ therefore
the sequence $(t\filter^\star r'_0) \filter r'_0 = t^\star \filter r'_0$ belongs
of $\travset^\star(M^{(r')})$ and therefore the expression $\varphi_{M^{(r')}}(t^\star \filter r'_0)$ is well-defined. By Definition \ref{dfn:phi_for_justsequ}, the sequences
$\varphi_M(t^\star)$ and $t^\star$ have the same justification pointers therefore
    ...
    \item Suppose $m\in T^0$. Then $m$ is hereditarily justified by some initial move $b$ in $B_j$ for some $j\in \{0..p\}$.
        ...
        \varphi_{N_j}(t_j-@) &= u \filter b \filter T^{00j}
         ...
         &= \varphi_{N_j}((t-@) \filter r_j )
            & \mbox{(By Lemma \ref{}).}
%         &= \varphi_{N_j}((t \filter r_j)-@)
%            & \mbox{(By def. of hereditary filtering on $\travset^{-@}(M)$).} \\
%         &= \varphi_{M}((t \filter r_j)-@)
%            & \mbox{($\varphi_M = \psi_M \union \Union_{k\in \{0..p\}} \varphi_{N_k}$ by definition).}
        \end{align*}
