...
hereditarily enabled by $\theroot$. Let us do a case analysis on
$t$'s last node:
\begin{itemize}
...
\item If $t =  t' \cdot \rnode{m}{m} \cdot  u \cdot \rnode{lmd}{n}
    ...
    Let us do a case analysis on the rule used to visit $n$ in
    $t$. Since the term is in $\beta$-normal form, there is no
    @-node in $\tau(M)$ so the rules \rulenamet{App} and
    \rulenamet{Var} are unused. Since $n\not\in N^{\theroot
    \vdash}_\lambda$, $n$ cannot be visited by the rule
    \rulenamet{InputVar}. Hence $n$ is necessarily visited with
    a \rulenamet{$\Sigma$} rule, but since $\Sigma$-constants
...
\item If $t =  t' \cdot \rnode{m}{m} \cdot u \cdot \rnode{lmd}{n}
    \link[nodesep=1pt]{30}{lmd}{m}$ with $n\in N^{\theroot \vdash}_\lambda$ then:
        ...
        &= ?(\pview{t'\filter r}  \cdot \rnode{m}{m} \cdot  \rnode{lmd}{n} )
               \link[nodesep=1pt]{60}{lmd}{m}                 & \mbox{($m$ and $n$ are unmatched)} \\
