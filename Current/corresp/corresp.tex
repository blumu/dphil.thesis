...
\end{enumerate}
    ...
    \begin{description}
        \item[$\syntrevsem{\Gamma \vdash M : T} \subseteq \varphi_{M}( \travset(M)^\star )$.] We show (constructively) by induction that there for any $v \in \Sigma \| ev$, there is some traversal $t$ such that
        the sequence $u = cover(v, \{0..l\}, \{0\})$ is equal to $\varphi_M(t^\star)$.
    ...
    By unraveling the definition of $v \in \Sigma \| ev$ we have
    \begin{eqnarray*}
    && \left\{
    \begin{array}{ll}
        v \in L_T \\
    ...
    We proceed by case analysis on $m$. If $m$ is a superficial internal move in $B$ then the result directly holds from the 
    It is either played in $A$, $B$ or $C$.
        ...
        \begin{itemize}
            ...
            \item Suppose $m$ is a superficial internal $B$-move then $cover(v,\{0..l\}, \{0\}) = cover(v', \{0..l\}, \{0\})$ so we can directly conclude from the I.H.

            \item $m$ is a profound internal moves in some $B_k$ for $k\in \{0..p\}$.

            \item $m = q_0''$ is an occurrence in $u$ of the initial $o''$-move. Then we necessarily have $v = q_0 \cdot q_0' \cdot q_0''$ where $q_0'$ is the initial move of $B_0'$, and thus $u = q_0 \cdot q_0''$. The traversal $t = \lambda^{[\theroot]} \cdot x_i$ formed using the rules \rulenamet{Root} and \rulenamet{Lam} meets the requirement.
