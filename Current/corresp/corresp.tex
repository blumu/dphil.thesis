...
\label{lem:subtraversal_progression} Let $M^{(r')}$ be a subterm of
$M$, $t \in \travset(M)$ and $r_0$ be an occurrence of $r'$ in $t$.
Let $t' = t \filter^+ r_0$ be the sub-traversal of $t$ (which
belongs to $\travset(M^{(r')})$ by Prop. \ref{prop:trav_filtering}.

If we can form a traversal $t' \cdot n$ of $\tau(M^{(r')})$ using a
rule different from \rulenamet{InputVar} and
$\rulename{InputVar^{val}}$ then the sequence $t \cdot n$, where the
justifier of $n$ in $t \cdot n$ is defined as the occurrence that
justifies $n$ in $t' \cdot n$, is a traversal in $\travset(M)$ .
\end{lemma}
\begin{proof}
...
This lemma says that extending a traversal locally also extends the
...
Let us do a case analysis on the rule used to visit the last occurrence of $t_j$:
    \begin{enumerate}[(a)]
...
%        where $B= \underbrace{((B_1' \times \ldots \times B_p') \rightarrow o')}_{B_0} \times B_1 \times \ldots \times B_p$
        ...
        thus since $z\not\in N^{\theroot\vdash}$, we can apply the (Var) rule to the second equation which gives us the following traversal of $\tau(M)$:
        \begin{eqnarray*}
            t' \cdot t_j^\omega = \Pstr[0.7cm]{ \ldots \cdot (n3){z} \cdot (n2){\lambda \overline{y}} \cdot \ldots \cdot (n1-n2,30:i){y} \cdot (n-n3,35:i){t_j^\omega} }
        \end{eqnarray*}
