...
\item Let $\Gamma(n) \vdash \kappa(n) : T(n)$ be the subterm of $\elnf{M}$ rooted at $n$. The function $\psi_{\kappa(n)}$ is defined as $\psi_{\kappa(n)}^{n,q}$ where $q$ denotes the initial move of
    ...
    \item We define $\mathcal{G}_M$ as the following disjoint union of games:
    $$\mathcal{G}_M = \sem{\Gamma \rightarrow T} \quad \uplus \quad  \biguplus_{n \in N \inter E\relimg{N_@ \union N_\Sigma} } \sem{T(n)}.$$

    \item The function $\varphi_M$ from $V_\lambda \union V_{var}$ to $\mathcal{G}_M$ is defined as:
        \begin{equation*}
                \varphi_M = \left\{
                              \begin{array}{ll}
...
Let us detail a bit more the definition of $\psi$:
\begin{itemize}
...
$\varphi_M$ \defname{preserves hereditary enabling}: a node $n$ is hereditarily
 enabled by some node $n'$ in the computation tree $\tau(M)$ if and only if
the move $\varphi_M(n)$ is hereditarily enabled by $\varphi_M(n')$ in the appropriate game from $\mathcal{G}_M$.
\end{remark}
...
\end{itemize}
\end{property}
We recall that in definition \ref{def:nodeorder}, the
...
V_{var})^* \rightarrow \mathcal{G}^*$ regarded as a function from
...
\mbox {for all occurrence $m$ in $s$ of an initial $\sem{type(T)}$-move, }\\
&& (s \filter m) \filter T' \in \Sigma' \\
...
\end{lemma}
\proof
It follows directly from the definition of the operation -@ and +@.
\qed
\smallskip
...
\begin{remark}
If $M$ is $\beta$-normal then $\tau$ does not contain any
...
Consequently:
$$(iii) \quad  \varphi_M(\travset^{-@}(M)) \filter \sem{\Gamma \rightarrow T} = \varphi_M(\travset^{\filter r}(M))\ .$$
\end{lemma}
\proof Let $t$ be a traversal of $M$:
$$\begin{array}{lrclr}
\mbox{i.}& \varphi( (t-@) \subtermfilter N ) &=& \varphi_M((t-@) \hefilter n ) & \mbox{(Def. subterm filtering)}\\
          &&=& \varphi_M(t-@) \filter \sem{\Delta \rightarrow A}  & \parbox[t]{5.5cm}{(By remark \ref{rem:phi_preserves_her_enabling}, $\varphi_M$ preserves hered. enabling,  and  moves in $\sem{\Delta \rightarrow T}$ are all hered. enabled by the initial move $m = \varphi_M(n)$).} \\
\\
...
\label{rem:corresp_proofreduction}
    ...
    \varphi_M(\travset(M)^{\upharpoonright r}) &=& \sem{\Gamma \vdash M :
    T}
    \end{eqnarray*}
...
Hence we can conclude using the induction hypothesis.
  ...
  \item (variable) $M \equiv x_i$. Since $M$ is in $\eta$-long normal form, $x$ must be of ground
    ...
    The diagram below gives the tree-representation of $\intersem{M}$ and fixes names for the different games involved in this interaction strategy. Node labels are of the form $\Pi : T' [A]$ where $\Pi$ is a strategy, $T'$ is the corresponding interaction game and $A$ is the standard game lying at the root of the interaction game $T$.
    $$
    \tree[levelsep=6ex,thistreesep=3cm]{\TR{\intersem{M}:T [!A\multimap C]}}
            {   \tree[levelsep=6ex]{\TR{\Sigma^\dagger:T^0[!A^0\multimap !B_0^0\otimes \ldots \otimes !B_p^0 ]}}
                ...
                \TR{ ev:T^1[!B_0^1 \otimes \ldots \otimes !B_p^1] }
            }
    $$
    where the games $A$, $B$ and $C$ are defined as follows:
    \begin{eqnarray*}
...
The tree-representation tells us all the information that we need about the strategy involved in $\intersem{M}$. For instance the revealed strategy $\Sigma$ is defined on the interaction arena
$\intersem{T^{00}}$ whose root is $!A^0 \multimap B^0$;
the strategy $ev$ is defined on the interaction arena $\intersem{T^1}$ with a single arena-node $!B^1 \multimap C^1$; thus plays of $ev$ do not contain uncovered moves.
...
\end{enumerate}
    ...
    $$
    is represented on the following tree:
    $$
    \tree[levelsep=6ex,thistreesep=3cm]{\TR{\intersem{M}:T [!A\multimap C]}}
            {   \tree[levelsep=6ex]{\TR{\Sigma^\dagger:T^0[!A^0\multimap !B_0^0\otimes \ldots \otimes !B_p^0 ]}}
                    {
