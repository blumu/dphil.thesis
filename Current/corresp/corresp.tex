...
Observe that since in a traversal extension $\overline{t}$, @/$\Sigma$-nodes can have pointers,
the traversal extensions are not proper justified sequences of nodes as
defined in Def.\ \ref{dfn:justseqnode}, therefore in order to manipulate such sequences we need to redefine the basic
transformations for them. Firstly, hereditary projection is defined identically as for justified sequences. 
To define the notions of O-view and P-view we make use of the ``long O-view'' of a sequence (see \cite{Harmer2005}):
...
In $\overline{t}$, the presence of the dummy node $\diamond$ changes
the parity of the alternation between nodes in $N_{\sf
var} \union L_\lambda \union N_@ \union N_\Sigma$ and $N_\lambda
\union L_{\sf var} \union L_\Sigma \union L_@$. For this reason, the 
notions of O-view and P-view are interchanged for traversal extensions: we
define the O-view of $\overline{t}$ to be the extension of $t$'s P-view 
and the P-view of $\overline{t}$ to be the extension of $t$'s  \emph{long O-view}:
\begin{equation}
...
We now establish an analogy between the traversal extension setting and the game
...
\end{align*}
By Lemma \ref{eqn:hat_subseq} the inclusion $\overline{\oview{t}_M\filterplus r_0}
\subseqof \overline{\longoview{t \filterplus r_0}}$ implies 
...
r_0}_{M^{(r')}}$ is a suffix of $\longoview{t \filterplus r_0}$, thus 
$\oview{t}_M\filterplus r_0 \subseqof w \cdot \oview{t \filterplus r_0}_{M^{(r')}}$ for some $w$ consisting of nodes preceding $r_0$ in $t$.
If $\oview{t}_M\filterplus r_0$ is empty then the relation $\oview{t}_M\filterplus r_0 \subseqof \oview{t \filterplus r_0}_{M^{(r')}}$ holds trivially. Otherwise, by definition of the operator $\_ \filterplus$, $\oview{t}_M\filterplus r_0$ necessarily starts with the occurrence
$r_0$, and since the nodes in $w$ occur before $r_0$ in $t$ this implies that
...
$\rput[t](0,0){\pstree{\TR{\lambda }}{\pstree{\TR{\varphi}}{\pstree{\TR{\lambda x}}{\pstree{\TR{@}}{\pstree{\TR{\lambda \psi}}{\pstree{\TR{\varphi}}{\pstree{\TR{\lambda x'}}{\pstree{\TR{@}}{\pstree{\TR{\lambda y}}{\pstree{\TR{\psi}}{\pstree{\TR{\lambda z}}{\TR{z}}}}\pstree{\TR{\lambda }}{\pstree{\TR{\varphi}}{\pstree{\TR{\lambda x''}}{\TR{x'}}}}}}}}\pstree{\TR{\lambda u}}{\pstree{\TR{u}}{\pstree{\TR{\lambda }}{\TR{e}}}}}}}}
}$ & 
\begin{asparablank}
\item 
$t = \Pstr[1.7cm]{(n0){\lambda }\ (n1-n0){\varphi}\ (n2-n1){\lambda x}\ (n3){@}\ (n4-n3){\lambda \psi}\ (n5-n0){\varphi}\ (n6-n5){\lambda x'}\ (n7){@}\ (n8-n7){\lambda y}\ (n9-n3){\psi}\ (n10-n3){\lambda u}\ (n11-n10){u}\ (n12-n9){\lambda z}\ (n13-n12){z}\ (n14-n10){\lambda }\ (n15-n0){e}\ }$

\item 
 $\overline{t} = \Pstr[1.7cm]{(d)\diamond (n0-d){\lambda }\ (n1-n0){\varphi}\ (n2-n1){\lambda x}\ (n3-n2){@}\ (n4-n3){\lambda \psi}\ (n5-n0){\varphi}\ (n6-n5){\lambda x'}\ (n7-n6){@}\ (n8-n7){\lambda y}\ (n9-n3){\psi}\ (n10-n3){\lambda u}\ (n11-n10){u}\ (n12-n9){\lambda z}\ (n13-n12){z}\ (n14-n10){\lambda }\ (n15-n0){e}\ }$
...
{$
    ...
    $$ \overbrace{X_1 \times \ldots \overbrace{((B_1'' \times \ldots \times B_p'') \typear o'')}^{X_i} \ldots \times X_n}^A \stackrel\Sigma\longrightarrow \overbrace{\psframebox[linestyle=dashed]{ \overbrace{((B_1' \times \ldots \times B_p') \typear o')}^{B_0} \times B_1 \times \ldots \times B_p }}^B \stackrel{ev}\longrightarrow \overbrace{o}^C$$
        ...
        \begin{enumerate}[1.]
            ...
            \item $m \in A$. Let $b$ denote the initial $B_k$-move that hereditarily justifies $m$ for some $k \in\{0..p\}$.
               ...
               \item  Suppose $m$ is an $\omove$-question.
                   ...
                   By P-visibility in $v\filter A,C$, $m$ occurs in $\pview{v' \filter A,C}$. But a property of the core (\cite{McCusker-GamesandFullAbstrac}) is that $\pview{v' \filter A,C} = \pview{v'} \filter A,C$ which in turn equals $\pview{\cover(v', \{0..p\}, \{0\})}\filter A,C$.
                   Hence
                    \begin{align*}
