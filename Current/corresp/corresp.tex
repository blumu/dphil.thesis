...
where:
    \begin{compactitem}
      \item $x$ is a variable node her.\ just.\ by some constant node ($n \in N^{N_\Sigma\vdash}_{\sf var}$);
      \item $P(t)$ is a predicate expressing some condition on $t$;
...
(thus $N^{(r')} = E^*(\{r'\})$).
%Let $\Gamma(r') \vdash M^{(r')} : T(r')$ be some subterm of $\elnf{M}$
\begin{proposition}[Traversal filtering]
    \label{prop:trav_filtering}
    Let $t \in \travset(M)$.
    \begin{itemize}
            \item[(T)] %Suppose $t^\omega$ is her.\ just.\ in $t-@$ by some
        ...
        \item[(*)]   $\pview{t\filter r} = \pview{t\filter^\star r} \filter N^{r\vdash}
$ ;
        ...
        \item[(**)] If $t= \ldots \cdot m \cdot n$ where
            ...
            \union L_\Sigma$ (in particular $n$ is not in
            $L_@$). Let $r_0$ be an occurrence in $t$ different
            from $n$ of a lambda-node $r' \in N_\lambda$. Then
            $n$ is her.\ just.\ by $r_0$ in $t^\star$ if and
            only if $m$ is.

            Moreover if they are her.\ just.\ by $r_0$ then they
            both belong to $N^{(r')}$.

% $m \in V^{r\vdash}$ if and only if  $n \in V^{r\vdash}$.
            %If $n$ is in $V'$ and is her.\ just.\ by $r_0$ in $t^\star$ then so is $m$;
            %If $m$ is in $V'$ and is her.\ just.\ by $r_0$ in $t^\star$ then so is $n$.

        \item[(O)] Suppose $|t|>0$. For all occurrence $r_0$ in $\oview{t}$ of a lambda node
$r'$ in $N_\lambda$ we have:
%            \begin{enumerate}[a.]
...
%            \item If $t^\omega \not\in V^{r\vdash}$ then there is an occurrence $r_0$ in $t$ of some lambda node $r'$ such that:
                \begin{itemize}
                \item  $\oview{t}_M \filter^\star r_0 = \oview{t\filter^\star
r_0}_{M^{(r')}}$,
...
%                \item and therefore all the occurrences in $\oview{t\filter^\star r_0}$ are in the same
%thread initiated by $r_0$.
                \end{itemize}
            \end{enumerate}
...
\begin{proof}
We prove (T), (P), (O) and (*) simultaneously by induction on the
...
verified for all traversal shorter than $t$.

\begin{itemize}
\item (T) We do a case analysis on the last traversal rule used to form $t$:

  \notetoself{
  \begin{itemize}
    \item \rulenamet{Lam}
    Suppose $t = w \cdot \lambda \overline{\xi} \cdot n \in
    \travset(M)$. By the induction hypothesis, for some
    traversal $w' \in \travset(M^{(r')})$ we have $w'-@ = ((w \cdot
    \lambda \overline{\xi}) -@)\filter r_0$. W.l.o.g. we can
    assume that $w'^\omega \neq @$.

    Since $\lambda \overline{\xi} = u^\omega$ is in $O$, $w'$'s
     last occurrence must be $\lambda \overline{\xi}$. Thus we can use
     the rule \rulenamet{Lam} in $\tau(M^{(r')})$ to produce the
     traversal $w' \cdot n$ of $M^{(r')}$ verifying
    $(w' \cdot n) - @  = ((w \cdot \lambda \overline{\xi} \cdot
    n) -@)\filter r_0$. Hence ($i$) holds.

    \item \rulenamet{App}
     Suppose $t = w \cdot \lambda \overline{\xi} \cdot @ \cdot n
     \in \travset(M)$.

     Since $n$ is her.\ just.\  by $r_0$ in $t-@$, so is
     $\lambda \overline{\xi}$ and we have $(t-@) \filter r_0 =
     (((w\cdot \lambda \overline{\xi})-@) \cdot n)\filter r_0 =
                                    ((w\cdot \lambda
    \overline{\xi})-@)\filter r_0 \cdot n$. By the induction
    hypothesis $((w\cdot \lambda \overline{\xi}) -@)\filter r_0
    = w'-@$ for some traversal $w'$ of $M$. Since $\lambda
    \overline{\xi}$ is her.\ just.\  by $r_0$ w.l.o.g. we can
    assume that the last move of $w'$ is $\lambda
    \overline{\xi}$. Thus we can use the rule \rulenamet{Lam}
    and \rulenamet{App} in $\tau(M^{(r')})$ to obtain the traversal
    $w' \cdot @ \cdot n$ of $M^{(r')}$ and we have: $(t-@) \filter r_0
    = (w'-@) \cdot n = (w' \cdot @ \cdot n) - @$.

    \item \rulenamet{Answer-@-$\lambda$}, \rulenamet{Answer-$\lambda$-@},
    \rulenamet{Answer-var-$\lambda$} and
    \rulenamet{Answer-$\lambda$-var}. These cases are all
    trivial application of the induction hypothesis.
...
By (T), the sequence $t \filter^\star  r_0$ is indeed a
traversal of $\tau(M^{(r')})$ therefore the expression $\pview{t
...
%\indent (**)
% By (P) we have $r_0 \in \pview{t_{\prefixof n}}$ therefore since
%the P-view of $t_{\prefixof n}$ gives the path from $n$ to the root,
%$n$ must be a descendant of $r'$ in the tree $\tau(M)$ thus $n\in
%N^{(r')}$.
\item[(**)]
Justification pointers attached to a node $n$ can only point to
 occurrences of nodes that lies in the path from $n$ to the root
 in the computation tree. Therefore if $m$ and $n$ are her.\
 just.\ by $r_0$ then both must be in the set of nodes
 $N^{(r_0)}$ of the computation tree rooted at $r_0$.

Let us write $n \in O$ as a shortcut for ``$n$ is her.\ just.\
by $r_0$ in $t^\star$''. We show by induction on the last
traversal rule used to form $t$ that $t^\omega \in O$ if and
only if $t^{\omega^2} \in O$. Since the last visited node is in
$N_\lambda\union L_{\sf var} \union L_\Sigma$, the only relevant
rules are \rulenamet{InputVar$^{val}$}, \rulenamet{InputVar},
% \rulenamet{Answer-@-$\lambda$},
\rulenamet{Answer-var-$\lambda$}, \rulenamet{Var} and the
constant rules:
    \begin{compactitem}
    ...
    If $t_2$ is empty then \Pstr[0.4cm]{t = t_1 \cdot (x){x}
    \cdot (xv-x,38:v){v_x} } so the last two nodes in $t$ are
    $x$ and $v_x$. Since $v_x$ is justified by $x$ in $t$ it
    must also be the case in $t^\star$. Hence $x \in O \iff v_x
    \in O$.

    If $t_2$ is not empty then by well-bracketing it is
    necessarily of the form \Pstr[0.4cm]{t_2 = (l){\lambda
    \overline{\xi}}  \ldots (x-l,38:v){v_{\lambda
    \overline{\xi}}}}. Suppose that $r_0 \neq \lambda
    \overline{\xi}$ then by the induction hypothesis we have $x
    \in O \iff \lambda \overline{\xi} \in O$. Thus since $v_x$
is justified by $x$ and $v_{\lambda \overline{\xi}}$ is
justified by $\lambda  \overline{\xi}$ we have $v_x\in O \iff
v_{\lambda \overline{\xi}} \in O$.

    Suppose that \Pstr[0.4cm]{t_2 = (l){r_0} \ldots
    (x-l,38:v){v_{\lambda \overline{\xi}}}}

    \item[\rulenamet{InputVar}] This case is identical to the previous
case.

    \item[Constant rules:] The proof is identical to the cases \rulenamet{InputVar} and
...
%    (lz-app,60){\lambda \overline{z}} \ldots
%    (lzv-lz,60:v){v}_{\lambda \overline{z}} \cdot
%    (appv-app,45:v){v}_@} then clearly the last two nodes are
%    both in the same thread initiated by the @-node.
    ...
    (vy-y,50:v){v}_y } then again by the induction hypothesis we
    can show that both $v_{\lambda \overline{\xi}}$ and $v_y$
    are not in the same thread.

    \item[\rulenamet{Var}] Take \Pstr[0.6cm]{ t = t' \cdot (n){n} \cdot (lx){\lambda
        \overline{x}} \ldots (x-lx,30:i){x_i}  \cdot
        (letai-n,40:i){\lambda \overline{\eta_i}} } for some
        variable $x_i$ not in $N_{\sf var}^{r\vdash}$. Again the
        induction hypothesis permits us to conclude.
    \end{compactitem}

\item (O) We do a case analysis on the type of $t$'s last occurrence:
    \begin{itemize}

    \item If $t = t' \cdot n$ with $n \in N_\lambda \union L_{\sf var} \union
    L_\Sigma \union L_@$.

    By (P), since $n\in N^{(r')}$, $n$ is her.\ just.\ by
    some occurrence $r_0$ of $r'$ in $t-@$. Thus by (*),
    $t'$'s last occurrence is also in $N^{(r')}$ and is
    her.\ just.\ by $r_0$ in $t-@$. Hence we can apply

    The induction hypothesis (O) on $t'$ which gives:
    \begin{itemize}
    \item all nodes in $\oview{t'}$ are her.\ just.\ by $r_0$ in $t$;
    \item $\oview{t}^M = \oview{t' \filter r_0}^{M^{(r')}} = \oview{t' \filter^\star  r_0}^{M^{(r')}}$.
    \end{itemize}

    Consequently, all the nodes in $\oview{t} = \oview{t'}
    \cdot n$ are her.\ just.\ by $r_0$ in $t^\star$ and:
        \begin{align*}
        \oview{t}^M \filter^\star r_0
    &= \oview{t' \cdot n}^M \filter^\star r_0 & (\mbox{definition of } t)\\
            &= \oview{t'}^M \cdot (n\filter^\star r_0)  & (\mbox{O-view computation}) \\
            &= \oview{t' \filter^\star  r_0 }^{M^{(r')}} \cdot (n\filter^\star r_0)            & (\mbox{induction hypothesis}) \\
            &= \oview{t' \filter^\star  r_0 \cdot (n\filter^\star r_0) }^{M^{(r')}} & \parbox[t]{7cm}{(O-view in $M^{(r')}$. $n$ is necessarily in $N^{(r')}$ if it is her.\ just.\ by $r_0$ in $t^\star$)} \\
            &= \oview{(t' \cdot n ) \filter^\star  r_0  }^{M^{(r')}}           & (\mbox{$n$ is her.\ just.\ by $r_0$ in $t-@$}) \\
            &= \oview{t \filter^\star  r_0  }^{M^{(r')}}
     & (\mbox{definition of } t).
        \end{align*}
    Hence $t$ verifies (O).

    \item If $\Pstr{ t =  t' \cdot (m){m} \cdot u \cdot (lmd-m,30){n} }$
    where $n \in N_{\sf var} \union N_\Sigma \union N_@
\union L_\lambda$  and $n$ is not an occurrence of $r'$ then
necessarily $m\in N_\lambda \union L_{\sf var} \union
L_\Sigma \union L_@$.


    If $n$ is a value-leaf then $m$ is $n$'s parent in $\tau(M)$.
    If $n$ is a bound variable node then $m$ is $n$'s binder in $\tau(M)$.
    If $n$ is a free variable node then $m$ is the root $r$ of $\tau(M)$.

    Since $m$ is the parent of the node $n$ in $N^{(r')}$ which is not the the root of $\tau(M^{(r')})$, necessarily $m$ also belongs to $N^{(r')}$. Therefore by the induction hypothesis, there is some occurrence $r_0$ of $r'$ in $t$ such that $\pview{r' \cdot m} = \ldots r_0 \cdot u$ and all the nodes in $u$
    ...
    Moreover:
            \begin{align*}
            \oview{t}^M \filter^\star r_0
            &= \oview{\Pstr{t' \cdot (m){m} \cdot u \cdot (n-m,40){n}}}^M \filter^\star r_0
                                                                  & (\mbox{definition of } t)\\
            &= (\Pstr{\oview{t'}^M \cdot (m){m} \cdot  (lmd-m,60){n}}) \filter^\star r_0
                                                                  & (\mbox{O-view computation}) \\
            &= \Pstr{ \oview{t'}^M \filter N^{(r')} \cdot (m){m} \cdot  (lmd-m,60){n} }
                                                                  & (\mbox{$m, n \in N^{(r')}$}) \\
            &= \Pstr{ \oview{t'\filter^\star r_0}^{M^{(r')}}  \cdot (m){m} \cdot  (lmd-m,60){n} }
                                                                  & \mbox{(induction hypothesis)} \\
            &= \oview{ \Pstr{t' \filter^\star r_0 \cdot (m){m} \cdot {(u \filter^\star r_0)} \cdot (lmd-m,35){n}}}^{M^{(r')}}
                                                               & \parbox[t]{6cm}{(O-view in $M^{(r')}$, nodes in $m\cdot (u \filter^\star r_0) \cdot n$ are all in $N^{(r')}$)} \\
            &= \oview{ (\Pstr{t' \cdot (m){m} \cdot u \cdot (lmd-m,35){n}}) \filter^\star r_0 }^{M^{(r')}}
                                                               & (m, n \in N^{(r')}) \\
            &= \oview{ t \filter^\star r_0 }^{M^{(r')}}                & \mbox{(definition of $t$).}
            \end{align*}

    \item If $t =  t' \cdot r_0$ where $r_0$ is an occurrence of $r'$.
    If $r'$ is the root $r$ then $t=r_0$ and the result trivially holds.
    If $r'$ is a prime node in $\tau(M)$ then $t'$'s last occurrence is necessarily an @-node thus:
    $\oview{t} = @ \cdot r_0$ and again the result holds trivially.
...
\notetoself{
 Suppose $t = \Pstr[12pt]{ t_1 \cdot (app){@} \cdot (a){\lambda \overline{\xi}}
        \cdot t_2 \cdot (va-a){v_{\lambda \overline{\xi}}} \cdot
          (vapp-app){v_@}}$ then $\oview{t} = @ \cdot \lambda
          \overline{\xi}
  \cdot v_{\lambda \overline{\xi}} \cdot v_@$ and $t-@ =
  \Pstr[12pt]{ {(t_1-@)} \cdot (a){\lambda \overline{\xi}}
        \cdot {(t_2-@)} \cdot (va-a){v_{\lambda \overline{\xi}}}
        }$.

  Let $m$ be a node in $t-@$. If $m$ occurs in $\oview{t}$ then
  either $m=v_{\lambda \overline{\xi}}$ or $m=\lambda
  \overline{\xi}$. These two moves are hereditarily justified by
  the same nodes in $t-@$ therefore ($i$) and ($ii$) hold.
}

\begin{corollary}
\label{cor:redtrav_trav} Suppose that  $M$ is in $\beta$-normal form
and does not contain any $\Sigma$-constant. Let $t$ be a non-empty
traversal of $M$ and $r$ denote the only occurrence of $\tau(M)$'s
root in $t$.
\begin{enumerate}[(i)]
\item If $t^\omega \in N$ ({\it i.e.}~$t$'s last occurrence is not a leaf) then
$ \pview{t \filter  r } = \pview{t \filter^\star  r } =
\pview{t}$ \ ;

\item %If $t^\omega \in V^{r\vdash}$ ({\it i.e.}~$t$'s last occurrence is
%hereditarily enabled by $\tau(M)$'s root) then
 $\oview{t \filter r } = \oview{t \filter^\star r} = \oview{t}$.
\end{enumerate}
\end{corollary}
\begin{proof}
Since $M$ is $\beta$-normal we have $t= t-@-\Sigma = t^\star$ and
$t\filter r = t \filter^\star r$. (i) Suppose $t^\omega \not\in L$.
...
\pview{t}$. (ii) All the nodes are her.\ just.\ by $r$ therefore
$t^\omega \in V^{r\vdash}$ and the previous lemma gives us
$\oview{t\filter^\star r} = \oview{t}$.
\end{proof}
...
definition of the rule \rulenamet{InputVar} we must have $n \in
