...
The problem with the previous result is that it does not give us an
equality. If we consider the subsequence consisting of unmatched
nodes, however, then we obtain an equality:
\begin{lemma}
\label{lem:betanf_orde1const_trav_pview_red} Suppose that $M$ is in
$\beta$-normal form and all the $\Sigma$-constants are well-behaved.
Let $t$ be a non-empty traversal of $M$ and $r$ denote the only
occurrence of $\tau(M)$'s root in $t$. If $t$'s last occurrence is
not a leaf then
$$ ?(\pview{t}) \filter r = ?(\pview{t \filter  r })\ .$$
\end{lemma}
\proof
By induction. It is trivial for the empty
traversal. Step case: consider a traversal $t$ and
suppose that the property (i) is verified for all traversal shorter
than $t$.
We first observe that since $t$ contains at most a single occurrence $r$ of the root $\theroot$, an occurrence $n$ in $t$ is hereditarily justified by $r$ if and only if the corresponding node in $\tau(M)$ is hereditarily enabled by $\theroot$.
Let us do a case analysis on $t$ last node:
\begin{itemize}
\item The case $t^\omega \in N_@$ does not happen since $M$ is $\beta$-normal.
...
\item If $t=r$ then the result holds trivially.
...
\item If $t =  t' \cdot \rnode{m}{m} \cdot  u \cdot \rnode{lmd}{n}
    \link[nodesep=1pt]{30}{lmd}{m}$ with $n\in N_\lambda \setminus N^{\theroot \vdash}_\lambda$ then necessarily $m \in N_@ \union N_{\sf var} \union N_\Sigma $.

    Let us do a case analysis on the rule used to visit $n$ in $t$.
    Since the term is in $\beta$-normal form,
    there is no @-node in $\tau(M)$ so the rules
    \rulenamet{App} and \rulenamet{Var} are unused. Since $n\not\in N^{\theroot \vdash}_\lambda$, $n$ cannot be visited by the rule
    \rulenamet{InputVar}. Hence $n$ has been visited with a \rulenamet{$\Sigma$} rule.
    Since $\Sigma$ constants are of order $1$ at most and since $n$ is not a leaf, $m$ is necessarily $n$'s parent and $n$ and $m$ are consecutive.     Hence $u=\epsilon$ and we have
        \begin{align*}
        \pview{t} \filter  r
        &= \pview{t' \cdot \rnode{m}{m} \cdot \rnode{n}{n}} \filter  r
...
\item If $t =  t' \cdot \rnode{m}{m} \cdot u \cdot \rnode{lmd}{n}
    \link[nodesep=1pt]{30}{lmd}{m}$ with $n\in N^{\theroot \vdash}_\lambda$ then:
        \begin{align*}
        \pview{t} \filter  r
...
r'_0$ belongs of $\travset(M^{(r')})^\star$ and therefore the
...
\notetoself{Review the use of lemma \ref{lem:he_filter_root_is_hj_filter_r}}
...
and \rulenamet{Lam} permit us to build the traversal $t = \lambda^{[r]} \cdot @ \cdot \lambda \overline{y}^{[r_0]}$ which clearly verifies $\varphi_M(t\star) = u$.
    \end{compactitem}
...
We do a case analysis on $m$: we either have $m\in C$ or $m\in T^0$:
    ...
    \item Suppose $m\in T^0$. Then $m$ is hereditarily justified by some initial move $b$ in $B_j$ for some $j\in \{0..p\}$.
        ...
        But by Corollary \ref{cor:varphi_bij}, $\varphi_{N_j}$ is a bijection from $\travset(N_j)^\star$ to
        $\varphi_{N_j}( \travset(N_j)^\star )$ therefore:
...
Suppose that $m \in T^{000}$ and $m^1 \in T^{001}$.
    ...
    move in $t'$.
        ...
        We have $\varphi(t'^\star) = u'$ by the induction hypothesis and $\varphi(n) = m$ by definition of $\varphi$.
        Therefore since $m$ and $n$ point to the same position we have $\varphi(t^\star) = u$.
