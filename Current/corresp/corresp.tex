...
\item
    ...
    \item The rule \rulenamet{InputValue} is just the counterpart of \rulenamet{InputVar}
    for value-leaves. The two rules are defined similarly but
    \rulenamet{InputValue} is more constrained: the visited node
    is necessarily justified by the pending node. This
    restriction ensures that the value-leaf visited by the rule
    respects the well-bracketing condition.

    \item For the rules \rulenamet{InputValue} and \rulenamet{InputVar}, since $x$ is the pending node, the traversal
    must be of the form
    \begin{quote}
    [\rulenamet{InputValue}] If $t_1 \cdot x \cdot t_2$ is a
traversal where $x \in N_{\sf var}^{\theroot\vdash}$ and either
$t_2 = \epsilon$ or $t_2 = \Pstr{(l){\lambda \overline{\eta}}
\ldots (v-l){v_{\lambda \overline{\eta}}} }$ then so is
\Pstr[0.5cm]{t_1 \cdot (x){x} \cdot t_2 \cdot (xv-x,38:v){v'_x}
