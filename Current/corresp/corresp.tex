...
\end{proof}
For the third case, we will see later on in Lemma \ref{lem:ifvar_in_tfilterstar_sodoes_certain_of_its_justifier} that in fact
all $y$'s auxiliary justifiers occurring after $j_i$ must also appear in $t\filter^\star r_0$.
...
\pstr[0.6cm]{t =
    ...
    If $y$ appears in $t\filter^\star r_0$ then all $y$'s justifiers occurring after $r_0$ in $t$ appear in $t\filter^\star r_0$ and those occurring before $r_0$ do not appear in $t\filter^\star r_0$.
\end{lemma}
\proof
By P-visibility all $y$'s justifiers appear in the P-view,
and by Lemma \ref{lem:pview_trav_filtering}, all the occurrences in the P-view preceding $r_0$ appear in $t\filter^\star r_0$
and all the occurrences in the P-view following $r_0$ do not appear in $t\filter^\star r_0$.
\qed
...
%
%     If $t^\omega$ appears in $t \filter^\star r_0$ then all the occurrences in $\oview{t}$ preceding $r_0$ in $t$ appear in $t \filter^\star r_0$.
%
%     If $t^\omega$ does not appear in $t \filter^\star r_0$ then all the occurrences in $\oview{t}$ following $r_0$ in $t$ do not appear in $t \filter^\star r_0$.
%
%    \item[(old d)]  Suppose that $t^\omega\in V^{\theroot\vdash}$. Suppose that $t^\omega$ appears in $t \filter^\star r_0$ for some occurrence $r_0$ in $t$ of a lambda node $r'$ in $N_\lambda$.
%
...
%
%    \item[(new d)]
     Suppose that $t^\omega$ appears in $t \filter^\star r_0$ for some occurrence $r_0$ in $t$ of a lambda node $r'$ in $N_\lambda$.

     Then $ \oview{t}_M\filter^\star r_0 \subseqof \oview{t\filter^\star r_0}_{M^{(r')}}$.
%    \end{itemize}
\end{lemma}
\proof
...
%$t \filter^\star r_0$ if and only if its only justifier does. Thus we can conclude using the I.H. on $\jp(t)$.
%\begin{itemize}
...
%        If $n$ does not appear in $t \filter^\star r_0$ then necessarily its binder does not either.
%        If $n$ appears in $t \filter^\star r_0$ then by P-visibility, $x$'s main justifier (its binder) must occur in the P-view, and since it occurs after $r_0$, by Lemma \ref{lem:pview_trav_filtering} it must appear in $t \filter^\star r_0$. Therefore we can conclude using the I.H. on $\jp(t)$.
%\end{itemize}
%\smallskip
%
%Let us consider the remaining rules: if $t^\omega = r_0$ then the result trivially holds. Otherwise, suppose that $t^\omega \in V^{\theroot\vdash}$. It suffices to show that $t^\omega$ appears in $t \filter^\star r_0$ if and only if $t^{\omega^2}$ does, the I.H. then permits us to conclude.
%\begin{itemize}
%\item[\rulenamet{InputValue}] Take \Pstr[0.4cm]{t =
%    t_1 \cdot (x){x} \cdot t_2 \cdot (xv-x,38:v){v_x} } for some
%    $v \in \mathcal{D}$.
...
%    (and $x$ and $y$ are possibly the same occurrence).
%
%        Since $n$ is not the occurrence $r_0$, it appears in $t \filter^\star r_0$ if and only if its only justifier $y$ does. But since $y$ occurs in $\oview{t_{\prefixof \lambda \overline{\xi}}}$ using the I.H. (d) on $t_{\prefixof \lambda \overline{\xi}}$ tells us that $y$ appears in $t \filter^\star r_0$ if and only if $\lambda \overline{\xi}$ does, which in turn happens if and only if        $v_{\lambda \overline{\xi}}$ does.
%
%
%    \item Suppose that $x$ is the last occurrence in $t'$, then
%    \Pstr[0.4cm]{t = \ldots (y){y} \ldots x \cdot (n){n} } where $y$ is a variable node occurring in $\oview{t_{\prefixof x}}$ (and $x$ and $y$ are possibly the same occurrence).
...
%    \begin{itemize}
%    \item $t =  t' \cdot r_0$. We have $t \filter^\star r_0 = r_0$ and  $\oview{t}_M \filter^\star r_0 = r_0 = \oview{t \filter^\star r_0}_{M^{(r')}}$.
%
%    \item $t = t' \cdot n$ with $n \in N_\lambda \union L_{\sf var} \union
%    L_\Sigma \union L_@$ where $n$ is not the occurrence $r_0$.
%
%    Since $n$ appears in $t\filter^\star r_0$, its only justifier (which is in $\oview{t'}$ by O-visibility) must also appear in $t\filter^\star r_0$.  By Lemma \ref{lem:trav_oview_single_threaded}, $t'^\omega$ is in $V^{(r')}$ and by (c), it appears in $t \filter^\star r_0$. Hence we can apply the I.H. on $t'$ which gives us  $\oview{t' \filter^\star  r_0}_{M^{(r')}} = \alpha \cdot \oview{t'}_M \filter^\star  r_0$
%    where $\alpha = r_0$ if $r_0$ occurs in $\oview{t}_M$
%    and $\alpha =\epsilon$ otherwise.
%    Thus:
%        \begin{align*}
%        \alpha \cdot \oview{t}_M \filter^\star r_0
%    &= \alpha \cdot \oview{t' \cdot n}_M \filter^\star r_0 & (\mbox{definition of } t)\\
%            &= \alpha \cdot \oview{t'}_M \filter^\star r_0 \cdot n  & \parbox[t]{7cm}{(\raggedleft O-view in $M$, $n$'s justifier appears in $\oview{t'}\filter^\star r_0$ by the previous remark)} \\
...
%            $t \filter^\star r_0$, in particular since $m$ is in the O-view $\oview{t}$ and $r_0$ occurs before $m$, $m$ must appear in $t \filter^\star r_0$ and we can apply the I.H. on $t' \cdot m$ which gives us $\oview{(t'\cdot m) \filter^\star  r_0}_{M^{(r')}} = \alpha \cdot \oview{t' \cdot m}_M \filter^\star  r_0$   where $\alpha = r_0$ if $r_0$ occurs in $\oview{t}_M$ and $\alpha =\epsilon$ otherwise.
%            Hence:
%            \begin{align*}
%            \alpha\cdot\oview{t}_M \filter^\star r_0
%            &= \alpha\cdot\oview{\Pstr{t' \cdot (m){m} \cdot u \cdot (n-m,40){n}}}_M \filter^\star r_0
%                    & (\mbox{definition of } t)\\
...
%        Let $j_1$, \ldots $j_k$ be the auxiliary justifiers of $m$. By Lemma \ref{lem:ifin_tfilterstar_so_does_justifier} and \ref{lem:pview_trav_filtering}, for some $i \in \{1..k\}$, $j_i = r_0$ and for all $i'\geq i$, $j_{i'}$ appears in $t\filter^\star r_0$.
%
%        Thus $j_i = r_0$ is the main justifier of $n$ in $t\filter^\star r_0$ and therefore $\oview{t\filter^\star r_0} = j_i \cdot n = r_0  \cdot n$.
%        We then have $r_0 \cdot (\oview{t}\filter^\star r_0) = r_0 \cdot (\oview{t'} \cdot m \cdot n) \filter^\star r_0 =  r_0 \cdot n = \oview{t\filter^\star r_0}$.
%        \end{itemize}
%    \end{itemize}
%
...
%    L_\Sigma \union L_@$ where $n$ is not the occurrence $r_0$.
%
%    By the I.H. on $t'$, for some occurrence $r_0$ of $r'$ we have $\oview{t'}_M \filter^\star  r_0 \subseqof \oview{t' \filter^\star  r_0}_{M^{(r')}}$.
%
%    If $n$ does not appear in $t\filter^\star r_0$ then
%    $\oview{t}_M \filter^\star r_0 = \oview{t'}_M \filter^\star r_0 \subseqof \oview{t' \filter^\star  r_0}_{M^{(r')}} = \oview{t \filter^\star  r_0}_{M^{(r')}}$.
%    If $n$ appears in $t\filter^\star r_0$ then:
%        \begin{align*}
...
%            &\subseqof  \oview{t' \filter^\star  r_0 }_{M^{(r')}} \cdot n            & (\mbox{induction hypothesis}) \\
%            &=  \oview{t' \filter^\star  r_0 \cdot n}_{M^{(r')}} & \parbox[t]{7cm}{\raggedleft (O-view in $M^{(r')}$:
%             $n$ is indeed a node of $\tau(M^{(r')})$ by Lemma \ref{lem:instarfilter_imp_insubtree})} \\
%            &=  \oview{(t' \cdot n ) \filter^\star  r_0  }_{M^{(r')}}           & (\mbox{$n$ occurs in $t\filter^\star r_0$}) \\
%            &= \oview{t \filter^\star  r_0  }_{M^{(r')}}
...
%    $m\in N_\lambda \union L_{\sf var} \union L_\Sigma \union L_@$.
%
%    By the I.H. on $t' \cdot m$, $\oview{t' \cdot m}_M \filter^\star r_0 \subseqof \oview{(t'\cdot m) \filter^\star  r_0}_{M^{(r')}}$.
%
%        \begin{itemize}
%        \item Suppose that $r_0$ occurs before $m$ in $t$.
%
%        If $n$ appears in $t\filter^\star r_0$ then necessarily by Lemma \ref{lem:ifvar_in_tfilterstar_sodoes_certain_of_its_justifier}, $m$ also appears in $t\filter^\star r_0$.
%            Hence:
%            \begin{align*}
%            \oview{t}_M \filter^\star r_0
...
%            &= (\pstr{\oview{t' \cdot \nd(m){m}}_M  \cdot \nd(lmd-m,60){n}}) \filter^\star r_0
%                    & (\mbox{O-view in $M$ }) \\
%            &= \oview{t' \cdot m}_M \filter^\star r_0
%                    & (\mbox{$n \not\in t \filter^\star r_0$}) \\
%            &\subseqof \oview{(t' \cdot m )\filter^\star r_0}_{M^{(r')}}
%                    & \mbox{(induction hypothesis)} \\
%            &= \oview{t'\filter^\star r_0}_{M^{(r')} }
%                    & \mbox{($m$ does not appear in $t \filter^\star r_0$)} \\
%          \end{align*}
%
%          We need to show that
%          $\oview{t'\filter^\star r_0}_{M^{(r')} } \subseqof \oview{ \Pstr[0.4cm]{t' \filter^\star r_0 \cdot (m){m} \cdot {(u \filter^\star r_0)} \cdot (lmd-m,30){n}}}_{M^{(r')}}$
%%            &= \oview{ \Pstr[0.4cm]{t' \filter^\star r_0 \cdot (m){m} \cdot {(u \filter^\star r_0)} \cdot (lmd-m,30){n}}}_{M^{(r')}}
%%                    & \parbox[t]{5cm}{\raggedleft(O-view in $M^{(r')}$, nodes in $m\cdot (u \filter^\star r_0) \cdot n$ are all in $V^{(r')}$)}\\
%%            &= \oview{ (\Pstr{t' \cdot (m){m} \cdot u \cdot (lmd-m,35){n}}) \filter^\star r_0 }_{M^{(r')}}
%%                    & \mbox{($m$ and $n$ both appear in } t \filter^\star r_0) \\
...
%        Let $j_1$, \ldots $j_k$ be the auxiliary justifiers of $m$. By Lemma \ref{lem:ifin_tfilterstar_so_does_justifier} and \ref{lem:pview_trav_filtering}, for some $i \in \{1..k\}$, $j_i = r_0$ and for all $i'\geq i$, $j_{i'}$ appears in $t\filter^\star r_0$.
%
%        Thus $j_i = r_0$ is the main justifier of $n$ \emph{in the sequence $t\filter^\star r_0$} and therefore $\oview{t\filter^\star r_0} = j_i \cdot n = r_0  \cdot n$.
%        We then have $(\oview{t}\filter^\star r_0) = (\oview{t'} \cdot m \cdot n) \filter^\star r_0 =  n \subseqof r_0 \cdot n = \oview{t\filter^\star r_0}$.
%        \end{itemize}
%
%\end{itemize}
\qed
...
\end{proposition}
The proof of this proposition can be found in the appendix of \cite{hylandong_pcf}. 
Since it is non-trivial, instead of reproving the result in our setting, we will just show how the proof of the previous proposition 
can be adapted to our setting to give the result of Proposition \ref{lem:oview_trav_filtering}.

The argument used in \cite{hylandong_pcf} relies on several properties on the legal position $s$:
\begin{itemize}
  ...
  \item[(w5)]  Visibility: $s$ satisfies P-visibility and O-visibility;
  \item Finally, positions of the game $A\rightarrow B$ satisfy the following switching condition: the Proponent is the only player
      who can switch from game $A$ to $B$ or from $B$ to $A$;
\end{itemize}
Observe that most of these requirement coincide with properties that we have shown for our notion of traversals. 
Unfortunately traversals do not strictly satisfy explicit justification: some nodes in the traversal do not have justification pointers (@-nodes and $\Sigma$-nodes). Also, variable nodes can have more than one pointer. 
This can however be remedied easily:
 
 fully match all of these requirements. mismatch with the
...
%    The proof is identical to the previous case.
   ...
   \item \rulenamet{InputValue} Take \Pstr[0.4cm]{t =
    ...
    If $\lambda \overline{\eta_i}$ is the occurrence $r_0$ then the traversal $t\filter^\star r_0 = r_0$ can be formed using the rule \rulenamet{Root}.
