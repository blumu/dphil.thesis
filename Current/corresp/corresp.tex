...
node). Further, for every variable node $x$ we add auxiliary pointers going to each of the lambda nodes occurring in $\pview{t_{\prefixof x}}$. These additional justifiers are called the \defname{auxiliary justifiers} of $x$ while its original justifier is called its \defname{main justifier}.
...
\begin{compactitem}
  ...
  \item Suppose that \pstr[0.6cm]{t =
    ...
    } where $y\in N_{\sf var}$
    and $j_1$, \ldots $j_k$ are the lambda nodes occurring in the $\pview{t}$. (In particular, $j_0$ is $y$'s justifier.)
...
\begin{proof}
  ...
  $t\filterplus r_0$ if and only if one of its justifiers in $t^+$ appears in $t\filterplus r_0$.
\end{proof}
For the third case, we will see later on (Lemma \ref{lem:pview_trav_filtering}) that
in fact all the $j_{i'}$ for $i' \geq i$ also appear in $t\filterplus r_0$.
\bigskip
...
\begin{lemma}
\label{lem:infilterplusr0_imp_infilterplusr1}
Let $t$ be a traversal and $r'$ be a lambda node.
If $t^\omega$ appears in $t\filterplus r_0$ for some occurrence $r_0$ of $r'$ in $t$ then
$t^\omega$ does not appear in $t\filterplus r_0$ for any other occurrence $r_1 \neq r_0$ of $r'$ in $t$.
\end{lemma}
\proof An easy induction on $t$.\qed
...
%        \begin{compactitem}
%        \item[(a)]
        If $t^\omega$ belongs to $V^{(r')}$ for some lambda node $r'\in N_\lambda$
...
\begin{lemma}
\label{lem:if_binder_before_r0_in_tfilterplus}
Let \pstr[0.6cm]{t = \ldots r_0 \ldots \nd(l){m} \ldots \nd(n-l,30){n} } and $r_0$ is an occurrence of a lambda node $r'$ in $t$.
Then $n$ appears in $t\filterplus r_0$ if and only if $m$ does.
\end{lemma}
\proof
...
If $x$ appears in $t\filterplus r_0$ then by Lemma \ref{lem:instarfilter_imp_insubtree}, $x$ is in
$V^{(r')}$. Therefore by Lemma \ref{lem:pview_trav_filtering}, $\pview{t} = \ldots r_1 \cdot u$ for some
occurrence $r_1$ of $r'$ and all the nodes in $u$ appear in $t\filterplus r_1$.
By P-visibility, $\lambda \overline{x}$ occurs in $\pview{t}$ and by Lemma \ref{lem:infilterplusr0_imp_infilterplusr1} $r_0 = r_1$ therefore
$\lambda \overline{x}$ occurs in $u$ and hence it appears in $t\filterplus r_1$. \qed
...
Suppose that the thread of $t^\omega$ is initiated by an @-node or $\Sigma$-node $\alpha$
and that $t^\omega$ appears in $t\filterplus r_0$.

(i) If $\alpha$ precedes $r_0$ in $t$ then all the nodes of the thread appearing in $t\filterplus r_0$ (and therefore occurring after $r_0$) are hereditarily justified by $r_0$.

(ii) If $r_0$ precedes $\alpha$ in $t$ then all the nodes occurring in the thread appear in $t\filterplus r_0$.
\end{lemma}
\proof
(i) The proof is by an easy induction. The only non-trivial case is for a traversal of the form
$$\ldots \Pstr[0.5cm]{(l){\lambda \overline{x} \cdots r_0 \cdots (x-lx){x}}}$$
where $x$ appears in $t\filterplus r_0$. But then by Lemma
\ref{}

(ii) By definition of a thread, the nodes occurring in the thread
are all hereditarily justified by $\alpha$.
Since $r_0$ precedes $\alpha$ and $t^\omega$ appears in $t\filterplus r_0$, by Lemma \ref{lem:if_binder_before_r0_in_tfilterplus} all the nodes in the thread must also appear in $t \filterplus r_0$. \qed
...
$$\begin{array}{rcll}
 ...
 \longoview{\Pstr[15pt]{s \cdot (m){m} \cdot \ldots \cdot (x-m,30){n}}} &=&
    ...
    & \parbox[t]{10cm}{for $n \in N_{\sf var} \union L_\lambda \union N_@ \union N_\Sigma$.}
\end{array}$$
\end{definition}
...
directly reused in our setting. A weaker version of condition (w8) holds however: we know by Lemma \ref{lem:if_binder_before_r0_in_tfilterplus} that
if $r_0$ occurs before $n$'s justifier then $n$ appears in
...
we do a case analysis on the type of $t^\omega$:
    ...
    $m\in N_\lambda \union L_{\sf var} \union L_\Sigma \union L_@$.
     ...
     Suppose that $r_0$ appears in $t' \cdot m$, then since $n$ appears in $t\filterplus r_0$, by Lemma \ref{lem:if_binder_before_r0_in_tfilterplus} so does $m$. Thus we can apply the I.H. on $t' \cdot m$:
     \begin{align*}
        \pview{\overline{t}} \filterplus r_0
...
\proof
  ...
  \begin{itemize}
    \item \rulenamet{Lam}
        ...
        If $r_0$ occurs before $m_2$ then by Lemma \ref{lem:if_binder_before_r0_in_tfilterplus}, $m_2$ appears in $t \filterplus r_0$ thus
...
%    The proof is identical to the previous case.
    ...
    Consider the node @ that hereditarily enables $y$ in the tree $\tau(M)$.
    ...
    the rule \rulenamet{Root}.  Suppose that $\lambda \overline{\eta_i}$ is not the occurrence $r_0$.

    Both $\lambda \overline{\eta_i}$ and its justifier $p$
    appear in $t\filterplus r_0$. The nodes $\lambda
    \overline{x}$ and $x_i$, however, do not necessarily appear in    $t\filterplus r_0$.

    Consider the node @ that initiate the thread of $\lambda \overline{\eta_i}$.

    Suppose that $r_0$ precedes @ in $t$ then
    by Lemma \ref{lem:thread_filterplus}(ii), the nodes $\lambda \overline{\eta_i}$, $p$, $\lambda \overline{x}$ and $x_i$ as well as @ all appear in $t\filterplus r_0$. Moreover since @ appear in $t\filterplus r_0$, it must be an occurrence of an application node that appear in the subtree rooted at $r'$ thus $@ \in N_{\sf var}^{r'\vdash}$.  Hence we can use the use the rule \rulenamet{Var} in $M^{(r')}$ to form the traversal $t \filterplus r_0$ of $M^{(r')}$.
...
%    Otherwise, $p$ must be a variable node $y$ that does not belong to $N^{\theroot\vdash}$.
    ...
    Suppose that @ precedes $r_0$ in $t$  then
...
Figure \ref{fig:interaction_strategy_denotations} contains tree representations of the interaction games of the revealed strategy $\intersem{\Gamma \vdash M : A}$ for the application cases. These trees tell us all the information that we need about the strategy involved in $\intersem{M}$. For instance the revealed strategy $\Sigma$ is defined on the interaction game $\intersem{T^{00}}$ whose root is $!A^0 \multimap B^0$; the strategy $ev$ is defined on the interaction game $\intersem{T^1}$ with a single game-node $!B_0^1 \otimes \ldots \otimes !B_p^1 \multimap o$; thus plays of $ev$ do not contain any uncovered move.
...
to denote the distance between $n$ and its justifier in $t$. If
...
We do a case analysis on $m$: we either have $m\in C$ or $m\in T^0$:
    ...
    \item Suppose $m\in T^0$. Then $m$ is hereditarily justified by some initial move $b$ in $B_j$ for some $j\in \{0..p\}$.
        ...
        But by Corollary \ref{cor:varphi_bij}, $\varphi_{N_j}$ is a bijection from $\travset(N_j)^{-@}$ to
        $\varphi_{N_j}( \travset(N_j)^{-@})$ therefore:
         $$t_j^\star = t^\star \filter r_j $$

        By Eq. \ref{eqn:starfilter_filterplusstar} from Section \ref{sec:tstar} we
        have $t^\star \filter r_j = (t\filterplus
        r_j)^\star$. Hence since $t_j^\omega$ and
        $t^\omega$ are not in $N_@$ we have
