...
\ht0 1.5cm\box0 % Make sure the height of box containing the traversal remains constant across the different generated slides
...
\begin{tikzpicture}[level distance=7mm,inner ysep=0.5mm]
 \node {\highlightat{3}{$\lambda f z$}}
    child {
        node {\highlightat{4}{@}}
        child {
            node {\highlightat{5}{$\lambda u v$}}
            child{
                node{\highlightat{6}{$u$}}
                child {
                    node{\highlightat{7}{$\lambda$}}
                    child {
                        node{\highlightat{8}{$v$}}
                    }
                }
            }
        }
...
\vspace*{0.4cm}
\visible<15->{\highlight{Traversal reduction} keeps
occ.\ hereditarily justified by the root:
...
$\Pstr[0.7cm]{ t - @ = (n0){\lambda f z}\ (n1-n0){\lambda u v}\ (n2-n1){u}\ (n3-n0){\lambda y}\ (n4-n0){h}\ (n5-n4){\lambda }\ (n6-n3){y}\ (n7-n2){\lambda }\ (n8-n1){v}\ (n9-n0){\lambda }\ (n10-n0){z}}$
}
}
