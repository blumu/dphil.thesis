...
the elements of $N_{\sf var}^{\upharpoonright r}$.
...
$N_{\sf var}$ for the set of nodes, @-labelled nodes, $\lambda$-nodes and variable nodes respectively. For $n \in N$, $\kappa(n)$ denotes the subterm of $\elnf{M}$ corresponding to the subtree rooted at $n$, in particular $\kappa(r) = \elnf{M}$.  The \defname{type} of a variable node labelled $x$ is the type of $x$, the type of the root is $(A_1,\ldots,A_p, T)$ where $x_1:A_1,\ldots, x_p:A_p$ are the free variables of $M$ and the type of $n\in (N_\lambda \union N_@)
...
We say that a node $n$ is \defname{answered} by $v_n$ if there is an
occurrence of $v_n$ in the sequence that points to $n$, otherwise we
say that $n$ is \defname{unanswered}. The last unanswered node is
called the \defname{pending node}.  A justified sequence of nodes is
...
with $x \in N_{\sf var}^{\upharpoonright r}$ and $?(t_1 \cdot x \cdot
...
\item[{\bf (InputVar)}] If $t_1 \cdot x \cdot t_2$ is a traversal with
  $x \in N_{\sf var}^{\upharpoonright r}$ and $?(t_1 \cdot x \cdot
...
for some variable $y$ not in $N_{\sf var}^{\upharpoonright r}$ then so
...
If \Pstr[0.5cm]{t' \cdot (n){n} \cdot
    ...
    (x-lx,50:i){x_i} } is a traversal for some variable $x_i$ not in $N_{\sf var}^{\upharpoonright r}$ then
so is
\Pstr[0.6cm]{
t' \cdot (n){n} \cdot
    (lx){\lambda \overline{x}}  \ldots
...
\item[(iii)] If $t$'s last node is not a value-leaf, then $\pview{t}$ is the path in the computation tree going from the root to $t$'s last node.
\end{itemize}
\end{proposition}
...
\notetoself{ Update the following according to the change in the
chapter on local comput of beta red.
\begin{lemma}[View of a traversal reduction]
\label{lem:redtrav_trav} If $M$ is in $\beta$-normal form then for
all $t\in \travset(M)$ we have $ ?(\pview{t \upharpoonright  r }) =
...
\travset(M)^{\upharpoonright r }$. This remains valid in the
...
\psi^{n,q}_M &=& \{ n \mapsto q \} \union  \{ v_n \mapsto v_q \ | \ v \in \mathcal{D} \}\\
 &&\union \left\{
                \begin{array}{ll}
                  ...
                  \Union_{i=1..p} \psi_M^{n.i, q^i}, & \hbox{if $p\geq1$ and $n\in N_{\sf var}$\ .}
                \end{array}
              \right.
\end{eqnarray*}
...
$V_\lambda \union V_{\sf var}$ to $\sem{T} \uplus \biguplus_{n \in N'_@}
...
of nodes in $V_\lambda \union V_{\sf var}$ then $\varphi(t)$ is defined
...
$\revsem{\Gamma \vdash M : T}$, is obtained by uncovering\footnote{An
...
\begin{eqnarray*}
(i)~\varphi_M  &: \travset(M)^{-@} \stackrel{\cong}{\longrightarrow} \revsem{\Gamma \vdash M :T} \\
     ...
     {  \tree{@}
        {   \tree{\lambda g x}{
            ...
            \tree{\lambda ^{[3]}}{
                ...
                \pstree{\TR[name=lmd2]{\lambda^{[5]}}}{\TR[name=r]{z}}
                }
            }
        }
     }
\hspace{1cm}
  \pssetarena\pstree[levelsep=8ex,treesep=0.3cm]{ \TR[name=q0]{q^0} }
