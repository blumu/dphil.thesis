...
We recall the definition of finite type theory. Let us define $\mathcal{D}_0 =
\{\mathbf{true},\mathbf{false}\}$ and $\mathcal{D}_{k+1}
=powerset(\mathcal{D}_k)$. For any $k\geq0$, we write $x^k$, $y^k$
and $z^k$ to denote variables ranging over $\mathcal{D}_k$. Prime
...
deciding the truth of such formulae requires nonelementary time
\cite{Meyer1974}.
\smallskip
