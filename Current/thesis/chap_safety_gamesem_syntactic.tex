...
\sem{\Gamma \rightarrow T} = \varphi(\travset(M)^{\filter \theroot})
$.
 Consequently,
...
to $\varphi(\travset(M)^*)$;
\item[(ii)] $\varphi$ defines a bijection from $\travset(M)^{\filter \theroot}$ to
$\varphi(\travset(M)^{\filter \theroot})$.
\end{itemize}
\end{corollary}
...
defines a bijection from $\travset(M)^{\filter \theroot}$ to
$\sem{M}$:
