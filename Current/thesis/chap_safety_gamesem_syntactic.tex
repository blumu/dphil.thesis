...
\begin{definition}
We call \defname{almost-safe application} a term of the form
...
We call \defname{almost-safe abstraction} a term that can be written under the form $\lambda \overline{x} . N$ for some safe term $N$. They are denoted by judgments
...
We say that a term is \defname{almost-safe}, written ``$\Gamma \asentail s : A$'', if it is safe or if it is an almost-safe application or an almost-safe abstraction:
$$
...
\rulenamet{app} and \rulenamet{abs} rules we can build the equivalent safe term $\Gamma \sentail \lambda \varphi_1 \ldots \varphi_n . N_0 N_1 \ldots N_k \varphi_1 \ldots \varphi_n :T$.
...
\end{lemma}
\begin{lemma}
\label{lem:almostsafe_iif_etalnf_almostsafe}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\notetoself{the following section needs to be integrated into the previous chapter.}
...
\section{Game-semantic analysis of safe IA}
