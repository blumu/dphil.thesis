...
\begin{enumerate}[(i)]
\item If $M$ is $\beta$-normal and all the \emph{reachable} input-variable nodes of the computation tree
...
$N^{\theroot\vdash}$-incrementally-bound.

%then so is $\tau(\etalnf{\closure{M}})$. Thus by Corollary
%\ref{cor:betared_preserve_incrbound} $\etalnf{\closure{M}}$ is safe
%and since safety is preserved by $\beta$-reduction, so is the
%beta-normal form of $\etalnf{\closure{M}}$. Thus by Lemma
%\ref{lem:incrbound_iff_etanf_safe}(i),
%$\tau(\betanf{\etalnf{\closure{M}}})$ is incrementally-bound.
%Hence we can assume without loss of generality that $M$ is a closed
%term
...
root $\theroot$ such that $\psi_M (t\filter r) = s$. We assume $t$
to be the shortest such traversal. Thus the last occurrence of $t$,
- let us name it $n$ - is hereditarily justified by $r$ and is by
definition an occurrence of a reachable node. Moreover since
$\psi_M$ maps $n$ to $q$, $n$ is necessarily an occurrence of a
variable node $x$. There are two cases:
\begin{itemize}
\item Suppose $x$ is bound variable. Let $m$ denote its justifier
in $t$ (which is an occurrence of $x$'s binder in $\tau(M)$).

By assumption $\tau(M)$ is
$N^{\theroot\vdash}$-incrementally-bound therefore since $n$
belongs to $N^{\theroot\vdash}$, $m$ must be the last
$\lambda$-node in $[\theroot,n]\ \inter N^{\theroot\vdash}$ of
order strictly greater than $\ord{n}$. By visibility, $m$ occurs
in $\pview{t}$ and since $m$ is also hereditarily justified by
$r$, $m$ must occur in $\pview{t} \filter N^{r \vdash}$ which in
turn is equal to $\pview{t \filter r}$ (by Lemma
...
Finally, since $\psi_M$ maps nodes of a given order to moves of
the same order (property \ref{proper:psi_properties}), we obtain
that $q$ points to the last O-move in $\pview{s}$ whose order is
strictly greater than $\ord{q}$.
...
\ref{prop:Nher_incrbound_iff_incrjustified} $\sem{\lambda x . f
...
Putting Proposition \ref{prop:Nher_incrbound_iff_incrjustified} and
...
incrementally-bound and by proposition
\ref{prop:Nher_incrbound_iff_incrjustified}, $\sem{M'}$ is a
...
\label{prop:Nher_incrbound_iff_incrjustified_pcf} Let $\Gamma \vdash
...
\ref{prop:Nher_incrbound_iff_incrjustified_pcf}(ii),
