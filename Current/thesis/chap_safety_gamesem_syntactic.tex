\notetoself{This chapter is taken from my transfer report. I need to rework it to integrate it correctly within the present thesis.}
...
nodes obtained by removing $n_1$ from $[n_1,n_2]$. We fix a term
$\Gamma \vdash M : T$ and we write $\theroot$ to denote the root of
the computation tree $\tau(M)$.
...
\defname{$A$-incrementally-bound} if either:
\begin{enumerate}
...
$$ x \mbox{ bound by } n \quad \imp \quad n \in [\theroot,x]\inter A \wedge \ord{n} > \ord{x} \wedge \forall \lambda\mbox{-node } n' \in ]n,x]\inter A  . \ord{n'} \leq \ord{x},$$
...
$$ x \mbox{ free } \quad \imp \quad  \forall \lambda\mbox{-node } n' \in ]\theroot,x]\inter A . \ord{n'} \leq \ord{x} \ .$$
\end{enumerate}
...
Let $\closure{M}$ denote the function that converts $M$ into a
closed term obtained from $M$ by abstracting all its free variables.
From the previous definition, if $\tau(M)$ is i.b.\ then so is
$\tau(\closure{M})$.
...
\item[(ii)] conversely, if $M$ is \emph{closed} and $\tau(M)$ is i.b.\ then the $\eta$-normal form of $M$ is safe.
\end{itemize}
\end{lemma}
\begin{proof}
...
$\lambda \overline{\xi}$; If its binder is $\lambda \overline{\xi}$
then since $\tau(M)$ is i.b.\ we have $\ord{z} \geq \ord{\lambda
...
\subsubsection{P-incremental-justified strategies}
...
\label{prop:incrbound_imp_incrjustified} The game denotation
$\sem{\Gamma \vdash M : T}$ is P-incrementally-justified if and only
if all the reachable input-variables in $\tau(\Gamma \vdash M : T)$
are incrementally-bound.
\end{proposition}

\begin{proof}
Take a simply-typed term $\Gamma \vdash M : A$ and let $\theroot$
denote the root of $\tau(M)$.

\noindent (If) Suppose that $\tau(M)$ is incrementally-bound, then
so is $\tau(\closure{M})$. Thus by Lemma
\ref{lem:incrbound_iff_etanf_safe}(ii), $\etalnf{\closure{M}}$ is
safe and since safety is preserved by $\beta$-reduction, so is the
beta-normal form of $\etalnf{\closure{M}}$. Thus by Lemma
...
$\sem{\betanf{\etalnf{\closure{M}}}}$ is P-i.j.\ since these two
game denotations are isomorphic).

Take a play $s \in \sem{\Gamma \vdash M}$ ending with a question
P-move $q$. By proposition \ref{prop:rel_gamesem_trav}, there is a
traversal $t$ of $\tau(M)$ starting with an occurrence $r$ of the
root $\theroot$ such that $\psi_M (t\filter r) = s$. Let $n$ be the
last occurrence in $t$. W.l.o.g.\ we can assume that $n$ is
...
By definition of $\psi$, $\psi(n) = x$ must be a move enabled by
the initial move $q_0 = \psi(r)$ in the arena $\sem{\Gamma
\rightarrow A}$. Therefore $\ord{q_0} > \ord{x}$. Since the
computation tree is incrementally-bound, all the $\lambda$-nodes
in $]\theroot,n]$ have order smaller than $\ord{n}$. Therefore
by the correspondence theorem, all the O-moves in $\pview{s}$
have order smaller than $\ord{x}$.
\end{itemize}

\noindent (Only if) Suppose $\sem{M}$ is P-incrementally-justified.
Let $x$ be a reachable input-variable node of $\tau(M)$: there
exists a traversal of the form $t \cdot x$ in $\travset(M)$ such
that $x$ is hereditarily justified by the first occurrence $r$ of
$\tau(M)$'s root in $t$.
...
the last $\lambda$-node in $\pview{?(t \filter r)}$ with order
strictly greater than $\ord{x}$. Moreover we have:
\begin{align*}
\pview{?(t \filter r)} &=
\pview{?(t)\filter r } \\
?(\pview{t}) \filter r & \mbox{by Lemma \ref{lem:betanf_wellbehavedconst_trav_pview_red};} \\
& = ?([\theroot,x[)  \filter N^{r \vdash} & \mbox{Prop.\ \ref{prop:pviewtrav_is_path}: Path-P-view correspondence;} \\
& = [\theroot,x[  \filter N^{r \vdash} & \mbox{The path $[\theroot,x[$ contains no value-leaf;} \\
& = [\theroot,x[  & \mbox{$M$ is in $\beta$-nf and $N_\Sigma =
\emptyset$}.
\end{align*}
...
last $\lambda$-node in $[\theroot,x[$ with order strictly greater
than $\ord{x}$ and if $x$ is a free variable (in which case it is
justified by the root) then all the $\lambda$-node in $]\theroot,x[$
have order smaller than $\ord{x}$. Hence $\tau(M)$ is
incrementally-bound.
\end{proof}

\begin{corollary}
  Let $M$ be a pure (\ie with no interpreted constants) simply-typed term
  in $\beta$-normal form. Then $\sem{M}$ is P-incrementally-justified if and only if $\tau(M)$ is incrementally-bound.
\end{corollary}
\proof This is because for a simply-typed $\beta$-normal term $M$,
all the variable nodes in the computation tree are reachable
input-variables. Indeed, let $x$ be a variable node of $\tau(M)$.
Since $M$ is $\beta$-normal, by lemma \ref{lem:betanorm_enabling},
$x$ is either hereditarily enabled by the root or by a constant in
$N_\Sigma$. But the pure simply-typed $\lambda$-calculus does not
have constants thus $N_\Sigma = \emptyset$ and $x$ is hereditarily
enabled by the root. Hence all variable nodes are input-variable
nodes. This in turns implies that every node of the computation tree
is reachable by the traversal consisting of the path from the root
to that node (the \rulenamet{InputVar} permitting us to visit the
children of the input-variable nodes).\qed
...
\ref{lem:incrbound_iff_etanf_safe} together we obtain a
...
$$ \sem{M} \mbox{ is P-incrementally-justified if and only if $\etabetalnf{M}$ is safe,} $$
where $\etabetalnf{M}$ denotes the $\eta$-normal form of the
...
denoted by $M'$ is also safe. By lemma
\ref{lem:incrbound_iff_etanf_safe} (i), $\tau(M')$ is
incrementally-bound and by proposition
