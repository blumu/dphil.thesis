...
\node(a0){$t = \ldots \cdot$};
\node[right=0pt of a0](a1){$\ \alpha\ \cdot$};
\node[right=0pt of a1](a2){$\ \lambda'$};
\node[right=0pt of a2](a3){$\cdot \ldots $};
\node[right=0pt of a3](a4){$\ v_{\lambda'} \ldots$};
\node[right=0pt of a4](a5){$\ \cdot\ \lambda$};
\draw[decorate,decoration={brace,raise=6pt,mirror},thick](a1) -- (a5) ;
\path(a1) --node[yshift=-12pt, below]{$u$} (a5) ;
\draw[->,\arrowcolor] (a5.north) .. controls +(150:1cm) and +(60:20pt) .. (a1.north);
\draw[->,\arrowcolor] (a2.north) .. controls +(160:8pt) .. (a1);
\draw[->,\arrowcolor] (a4) .. controls +(150:20pt) and +(50:20pt) .. (a2);
\end{tikzpicture}
\else
\pstr[0.5cm]{
...
\underbrace
    {
        \nd (lp-a,35){\lambda'}
        \nd \cdot  \ldots (v-lp){v_{\lambda'}}
        \txt \ldots
    }_{u}
    }
\nd \cdot (l-a,35){\lambda}
}
\fi
\end{center}
