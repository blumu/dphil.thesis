...
$$t-@ = \Pstr[0.5cm]{ (l){\lambda \overline{\xi}}
 \cdot (lx-l){\lambda x}\cdot (f)f\cdot (l2-f){\lambda} \cdot (x-lx)x} \ .$$
\end{example}
...
$$t^\star = \Pstr[0.5cm]{ (l){\lambda \overline{\xi}}
 \cdot (lx-l){\lambda x}\cdot (l2-lx){\lambda} \cdot (x-lx)x} \ .$$
\end{example}
...
2.\ Converesely, all the nodes that are hereditarily justified by
...
\item[(i)] If $t^\omega$ appears in $t\filterplus r_0$ then
    ...
    \end{itemize}
\item[(ii)] Suppose \Pstr[0.3cm]{t = \ldots r_0 \ldots (l){m} \ldots (n-l,30){n} }. The node  $n$ appears in $t\filterplus r_0$ if and only if $m$ does.
\end{itemize}
\end{lemma}
...
\subsubsection{Relating the two revealed denotations}
   ...
   \begin{center}
    ...
    \draw(Bk_op) .. controls +(160:2cm) and +(180:4cm) .. node[above]{$\Sigma$} (Xi_p);
    \draw[<->](Xj_o.north east) .. controls +(50:2cm) and +(-40:3cm) .. node[right]{$\Sigma$} (Xi_p);
    \end{tikzpicture}
    \end{center}
