...
there is a traversal $t$ of $\tau(M)$ such that $\psi_{M}(t\filter r) = s$. We assume that the last node $n$ of $t$ is
...
$m$ must occur in $\pview{t} \filter r$ which in turn
is equal to $\pview{t \filter r}$ (by lemma \ref{lem:redtrav_trav}(i), since $M$ is in $\beta$-normal form).
...
By property \ref{proper:psi_properties} (iii), the P-view of $s$ and
...
Finally, since $\psi$ maps nodes of a given order to moves of the
...
have:
\begin{align*}
\pview{t \filter r} &= \pview{t} \filter r & (\mbox{by lemma \ref{lem:redtrav_trav}}) \\
& = [r,x[\  \filter r & (\mbox{by proposition \ref{prop:pviewtrav_is_path}}) \\
