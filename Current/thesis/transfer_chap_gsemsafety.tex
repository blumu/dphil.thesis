...
there is a traversal $t$ of $\tau(M)$ such that $\psi_{M}(t
...
$\pview{t}$ which is equal to $[r,n]$ (by proposition \ref{prop:pviewtrav_is_path}), therefore $m$ is also the last $\lambda$-node in $\pview{t \upharpoonright  r}$ that has order strictly greater
than $\ord{n}$.
...
have:
\begin{align*}
\pview{t \upharpoonright r} &= \pview{t} \upharpoonright r & (\mbox{by lemma \ref{lem:redtrav_trav}}) \\
...
\upharpoonright r = (l0){\lambda^0} \cdot
(6l0-l0){6_{\lambda^0}}. }
$$
