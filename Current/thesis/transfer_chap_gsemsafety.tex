...
\ref{lem:betanf_orde1const_trav_pview_red}, since $M$ is in $\beta$-normal form).
...
have:
\begin{align*}
\pview{t \filter r} &= \pview{t} \filter N^{r \vdash} & (\mbox{by Lemma \ref{lem:betanf_orde1const_trav_pview_red}}) \\
...
\ref{lem:sigma_order1_are_wellbehaved}), Lemma
\ref{lem:betanf_orde1const_trav_pview_red} holds in the safe \pcf\
setting. Thus proposition \ref{prop:incrbound_imp_incrjustified}(i)
remains valid in \pcf\ for infinite computation trees: infinite
terms in $\beta$-nf with an incrementally-bound computation tree are
denoted by P-incrementally-justified strategies. Consequently,
