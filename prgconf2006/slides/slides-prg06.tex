...
\begin{itemize}
\item Model of programming languages based on games (Abramsky et al.; Hyland and Ong; Nickau).
\item 2 players: \highlight{O}pponnent (system) and \highlight{P}roponent (program)
\item The game definition is given by the type of the term: the \highlight{arena} indicates which moves are possible.
 \begin{center}
 \psset{treesep=2ex,treefit=loose,nodesep=1pt}
$ \sem{\nat} = \pstree[levelsep=4ex]
    { \TR{q} }
    {    \TR{\scriptstyle 0} \TR{\scriptstyle 1} \TR{\scriptstyle \ldots} }
    \hspace{1cm}
  \sem{\nat \rightarrow \nat} = \pstree[levelsep=3ex,treesep=2ex]
    { \TR{q^0} }
    {  \pstree
        { \TR{q^1} }
        {    \TR{\scriptstyle 0} \TR{\scriptstyle 1} \TR{\scriptstyle \ldots} }
...
\end{center}
\item \highlight{Play} = sequence of moves played by O and P with justification pointers.
\item \highlight{Strategy for P} = prefix-closed set of plays. $s \cdot a \cdot b$ in the strategy means that P should responds $b$ when O plays $a$ in position $s$.
...
on it.}
\item Allows us to give game-semantic characterization of syntactic restrictions such as Safety.
\end{itemize}

\highlight{Related works:}
\begin{itemize}
\item Ong introduced computation trees in LICS2006 and proved decidability of MSO theory on infinite trees
...
\item Extending the pointer economy result to Safe Idealized Algol. Decidability of program equivalence?
\end{itemize}
}
