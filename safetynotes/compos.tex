...
\tikzstyle{move}=[->,shorten <=1pt,>=latex',line width=1pt,color=\genpmovecolor]
...
% arrows representing the moves
  ...
  (pop.20)    edge[move, genomove,genpmove]
		node[right] {$\sigma$}
		node[left]{$\pomove$} (opp.-20)
  (opp.-160)  edge[move, genomove, genpmove]
		node[left] {$\mu$} 
...
{\bf Notation} We use the symbol $\omove$ to represent an O-move and $\pmove$ to represent a P-move.

Let us fix two strategies $\sigma : A \rightarrow B$ and $\mu : B \rightarrow C$. Let $u$ be a play of the interaction $u \in \sigma \| \mu$ between $\sigma$ and $\mu$,
Moves played in the game $A\rightarrow B$ are said to belong to  the \defname{component $\mathcal{L}$} whereas
moves played in $B\rightarrow C$ are said to belong to the \defname{component $\mathcal{R}$}.
For any move $m$ in $A\union C$, $Cmp_x$ 
denotes the component $\mathcal{L}$ if $x\in A$ and $\mathcal{R}$ if $x\in C$.

We write $s \sqsubseteq t$ to say that $s$ is a subsequence
of $t$.
...
\begin{lemma}
\label{lem:interjump}
Let $X$ be a component (either  $\mathcal{L}$ or  $\mathcal{R}$).
...
\item  $z$ is a generalized P-move ($z \in \{ \pmove, \opmove, \pomove \}$) in $X$,
...
We define the interaction sequence $v$ as 
$v = u_{<z}$ if $z$ is a P-move in $A$ or $C$, and
$v = u$ if $z\in B$.
In both cases, the last move of $v$ is a generalized O-move
in the component $X$ (see diagram of Table \ref{tab:interseq}).
Therefore by \cite[Lemma 3.3.1]{Harmer2005}
we have $\pview{v \filter X} = \pview{\overline{v} \filter X}$.

Clearly since $x$ and $y$ are visible at $u$ they
must also be visible at $v$. Hence we have: 
\begin{align*}
x,y &\in \pview{v \upharpoonright X}
& \mbox{($x,y$ visible at $v$)} \\
&= \pview{\overline{v} \upharpoonright X} & \mbox{(last move of $v$ is a generalized O-move, \cite[Lemma 3.3.1]{Harmer2005})} \\ 
& \sqsubseteq \overline{v} \ .
\end{align*}
Since $x,y$ are played in $A$ or $C$ we have 
$x,y  \in \overline{v} \upharpoonright A,C 
= \pview{v \upharpoonright A,C}$ (lemma \ref{lem:interaction_projection}).
The move $b$ lies underneath the PO-arc $y$-$x$ therefore $b$ does not appear in the P-view  $\pview{v \upharpoonright A,C}$.

Finally we have $\pview{u\filter A,C}=
\pview{v\filter A,C} \cdot 
\left\{ \begin{array}{ll} \epsilon & \mbox{ if $z\in B$} \\
z & \mbox{ elsewhere.}  \end{array} \right.$. Thus
$b$ does not appear in $\pview{ u \filter A,C}$ either.
\end{proof}
...
move of $ u_{\leq m} \filter A,C$  it cannot be an initial move in C
...
Since $\mu$ is P-ij, $n'$ must not occur in the P-view $\pview{u_{\leq m} \filter B,C}$, thus $n'$ lies underneath a PO arc occurring in $\pview{u_{\leq m} \filter B,C}$. Let us denote this arc by $\beta$-$\alpha$ where $\beta$ and $\alpha$ denote the arc's nodes:
...
$X\leftarrow \mathcal{R}$ and $y,b,x,z \leftarrow \beta, n', \alpha, m$ we obtain that $n' \not\in\pview{u_{\leq m} \filter A,C}$.
...
\item Suppose $n'\in A$. 
The interaction sequence $u$ has the following form:
...
Since $\sigma$ is P-ij, $n'$ cannot occur in the P-view $\pview{u_{\leq m} \filter A,B}$ thus $n'$ must occurs underneath a PO arc occurring in $\pview{u_{\leq m} \filter A,B}$. Let us denote this arc $\beta$-$\alpha$:
$$ u_{\leq m} \filter A,B = \ldots  
\Pstr[0.5cm]{
 (n){\stk{n} \omove } \ldots (b){\stk\beta \pmove} \ldots \stk{n'} \omove  
\ldots (a-b){\stk\alpha \omove}  \ldots (m-n){\stk m  \pmove }
} \ .
...
By instancing Lemma \ref{lem:interjump} with
$X\leftarrow \mathcal{L}$ and $y,b,x,z \leftarrow \beta, n', \alpha, m$ we obtain that $n' \not\in\pview{u_{\leq m} \filter A,C}$.
...
\ncarc[arcangleA=90,arcangleB=90]{->}{a2}{b2}
 $$  
By instancing Lemma \ref{lem:interjump} with
$X\leftarrow \mathcal{L}$ and $y,b,x,z \leftarrow \beta^2, n', \alpha^2, \alpha$ we obtain that $n' \not\in\pview{u_{\leq \alpha} \filter A,C}$ and since $\alpha$ is visible at $m$ in the game $A\rightarrow C$, $n'$ does not appear in $\pview{u_{\leq m} \filter A,C}$ 
either. 
