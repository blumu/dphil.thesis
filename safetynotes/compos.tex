...
dead occurrence of $x$ if its corresponding node in the computation tree does not appear in any traversal of $\travset(M)$. Equivalently, thanks to the Correspondence Theorem, an occurrence of $x:B$ is dead if and only if the initial move
of the arena $\sem{B}$ does not appear in any play of $\sem{M}$.


Let $M$  be a \pcf\ term.
We define $M^*$ as the term obtained from $M$ after substituting
each dead variable occurrence $x:(B_1,\ldots, B_k, o)$ by
$\lambda \varphi^1 \ldots \varphi^k . 0$ for some fresh variables $\varphi^1:B_1$,\ldots,$\varphi^k:B_k$.
This process is called \defname{dead variable elimination}.
...
Let $\sigma$ be a P-incrementally-justified strategy with finite view function defined on the game $A$. There exists a \emph{safe} closed PCF term $\vdash_s M : A$ in $\eta$-long normal form such that:
$$ \sem{M_\sigma} = \sigma $$
\end{proposition}
\begin{proof}
