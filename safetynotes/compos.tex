...
\begin{definition}\rm
  A strategy $\sigma$ is said to be \defname{P-incrementally
    justified}, or {\emph P-ij} for short, if for any play $s \, q \in \sigma$ where $q$ is a
...
a sufficient condition for $A\rightarrow C$ to be homogeneous is  ``$\ord{A} \geq \ord{B}$''.
...
\subsection{Proof of compositionality when $\ord{A}\geq\ord{B}$}
...
We start by proving several useful lemmas.
\begin{lemma}
\label{lem:interjump}
...
\item $u\filter A,B$ is P-ij,
\item $u\filter B,C$ is P-ij.
\end{enumerate}
Then:
$$ \ord_{Y} m \geq \ord_{A\rightarrow C} n' \ .$$
\end{lemma}
\begin{proof}
...
$\ord_{A\rightarrow B} \gamma \geq \ord_{B\rightarrow C} \gamma$.
If $n=\omove_A$ then $\gamma$ is a $\pomove$-move therefore it is not initial in $B$ and Lemma \ref{lem:compositionorder} gives
$\ord_{A\rightarrow B} \gamma = \ord_{B\rightarrow C} \gamma$.

Hence:
\begin{align*}
\ord_{A\rightarrow C} n' 
& = \ord_{X} n' & \mbox{(n' non initial in $C$ \& Lemma \ref{lem:compositionorder})} \\
& \leq \ord_{X} \gamma & \mbox{($u\filter Y$ is P-ij \& $\gamma$'s justifier occurs before $n'$)} \\
& = \ord_{Y} \gamma & \mbox{(By Eq. \ref{eqn:gammaorderXY})} \\
& \leq \ord_{Y} m & \mbox{($u\filter X$ is P-ij \& 
4$^{th}$ assumption: $m$'s justifier occurs before $\gamma$)} \ . 
%\\ & = \ord_{A\rightarrow C} m & \mbox{($m$ is not initial in $C$ \& Lemma \ref{lem:compositionorder})}
\end{align*}
\end{proof}
...
\begin{enumerate}[(1)]
\item \label{case:mC}
Suppose $m =\pmove_C$, then $n$ necessarily $n=\omove_C$.

\begin{enumerate}[({\ref{case:mC}}.a)]
\item \label{case:mCnpC} Suppose $n' = \omove_C$. The projection of $u$ on the game $B\rightarrow C$ has the following form:
$$ u \filter B,C =  
\Pstr[0.5cm][2pt]{ \ldots (n){\stk {n} \omove}  \ldots
 {\stk {n'} \omove}  \ldots  (m-n,30){\stk m \pmove}
}$$
...
\begin{enumerate}[(i)]
\item Suppose $\alpha =\omove_C$ then necessarily
$\beta=\pmove_C$ (since $\alpha$ is an O-move).

By instancing Lemma \ref{lem:interjump} with
$X\leftarrow B,C$ and $n \leftarrow n'$ we obtain that $n' \not\in\pview{u \filter A,C}$.

\item Suppose $\alpha=\pomove$ then necessarily $\beta=\opmove$.
...
\begin{align*}
\ord_{A\rightarrow C} n' 
& \leq \ord_{A\rightarrow B} \alpha & \mbox{(Lemma \ref{lem:increasing_order} with $u\leftarrow u_{\prefixof \alpha}$)} \\
...
$ \ord_{A\rightarrow C} n'  \leq \ord_{B\rightarrow C} m$.
 By Lemma \ref{lem:compositionorder}, since
 $m$ is not initial in $C$ we have $\ord_{B\rightarrow C} m = \ord_{A\rightarrow C} m$ thus $\ord_{A\rightarrow C} n' \leq \ord_{A\rightarrow C} m$.
\end{enumerate}
...
Let us assume that $n'\in \pview{u\filter A,C}$ and let us prove that necessarily $\ord_{A\rightarrow C} n' \leq \ord_{A\rightarrow C} m$.
...
$\ord_{A\rightarrow C} n' \leq \ord_{A\rightarrow C} m$.
	...
	If $n'$ occurs between $b_0$ and $m$ then $m$ points before $n'$ in 		$u$ so we can use Lemma \ref{lem:increasing_order} which gives $\ord_{A\rightarrow C} n' \leq \ord_{A\rightarrow B} m
		...
		The assumption that $\ord{A} \geq \ord{B}$ becomes
		crucial now (note that we have not used it yet):
		\begin{align*}
		\ord_{A\rightarrow C} m 
		& = \ord_{A\rightarrow B} m & \mbox{(Lemma \ref{lem:compositionorder})} \\
		& = \ord_{A\rightarrow B} b_0 -1  & \mbox{($b_0$ initial in $B$)} \\
		& \geq \ord_{B\rightarrow C} b_0 & \mbox{($b_0$ initial in $B$ \& $\ord{A} \geq \ord{B}$)} \\
		& \geq \ord_{B\rightarrow C} n' & \mbox{($\mu$ P-ij
		 \& $n' \in \pview{u_{\prefixof b_0}\filter A,C}$ \& 
		$b_0$ points before $n'$)} \\
		& = \ord_{A\rightarrow C} n' & \mbox{(Lemma \ref{lem:compositionorder})} \ .
		\end{align*}
		\end{enumerate}
