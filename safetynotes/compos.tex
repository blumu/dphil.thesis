...
\subsection{Interaction sequences}
 ...
 The diagram below shows the structure of the interaction of two strategies:
...
Let $\sigma : A \rightarrow B$ and $\mu : B \rightarrow C$ be two P-ij strategies.

Let $u$ be a play of the interaction between $\sigma$ and $\mu$ :
$u \in \sigma \| \mu$ and $m$ be a P-move in $u$.


Suppose $m$ points in $\pview{u_{<m} \upharpoonright A , C}$ to a move $n_1$.

Let $n_1, n_2, \ldots, n_g$ be the O-moves of order $> \ord{m}$ 
occurring in that order in $\pview{u_{<m} \filter A,C }$.

To prove that $\pview{u_{<m} \filter A,C}$ is incrementally justified, we just
need to prove that $g=1$. We have:
$$ \pview{u_{\leq m} \filter A,C} =  
\Pstr[0.5cm][2pt]{ \ldots (n1){\stk {n_1} O}  n_1^+ \ldots
 {\stk {n_g} O} n_g^+ \ldots m^- (m-n1,30){\stk m P}
}
$$

Since $n_2, \ldots, n_g$ are O-moves occurring in the P-view at $m$, they are not initial in C, therefore $\ord_{B\rightarrow C}{n_i} = \ord_{A\rightarrow C} (n_k)$ for $k\in \{2..i\}$.

Suppose $m$ is played in $C$ then $m$ is not initial in $C$ and therefore
$\ord_{A\rightarrow C}{m} = \ord_{B \rightarrow C}{m}$.

Since $\mu$ is P-ij, $n_g$ cannot occur in the P-view $\pview{u_{\leq m} \filter B,C}$, $n_i$ must occurs underneath a PO arc, let us name the nodes of this arc $\beta$ and $\alpha$ :
