...
Let $\overline{A}=(A_1,\ldots A_i)$ and $\overline{B} =(B_1, \ldots, B_l)$ be two PCF types for some $i,l\geq 0$ and $\sigma$ be a well-bracketed innocent
P-i.j. strategy with finite view function defined on the game $!A_1 \otimes \ldots \otimes !A_i \fngamear (!B_1 \fngamear \ldots \fngamear !B_l \fngamear o) $. There exists a \emph{semi-safe} PCF' term $\overline{x} : \overline{A} \vdash_s M : \overline{B}$ in $\eta$-long normal form such that:
$$ \sem{\overline{x} : \overline{A} \vdash M_\sigma : \overline{B}} = \sigma $$
and a safe closed PCF' term $\vdash_s M'_\sigma : (\overline{A},\overline{B})$ in $\eta$-long normal form such that:
$$ \sem{\vdash M'_\sigma : (\overline{A},\overline{B})} \cong \sigma \ .$$
\end{proposition}
\begin{proof}
...
Take $M_\sigma$ to be $\etanf{\betanf{N}}^* $. We have $\sem{\overline{x} : \overline{A} \vdash M_\sigma : \overline{B}} =  \sem{\overline{x} :\overline{A} \vdash N : \overline{B}} = \sigma$ and by Theorem  \ref{thm:semisafeincrejust_pcf}(ii), $M_\sigma$ is semi-safe.
...
$\overline{B} = (B_1, \ldots, B_l,o)$ 
and $C=(C_1,\ldots,C_k,o)$ be three PCF types
for some $i\geq 1,l,k\geq 0$. Let
$f:\ !A_1 \otimes \ldots \otimes !A_i \fngamear \overline{B}$ and $g:\ !\overline{B}\fngamear C$ be two innocent well-bracketed and P-incrementally justified strategies with finite view function.
We would like to find under which conditions the composition $f\fatcompos g$ is also P-incrementally justified.

By the definability result, there are two closed safe terms (in $\eta$-nf) $\vdash M_f :(\overline{A},\overline{B})$  and $\vdash M_g :(\overline{B},C)$ such that $\sem{M_f} = f$
and $\sem{M_f} = g$.
...
We will now exploit this fact to extract a sufficient condition on the types $A$ and $B$ to guarantee that 
the composition of $f$ and $g$ is P-incrementally justified.

The term $M_f$ and $M_g$, being in $\eta$-nf, are of the following forms:
\begin{eqnarray*}
\vdash M_f &=& \lambda \overline{x}^{\overline{A}} \varphi_1^{B_1} \ldots \varphi_l^{B_l} . N_f^o\\
...
& \mbox{($\overline{x}$ and the $\phi$'s are disjoint)}.
\end{align*}
...
for $\lambda x \phi_1 \dots \phi_k. N_g [(M_f x) / y]$ to be safe , and hence for $f\fatcompos g$ to be P-incrementally justified. However it is not a necessary condition, for instance we could have $N_g[(M_f x) /y]$ unsafe while its eta-beta normal form is safe.

\subsubsection{A simpler sufficient condition}
% The following commented lemma has been simplified into the lemma following the commented text using the 
% substitution lemma of the safe lambda calculus.
%
%\begin{lemma}
%If $y:B, \Sigma \vdash N : T$ is a safe term in $\eta$-nf
%and $\vdash M : A \typear B$ is a safe term with $\ord{A} \geq \ord{B}$
...
Hence if for all $1 \leq i \leq n$ we have $\ord{A_i}\geq\ord{B}$
then by the previous lemma the term $\vdash \lambda x \phi_1 \dots \phi_k. N_g [(M_f x) / y]$
is safe and therefore its denotation $\sem{\vdash \lambda x \phi_1 \dots \phi_k. N_g [(M_f x) / y]} = f\fatcompos g$ is P-incrementally justified.
This gives us a sufficient condition for $f\fatcompos g$ to be P-incrementally justified.

Of course this condition is not necessary. For instance take $A=o$, $B=(o,o)$, $C=(o,o)$ and consider the two safe terms $M_f = \lambda x u.u$ and $M_g = \lambda y . y a$ for $x,u:o$, $y:B$ and constants $a:o$. Then we have $M_{f\fatcompos g} = \lambda x . a$ which is safe hence $f\fatcompos g$ is P-incrementally justified although $\ord{A} < \ord{B}$.
...
$f\fatcompos g$ is not P-incrementally justified. However since $M_g$ contains an order $2$ constants ($\varphi$), it is not a valid simply typed $\lambda$-term nor a \pcf-term therefore we cannot use this theorem.
...
$x,u,v:o$, $y:B$ and $\varphi:((o,o),o)$ and the $\Sigma$-constant $a:o$. Take the two safe terms $\vdash  M_f = \lambda xv.x : A\typear B$ and  $\vdash M_g = \lambda y \varphi. \varphi (\lambda u . y a) : B\typear C$.
