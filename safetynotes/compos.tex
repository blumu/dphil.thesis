...
if $X=A,B$ then $n'$ is in $A$. Thus in both cases, Lemma \ref{lem:compositionorder} gives us the required equality.
...
\item \label{case:mA} Suppose $m = \pmove_A$ and $n=\omove_C$.
...
Since $n' \in \pview{u\filter A,C}$
and $b_0 \in \pview{u\filter B,C}$,
applying Lemma \ref{lem:visibleatprefixofu} 
...
$n' \in \pview{u_{\prefixof b_0}\filter A,C}$. This allows us to apply Lemma \ref{lem:increasing_order} on $u_{\prefixof b_0}$: 
		...
		& \geq \ord_{B\fngamear C} n' & \mbox{(Lemma \ref{lem:increasing_order} on $u_{\prefixof b_0}$)} \\
		& \geq \ord_{A\fngamear C} n' & \mbox{(Lemma \ref{lem:compositionorder} \& $n'$ non initial)} \ .
		\end{align*}

\item Suppose $n'$ occurs after $b_0$ (and necessarily before $m$).

	\begin{enumerate}[a.]
	\item Suppose $n'=\omove_C$. Since $m$'s justifier occurs before $n'$ in $u$, we can use Lemma \ref{lem:increasing_order} which gives $\ord_{A\fngamear C} n' \leq \ord_{A\fngamear B} m
		= \ord_{A\fngamear C} m$.
