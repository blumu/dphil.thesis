...
The order of the arena $A$ is defined to be the maximal order of its initial moves. The order of a (simple, PCF or IA) type is defined as the order of the arena denoting it; equivalently a ground type has order 0, $\ord{A\rightarrow B} = \max(1+\ord{A},\ord{B})$ and $\ord(A\times B) = \max(\ord A, \ord B)$.
...
\subsection{Node-order after composition}
...
\subsection{Safe $\Lambda^{\rightarrow}$ + product}
We will now show how product types and pairing can be added to the safe lambda calculus. 
To achieve this some care must be taken. Indeed, the arenas involved in the calculus defined in the previous section are all prime arenas {\it i.e.}~they have only one initial move. With product types however, arenas can have more than one initial move. We therefore need to adapt the side-conditions of the application and abstraction rules in order to guarantee that the game semantic characterization still holds.
