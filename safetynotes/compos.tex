...
is a \defname{generalized O-move in $A,B$}
and a \defname{generalized P-move in $B,C$}.
This means that $\opmove$  plays the roles of O in  $A\rightarrow B$ and P in 
$B\rightarrow C$.
Similarly, the move $\pomove$ (with component $B,C$)
is a \defname{generalized P-move in $A,B$}
and a \defname{generalized O-move in $B,C$}.
...
%\hrulefill
%First method:
%Let $n$ be the last external O-move in $u_{]n,m[}$
%such that $n\in \pview{u_{\prefixof m} \filter A,C}$
%with $\ord_{A\rightarrow C} n > \ord_{A\rightarrow C} m$.
%We want to prove that $m$ must necessarily point to $m$.
%
%Suppose that $m \in C$ and $n\in C$.
%
%Let us consider the game in the $B,C$ component.
%Then we have $\ord_{B\rightarrow C} n > \ord_{B\rightarrow C} m$.
%Indeed this is clearly true if $n$ is an initial move in $C$
%and if it is not then by Lemma \ref{lem:compositionorder} we have
%$\ord_{B\rightarrow C} n = \ord_{A\rightarrow C} n > \ord_{B\rightarrow C} m = \ord_{B\rightarrow C} m$.
%
% Suppose that $n$ is non initial
%
%\hrulefill
%%%%%%%%%%%%%%%%%%%%%%
...
Instancing Lemma \ref{lem:oviewsegmentinB} with 
$A,B,x,y\leftarrow B,C,\beta,\alpha$ shows that 
all the moves occurring between $\beta$ and $\alpha$ in the O-view are in $B$. Thus since $n'\in C$, $n'$ must lies underneath one
of the OP-arc in the O-view $u_{\prefixof \alpha} \upharpoonright B,C$. Call it $\beta^2$-$\alpha^2$:
$$ 
u_{\prefixof \alpha} \upharpoonright {B, C}
=
\Pstr[0.1cm]{\cdots \stk\pmove{\beta} (o1)\omove \cdots (p1-o1)\pmove 
(o2)\omove \cdots (p2-o2)\pmove (o2)\omove \ \cdots\ 
\pmove  (ojump) {\stk\omove{\beta^2}} \cdots
\stk\omove{n'} \cdots
(pjump-ojump) {\stk\pmove{\alpha^2}} \omove
\cdots
\pmove (o4)\omove \cdots (p4-o4)\pmove \ \stk\omove{\alpha}
}\ .
$$
%We have 
%\begin{eqnarray*}
%\alpha,\alpha^2  \in&  \pview{u_{\prefixof \alpha} \filter B,C} \\
% \prefixof & \pview{u_{\prefixof m} \filter B,C}  & \mbox{($\alpha$ P-visible from $m$)}\\
%\subseqof & \pview{u_{\prefixof m}} & \mbox{($\alpha$ P-visible from $m$)}\\
%\end{eqnarray*}

By P-ij of $\mu$ we have:
$$ 
 \ord_{B\rightarrow C} \alpha \leq \ord_{B\rightarrow C} m$$
