...
$n' \in A$ and $m$ is either a P-move in $C$ or a $B$ move ($\opmove$ or $\pomove$). Then there is a move $\gamma = \pomove \in B$ in $r$ such that $\gamma \in \pview{u\filter B,C}$ and $n' \in \pview{u_{\leq \delta} \filter A,B}$.
\end{lemma}
\begin{proof}
By induction on $|r|$.
...
Suppose $|r|>1|$ then we do a case analysis on $m$:
\begin{enumerate}[i.]
\item Suppose $m = \pmove \in C$.
