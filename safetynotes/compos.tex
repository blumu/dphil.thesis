...
We abbreviate this new system of rules as $\Lambda^{\rightarrow}_\times$.

One can easily check that most of the good properties of the safe lambda calculus remains: the free variables of a term have order greater that the term itself; the no-variable-renaming Lemma holds, and $\Lambda^{\rightarrow}_\times$-terms
are denoted by P-i.j. strategies.

However it is not true in general that terms are denoted by \emph{closed} P-i.j. strategies. Indeed, in the safe lambda calculus without product - as defined in the previous section - all the arenas involved are prime {\it i.e.}~they have a single initial move.  However when introducing product types, arenas can have many initial moves.
Consequently Lemma \ref{lem:closedpij_singleBinitmove}(ii) does not hold anymore.


It is therefore necessary to make a modification to our calculus to guarantee that the game semantic characterization still holds.
