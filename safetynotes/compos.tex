...
strategies. 
If $\ord{A} \geq \ord{B}$ then $\sigma ; \mu : A \fngamear C$
is a well-bracketed P-incrementally-justified
\end{proposition}

\begin{proof}
We already know that well-bracketness
is preserved by strategy composition (see \cite[Proposition 2.5]{abramsky94full}).
It remains to prove that $\sigma ; \mu$ is P-ij.
...
& \leq \ord_{B\fngamear C} m & \mbox{($\mu$ is P-ij)} \\
...
with $\ord_{B\fngamear C} \alpha \leq \ord_{B\fngamear C} m$ (by P-ij of $\mu$).
...
$\ord_{A\fngamear C} n' \leq \ord_{A\fngamear C} m$.
	...
	If $n'$ occurs between $b_0$ and $m$ then $m$ points before $n'$ in 		$u$ so we can use Lemma \ref{lem:increasing_order} which gives $\ord_{A\fngamear C} n' \leq \ord_{A\fngamear B} m
		...
		& = \ord_{A\fngamear B} b_0 -1  & \mbox{($b_0$ initial in $B$)} \\
		& \geq \ord_{B\fngamear C} b_0 & \mbox{($b_0$ initial in $B$ \& $\ord{A} \geq \ord{B}$)} \\
		& \geq \ord_{B\fngamear C} n' & \mbox{($\mu$ P-ij
		 \& $n' \in \pview{u_{\prefixof b_0}\filter A,C}$ \& 
		$b_0$ points before $n'$)} \\
		& = \ord_{A\fngamear C} n' & \mbox{(Lemma \ref{lem:compositionorder})} \ .
		\end{align*}
		\end{enumerate}
...
\begin{corollary}
Let $A$ and $B$ be two well-opened games with $\ord{A} \geq \ord{B}$. Let
$\sigma :\ !A \fngamear B$ and $\mu :\ !B\fngamear C$ be two well-bracketed and P-incrementally justified strategies then $\sigma \fatcompos \mu :\ !A \fngamear C$ is also well-bracketed and P-incrementally justified.
\end{corollary}
...
\subsection{Composition of P-ij strategies}
...
The $\eta\beta$-nf of $M_{f\fatcompos g}$ is $\vdash \lambda x \varphi. \varphi (\underline{\lambda u . x})$ which is unsafe because of the underlined term, thus by Theorem \ref{thm:safeincrejust}(ii), $f\fatcompos g$ is not P-incrementally justified. The following diagram illustrates  a play of $f\fatcompos g$ that is not P-ij.
...
Any strategy on the game $I \fngamear\ !var$ is P-ij since there is no P-question in the arena $var$.
