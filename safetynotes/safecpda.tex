...
is said to be \defname{safe} just if $\orddec{s} = \langle
\lambda \overline{\eta}_r^1, \ldots , \lambda
\overline{\eta}_1^1 \rangle$. {\it i.e.}~all the $k$-components
in $\orddec{s}$ are equal to $1$;

\item A an order-$j$ stack $s$ with $j\geq2$ is
\defname{$q$-safe} for some $1 \leq q \leq r$ just if
$\orddec{s} = \langle \lambda \overline{\eta}_r^1, \ldots ,
\lambda \overline{\eta}_q^1, \lambda
\overline{\eta}_{q-1}^{k_{q-1}}, \ldots , \lambda
\overline{\eta}_1^{k_1} \rangle$ where $collapse (s_{\prefixof
\lambda \overline{\eta}_i^1})$ is $1$-safe for all $q \leq i
\leq r$ such that $n-\ord{\lambda \overline{\eta}_i}+1 <
\ord{s}$.
...
By Lemma \ref{lem:pushj_safe_implies_b-safe}, $s''$ is $b$-safe.
...
Since the recursion scheme is safe, $x$ is incrementally bound
 ...
 node to the root of the computation tree. Hence the binder of
$x$ is precisely $\lambda \overline{\eta}_{b}^1$.
...
When $collapse$ is executed, the top stack symbol is $u$'s
binder $\lambda \overline{\eta}_{b}^1$ which has a link of
type $k=1$ therefore condition (ii) is verified.

Since $push_{n-l+1}\ s$ is $b$-safe, by definition we have that
$collapse \left( (push_{n-l+1}\ s)_{\prefixof \lambda
\overline{\eta}_{b}} \right) = (push_{n-l+1};pop_1^p;collapse) \
s $ is safe. Finally we conclude using Lemma
\ref{lem:push1pop1_preserves_safety} that $s' = push_1
E_i(top_1)^{(n-l+1,1)}\ t$ is safe, thus condition (i) holds.
%\end{itemize}
