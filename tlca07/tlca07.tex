...
\label{def:nodeorder}
The order function $\textsf{ord}$ is defined on nodes as follows:
\begin{align*}
...
\noindent \rulename{($\epsilon$)} The empty sequence $\epsilon$ is a traversal.

\noindent \rulename{(Root)} The length 1 sequence constituted of the root of $\tau(M)$ is a traversal.
...
\begin{itemize}
  ...
  \item (CCAnswer-$\lambda$) If \raisebox{0cm}[0.5cm]{$t \cdot \lambda \overline{\xi} \cdot \rnode{x}{x} \cdot \ldots \cdot  \rnode{xv}{v_x}
     ...
     \item (CCAnswer-var) If \raisebox{0cm}[0.5cm]{$t \cdot y \cdot \rnode{lmd}{\lambda \overline{\xi}}
                   \cdot \ldots\cdot \rnode{lmdv}{v_{\lambda \overline{\xi}}} \bkptr[nodesep=1pt]{30}{lmdv}{lmd} \bklabelc{v}$} is a traversal,
                   where $y$ is a non input-variable node, then so is
        \raisebox{0cm}[1cm]{$t \cdot \rnode{y}{y}
...
If  \raisebox{0cm}[0.5cm]{$t' \cdot \rnode{n}{n} \cdot
    ...
    \rnode{x}{x_i}  \bkptr[ncurv=0.6]{50}{x}{lx} \bklabel{i}$} is a traversal where $x_i$ is not an input-variable then
    \vspace{0.3cm}
...
\noindent {\bf case 1} If $n$ is an order $0$ $\lambda$-node or a ground type variable node then
        $$f_A(n,q) = \{ n \mapsto q \} \quad \union \quad  \{ v_n \mapsto v_q \ | \ v \in \mathcal{D} \}$$

\noindent {\bf case 2} If $n$ is a $\lambda$-node labelled $\lambda \overline{\xi} = \lambda \xi_1 \ldots \xi_p$ with $p\geq 1$ then
...
\noindent {\bf case 3} If $n$ is a variable node labelled with a higher-order variable $x$ of type $(A_1,\ldots,A_m,o)$ then the computation tree and the arena $A_q$
...
\begin{example}
The strategy $\sem{ev : (A^1 \Rightarrow B^2) \times  A^1 \rightarrow B^1}$ is not
incrementally-justified. Indeed consider the play $s = q_0^{B^1} q_1^{B^2} q_2^{A^1}
q_3^{A^2} \in \sem{ev}$. The order of the moves are $\ord{q_3} = \ord{A}$,
...
$\tau(M)$. Then:
\begin{align*}
(i)  \quad\varphi_M :\travset(\Gamma \vdash M : T)^* \stackrel{\cong}{\longrightarrow}  \intersem{\Gamma \vdash M : T}\ ,  \\
(ii) \quad \varphi_M : \travset(\Gamma \vdash M : T)^{\upharpoonright r} \stackrel{\cong}{\longrightarrow} \sem{\Gamma \vdash M : T} \ .
\end{align*}
\end{proposition}
...
\subsubsection{Example : \pcfcond}
Consider the term $M = \lambda x y . \pcfcond\ 1\ x\ y$. Its
computation tree is represented on the right (without the value-leaves).
...
$$ t = \rnode{lxy}{\lambda x y} \cdot \rnode{cond}{\pcfcond} \cdot \rnode{l1}{\lambda^1} \cdot \rnode{1}{1} \cdot \rnode{11}{1_1}
    \cdot \rnode{l2}{\lambda^2} \cdot \rnode{y}{y} \cdot \rnode{vy}{v_y}  \cdot \rnode{vl2}{v_{\lambda^2}} \cdot \rnode{vcond}{v_{\pcfcond}}
    \cdot \rnode{vlxy}{v_{\lambda x y}}.
\bkptr[offset=-4pt]{20}{vlxy}{lxy}
\bkptr[offset=-4pt]{20}{vcond}{cond}
\bkptr[offset=-4pt]{20}{vl2}{l2} \bkptr[offset=-4pt]{20}{vy}{y}
\bkptr[offset=-4pt]{20}{y}{vxy} \bkptr[offset=-4pt]{20}{l2}{cond}
\bkptr[offset=-4pt]{20}{11}{1} \bkptr[offset=-4pt]{20}{l1}{cond}
$$

\parpic[r]{$$\begin{array}{ccccccccccc}
...
\nccurve[linewidth=0.5pt,nodesep=2pt,ncurv=0.9,angleA=180,angleB=180]{->}{vqy}{qy}
\ncarc[linewidth=0.5pt,nodesep=2pt,ncurv=0.9,angleA=180,angleB=180]{->}{qy}{qb}
...
$$}
We have $ t^* = \rnode{lxy}{\lambda x y} \cdot
...
\bkptr[offset=-4pt]{20}{vy}{y} \bkptr[offset=-4pt]{20}{y}{vxy}
\bkptr[offset=-4pt]{20}{l2}{lxy} \bkptr[offset=-4pt]{20}{l1}{lxy}$
and $t \upharpoonright r =
\rnode{lxy}{\lambda x y} \cdot \rnode{y}{y} \cdot \rnode{vy}{v_y}
\cdot \rnode{vlxy}{v_{\lambda x y}}
\bkptr[offset=-4pt]{20}{vlxy}{lxy} \bkptr[offset=-4pt]{20}{vy}{y}
\bkptr[offset=-4pt]{20}{y}{vxy}$.
The sequence of moves $\varphi(t^*)$ is the play of the
interaction semantics represented on the diagram on the right and the sequence $\varphi(t\upharpoonright r)$
is a play of the standard semantics obtained by hiding the internal
moves (represented in bold on the diagram).



Despite the fact that \pcf\ computation trees are potentially infinite, 
lemma \ref{lem:safe_imp_incrbound} still holds:
