...
We assign an \defname{order} to each node as follows:
\begin{align*}
...
\left\{
  ...
  \end{array}
\right. \\
    \ord{\alpha:T} &= \ord{T}, \quad \hbox{where $\alpha$ is a variable or constant;} \\
    \ord{@} &= 0 \ .
\end{align*}
...
\label{def:traversal}
The set $\travset(M)$ of traversals over a computation tree $\tau(M)$ is defined by induction
...
A traversal mainly follows the structure of the tree. However the (Var) rule can jump across
the computation tree. The idea is that after visiting a variable node $x$, we can jump to the node corresponding to the subterm
that would be substituted for $x$ if all the $\beta$-redexes occurring in the term were reduced.
\begin{example}
...
\noindent {\bf case 2} If $n$ is labelled $\lambda \overline{\xi} = \lambda \xi_1 \ldots \xi_p$ with $p\geq 1$, $n$'s child node
is labelled $\alpha$ and $\vdash( q ) = \{ q^1, \ldots, q^m \} \union \{  v_q : v \in \mathcal{D} \} $ then
    $$
    f_A(n,q) =  \{ n \mapsto q \} \quad  \union \quad  \{ v_n \mapsto v_q \ | \ v \in \mathcal{D} \}
...
Note that $f_A(n,q)$ is a partial function from $V$ to $A$ since it is undefined on nodes 
that are hereditarily justified by $N_{fv} \union N_@ \union N_\Sigma$.
\end{definition}
...
For a closed term $\emptyset \vdash M : T$, the total function $\varphi : V_\lambda \union V_{var} \longrightarrow  \sem{T} \uplus \biguplus_{n \in N \inter E \relimg{N_@ \union N_\Sigma} } \sem{type(\kappa(n))}$ is defined as follows:
\begin{align*}
...
For an open term $x_1 : X_1, \ldots, x_n : X_n \vdash M : T$, the function $\varphi_{M}$ is defined as
...
The \defname{revealed strategy} of a term $\Gamma \vdash M$ is written $\intersem{\Gamma \vdash M}$. It can be obtained by uncovering\footnote{In Part II of \cite{hylandong_pcf} an algorithm is described that
uniquely recover the moves hidden during composition.} the
internal moves of $\sem{M}$ that are generated by the composition with the evaluation map $ev$ at each @-node of the computation tree.
...
\begin{example}
In order to distinguish the term $M_1 = \lambda f . f
(\lambda x . f (\lambda y .y ))$ from $M_2 = \lambda f . f (\lambda x
. f (\lambda y .x ))$ we have to preserve the pointers in their strategy denotation.
In the Safe $\lambda$-Calculus the ambiguity disappears since $M_1$ is
safe whereas $M_2$ is not (the ground type variable $x$ occurs free in $(\lambda y . x)$ of order $1$).
\end{example}
...
(ii) If $M$ is a \emph{closed} simply-typed term and $\tau(M)$ is incrementally-bound then the $\eta$-normal form of $M$ is safe.
\end{lemma}
\begin{proof}
...
Let $M$ be a closed simply-typed term without constants then
