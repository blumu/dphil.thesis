...
The $\eta$-expansion of $M: A\typear B$ is the term
$\lambda x . M x : A\typear B$ where $x$ is a fresh variable. Suppose that
$M$ is of type $(A_1,\ldots,A_n,o)$ then it can be $\eta$-expanded into
...
where the the \textsl{dummy} lambda $\lambda .$ in the right-hand
side of the equation  is deliberately kept as it plays an important role in the
...
A computation tree can be regarded as an abstract syntax tree
of the $\eta$-long normal form of a term. It provides a canonical representation
of a term.
...
$$ \tau( \alpha s_1 \ldots s_m) =
    ...
    {
        \tree{\alpha}
        { \TR{\tau(s_1)} \TR[linestyle=none]{\ldots} \TR{\tau(s_m)} }
    }
$$
...
$$ \tau((\lambda x.s_0) s_1 \ldots s_n) =
    ...
    {
        \tree{@}
        { \TR{\tau(\lambda x.s_0)}    \TR{\tau(s_1)} \TR[linestyle=none]{\ldots} \TR{\tau(s_n)} }
    }
$$
\end{itemize}
