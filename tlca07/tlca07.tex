...
such that $m \vdash n$.  We represent the pointer in the sequence as follows \Pstr[0.4cm]{ (m){m} \ldots (n-m,45) n \lnklabel*{i} }.
...
\pstr[0.4cm]{ \nd(n) n \ldots \lnk(vn-n,30){v_n}
\lnklabel*{v} }.  For the definition
...
\item[{\bf (App)}] If $t \cdot @$ is a traversal then so is \pstr{t \cdot \nd(m) @  \cdot \lnk(n-m,20) n \lnklabel{0}}.
...
The sequence 
\pstr[0.6cm]{\lambda \cdot
\nd(app) @  \cdot
\nd(ly) {\lambda y}  \ldots
\lnk(y-ly,35) y \lnklabel*{1} \cdot
\lnk(lx-app,40) {\lambda \overline{x}} \lnklabel*{1} \ldots
\lnk(x-lx,30) {x_i} \lnklabel*{i} \cdot
\lnk(leta-y,40) {\lambda \overline{\eta_i} }  \lnklabel*{i} \ldots}
