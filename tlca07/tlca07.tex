...
We propose a simpler definition for the safe $\lambda$-calculus which does not require the type homogeneity constraint originally used in 
\cite{KNU02} to define safety.

In general safety is not preserved by 
$\beta$-reduction so we define the notion of safe $\beta$-reduction, a multistep $\beta$-reduction which preserves safety.

It was shown by Schwichtenberg in \cite{citeulike:622637} that using the Church numerals encoding, the simply typed $\lambda$-calculus computes exactly the multivariate polynomials extended with the conditional operator. We show that the safe $\lambda$-calculus computes exactly the multivariate polynomials. Hence conditional is not definable in the safe $\lambda$-calculus.

Finally we give a game-semantic account of safety and we prove that game denotations of safe terms are \emph{P-incrementally-justified}. This implies that P-pointers are  superfluous i.e. there is an algorithm that, given a play of the denotation, recovers uniquely the pointers from the underlying sequence of moves and from the pointers associated to O-move. Consequently, pointers become entirely superfluous at order $3$ (but are still necessary for O-moves at order 4). 
\end{abstract}
...
Using that correspondence and after introducing the notion of \emph{P-incrementally-justified strategies}, we show that $\beta\eta$-normal terms are \emph{safe} if and only if their strategy denotation is P-incrementally-justified. A consequence of this is that P-moves' pointers can be recovered uniquely from the underlying sequences of moves and from O-moves' pointers.
...
Natural numbers can be encoded into the simply-typed $\lambda$-calculus using the Church Numerals: each $n\in\nat$ is encoded into the term $\encode{n} = \lambda s z. s^n z$ of type $I = ((o,o),o,o)$ where $o$ is a ground type.  In \cite{citeulike:622637}, Schwichtenberg showed that the numerical functions representable in the simply typed $\lambda$-calculus with this encoding are exactly the multivariate polynomials \emph{extended with the conditional function}:
\begin{theorem}[Schwichtenberg, 1979]
The functions representable by $\lambda$-terms of type $I\rightarrow \ldots \rightarrow I$ are exactly the multivariate polynomials extended with the conditional operator.
\end{theorem}
...
Putting propositions \ref{prop:incrbound_imp_incrjustified} and \ref{prop:safe_imp_incrbound} together gives:
\begin{corollary}[Safety and incremental-justification]
\label{cor:safeincrejust}
...
Putting Corollary \ref{cor:safeincrejust}(i)
and Lemma \ref{lem:incrjustified_pointers_uniqu_recover} together
we get:
\begin{theorem}
\label{thm:safe_ptr_recoverable} In the game semantics of
...
Moreover since \pcf\ traversal rules are \emph{well-behaved}, lemma \ref{lem:redtrav_trav} remains true in the presence of
constants and therefore the game-semantic analysis of safety remains valid for \pcf:
\begin{theorem}
In the game semantics of safe PCF, P-moves' pointers are superfluous.
\end{theorem}
...
\section*{Appendix : Proof of Proposition \ref{prop:safe_imp_incrbound}}
