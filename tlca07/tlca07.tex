...
\item \emph{Trees}. Knaptik \emph{et al.} have shown that the Monadic
  ...
  generating trees: A tree is generated by a \emph{safe} order-$n$
  grammar if and only if it is generated by a level-$n$ pushdown
  automaton.
...
that occur free in a safe lambda term have orders no smaller than that
of the term itself.  We can now explain the sense in which the safe
lambda calculus is safe by establishing its salient property: No
...
\union \mathcal{N} \union \{z_1, \ldots, z_m \}$. We say that the
grammar is \emph{order-$n$} just in case the order of the
...
\parpic[r]{\small$\xymatrix@R-.8cm@C-.5cm{& g \ar@{-}[dl]
\ar@{-}[dr] & & \\
a & & g \ar@{-}[dl] \ar@{-}[dr] & \\
& a & & h \ar@{-}[d] \\
& & & h \ar@{-}[d]\\
...
grammar) is \emph{homogeneous} just if all symbols that occur in it
...
\begin{definition}[Safe grammar]\rm
  ...
  such that
\begin{enumerate}[(i)]
\item $t$ occurs in an {\em operand} (or second) position of the
  implicit application operator i.e.~$e$ has the form $\cdots (s \, t)
  \cdots $ for some $s$
...
have order no smaller than that of the term being formed.

We do not impose any constraint on types. In particular,
type-homogeneity as used originally to define safe grammars
\cite{KNU02} is not required here. Another difference is that we allow
$\Xi$-constants to have any higher-order type.  Thus our formulation
...
\begin{proposition} Let $G = \langle \Sigma, \mathcal{N}, \mathcal{R},
  ...
  from the symbols in $\mathcal{N} \cup \Sigma \cup \makeset{z_1,
    \cdots, z_n}$.  A rule $F z_1 \ldots z_m \rightarrow e$ in
  $\mathcal{R}$ is safe if and only if $ z_1 \ldots z_m \vdash_{\Sigma
    \cup \mathcal{N}} e : o$ is a valid typing judgement of the
  \emph{homogeneous} safe lambda calculus.
\end{proposition}
...
\ref{prop:incrbound_imp_incrjustified} allow us to show the
...
order $3$ for safe terms. Indeed, up to level $2$ of an order $3$
arena, the pointers associated to question moves are recoverable by
visibility.  At the last level, the question moves are P-moves
