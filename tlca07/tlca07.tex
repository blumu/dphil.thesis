...
\[\begin{array}{rll}
G^{(o, o)} \, x & \quad \rightarrow \quad & H \, \underline{(f \, {\red x})} \\
...
lower-bound of $S$. For convenience, we shall omit the subscript from
$\vdash_\Xi$ when the generator-set $\Xi$ is clear from the context.

\noindent \item The sub-system that is defined by the same rules
except that all types are required to be homogeneous is called
\defname{homogeneous safe $\lambda$-calculus}.
\end{inparaenum}
\end{definition}
