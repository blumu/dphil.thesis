...
We assign types to nodes as follows: The root node has type $(A_1,\ldots,A_p, T)$ where $A_1,\ldots, A_p$ are the types of the variables in the context $\Gamma$, 
a lambda node different from the root has the same type as
the term whose computation tree is the subtree rooted at that node,
the type of a variable-node is the type of the variable labelling it and @-nodes have ground type $o$. The \defname{order} of a node $n$, written $\ord{n}$, is defined to be the order of its type.

We say that a variable node $n$ labelled $x$ is bound by a node $m$, and $m$ is called the \defname{binder} of $n$, if $m$ is the closest node in the path from $n$ to the root such that $m$ is labelled
