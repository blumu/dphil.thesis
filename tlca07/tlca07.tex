...
\pstr[0.3cm]{ \lnk(n) n \ldots \lnk(vn-n,35){v_n}
...
\item[{\bf (App)}] If $t \cdot @$ is a traversal then so is \Pstr[0.4cm]{t \cdot (m) @  \cdot (n-m,40) n \lnklabel{0}}.
...
\Pstr[0.4cm]{t_1 \cdot (x){x} \cdot t_2 \cdot (xv-x,38){v_x} \lnklabel*{v}}
...
\item[{\bf (InputVar)}] If $t_1 \cdot x \cdot t_2$ is a traversal with
  ...
  t_2)=?(t_1) \cdot x$ then so is \Pstr[0.4cm]{t_1 \cdot (m){x} \cdot t_2 \cdot (n-m,38){n}
\lnklabel*{i}} for all
...
\item[{\bf (Copycat$^@$)}]
  If \Pstr{t \cdot (app){@} \cdot (lz-app,60){\lambda \overline{z}} \lnklabel{0} \ldots  (lzv-lz,60){v}_{\lambda \overline{z}} \lnklabel*{v}} is a traversal then so is
\Pstr[0.6cm]{t \cdot (app){@} \cdot (lz-app,60){\lambda \overline{z}} \ldots  (lzv-lz,60){v}_{\lambda \overline{z}} \lnklabel*{v} \cdot (appv-app,45){v}_@ \lnklabel*{v}}.

\item[{\bf (Copycat$^\lambda$)}] If \Pstr[0.4cm]{t \cdot \lambda \overline{\xi} \cdot (x){x}  \ldots   (xv-x,80){v}_x \lnklabel*{v}}
is a traversal then so is
\Pstr[0.5cm]{t \cdot (lmd){\lambda \overline{\xi}} \cdot (x){x}  \ldots  (xv-x,50){v}_x \lnklabel*{v} \cdot (lmdv-lmd,30){v}_{\lambda \overline{\xi}} \lnklabel*{v}}.

\item[{\bf (Copycat$^{var}$)}] If \Pstr[0.4cm]{t \cdot y \cdot (lmd){\lambda \overline{\xi}}
\ldots (lmdv-lmd,50){v}_{\lambda \overline{\xi}} \lnklabel*{v}} is a traversal for some variable $y$ not in $N_{var}^{\upharpoonright r}$ then so is
\Pstr[0.6cm]{t \cdot 
(y){y} \cdot
(lmd){\lambda \overline{\xi}} \ldots
(lmdv-lmd,30){v}_{\lambda \overline{\xi}} \lnklabel*{v} \cdot
(vy-y,50){v}_y \lnklabel*{v}}.

\item[{\bf (Var)}]
If \Pstr[0.5cm]{t' \cdot (n){n} \cdot
    (lx){\lambda \overline{x}}  \ldots
    (x-lx,50){x_i} \lnklabel*{i}} is a traversal for some variable $x_i$ not in $N_{var}^{\upharpoonright r}$ then
so is
\Pstr[0.6cm]{
t' \cdot (n){n} \cdot
    (lx){\lambda \overline{x}}  \ldots
    (x-lx,30){x_i} \lnklabel*{i} \cdot
    (letai-n,40){\lambda \overline{\eta_i}}
	\lnklabel*{i} }.
\end{description}
\end{definition}
...
The sequence 
\pstr[0.7cm]{\lambda \cdot
\lnk(app) @  \cdot
\lnk(ly) {\lambda y}  \ldots
\lnk(y-ly,35) y \lnklabel*{1} \cdot
\lnk(lx-app,38) {\lambda \overline{x}} \lnklabel*{1} \ldots
...
form ``\Pstr{t = t_1\cdot n \cdot t_2 \in \travset \zand ?(t) = ?(t_1)
\cdot x \zand n \in N_{\Sigma}\union N_{var} \zand P(t) \zand m\in
S(t) \imp t_1\cdot (n){n} \cdot t_2 \cdot (m-n,25){m} \in \travset}'' for some expression $P$
