...
By remarking that pointers are unnecessary up to order $2$,
it was shown in \cite{ghicamccusker00} that observational equivalence is decidable (and in PSPACE) for the order $2$ finitary fragment of Idealized Algol ($\ialgol_2$ for short). 
At order $3$, although pointers are necessary, it was shown
that observational equivalence is in EXPTIME \cite{DBLP:conf/fossacs/MurawskiW05}. 
...
(note that it is unlikely to obtain the PSPACE complexity since the complete plays of the safe term $\lambda f^{(o,o),o} . f (\lambda x^o . x)$ is not regular, see \cite{DBLP:journals/apal/Ong04}).

Finitary $\ialgol_i$ was shown to be undecidable for $i\geq4$ \cite{murawski03program}.
It would also be interesting to know whether this remains true for the corresponding safe fragments.
...
Serially Re-entrant Idealized Algol proposed by Abramsky  in \cite{abramsky:mchecking_ia} is an example of PUR language: it allows multiple occurrences or uses of arguments, as long as they do not overlap in time. Therefore in the game denotation, a given question occurs pending at most once at any time hence each move has a unique justifier. Another way to generate PUR-languages consists in constraining types. For instance the finitary fragment of order $2$ Idealized Algol ($\ialgol_2$ for short) is a PUR-language (for which observational equivalence is decidable \cite{ghicamccusker00}). There are strong suggestions that Safe $\ialgol_3$ is also a PUR language.
