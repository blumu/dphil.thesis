...
\begin{lemma}[Safety is preserved by $\eta$-reduction]
...
using the Church Numerals: each constant $n\in\nat$ is encoded into the term $\encode{n} = \lambda s z. s^n z$ of type $I = ((o,o),o,o)$ where $o$ is a ground type.  In \cite{citeulike:622637}, Schwichtenberg showed that with this encoding, the numerical functions representable in the simply typed $\lambda$-calculus are exactly the multivariate polynomials \emph{extended with the conditional function}:
\begin{theorem}[Schwichtenberg, 1979]
The functions representable by $\lambda$-terms of type $I\rightarrow I \rightarrow \ldots \rightarrow I$ are exactly the multivariate polynomials extended with the conditional operator.
\end{theorem}

If we limit ourselves to safe terms, the representable functions are exactly the multivariate polynomials:
\begin{theorem}
\label{thm:polychar}
The functions representable by safe $\lambda$-expressions of type $I\rightarrow I \rightarrow \ldots \rightarrow I$ are exactly the multivariate polynomials.
\end{theorem}
