...
\begin{dfn}[Safe higher-order grammars]
...
\begin{prop} Let $G = \langle \Sigma, \mathcal{N}, V, \mathcal{R}, S \rangle$ be a grammar.


A production rule $F z_1 \ldots z_m \rightarrow e$ in $\mathcal{R}$ is safe if and only if
...
\begin{dfn}[P-view]
The P-view of a justified sequence of nodes $t$ of $\tau$, written $\pview{t}$, is defined as follows:
\begin{align*}
...
Remark: $t-@$ is not necessarily a proper justified sequence of nodes.
...
\begin{rem} Let us give the intuition of the proof.
...
straightforwardly to Safe \pcf. In particular, it can be shown
...
game semantics of safe $\lambda$-terms to safe \pcf-terms. In
this section we lay out the key elements permitting to prove that
the pointers in the game semantics of safe \pcf\ terms can be recovered
uniquely.

\subsection{Formation rules of Safe \pcf}
We call safe \pcf\ term any term that is typable within the
...
type $A$. We believe that what we have proved for Safe \textsf{PCF}
can also be proved for full Safe \textsf{IA}.
