...
``internal nodes'' of the computation. This leads to a
...
traversals. This is guaranteed by the fact that the P-view of the
...
\emph{Related works}: Traversals of a computation tree provides a
way to perform \emph{local computation} of $\beta$-reductions as
opposed to a global approach where the $\beta$-reduction is
implemented by performing substitutions. A notion of local
computation of $\beta$-reduction has been investigated in
\cite{DanosRegnier-Localandasynchronou} through the use of special
graphs called ``virtual nets'' that embeds the lambda-calculus.
...
in the literature as a way to implement graph-based reduction of
lambda-expressions. We can regard traversals as an alternative
version of paths which is adapted to the graph representation of
