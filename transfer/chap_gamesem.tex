...
\item $\lambda : M \rightarrow \{ O, P\} \times \{Q, A\}$ is a labelling functions indicating whether a given move
...
For the sake of convenience we write the set $\{O,P\} \times \{Q,A\}$ as $\{OQ,OA,PQ,PA\}$.
$\overline{\lambda}$ denotes the labelling function $\lambda$ with the question and answer swapped. For instance:
...
\vspace{18pt}
\emph{Notation:} we write $s t$ or sometimes $s \cdot t$ to denote the
...
D & O & $q_0 \cdot L^* \cdot q$      & $q_0 \vdash q \quad \wedge\quad  \ord{p} = 0$ \\ % $i \in I_0$
\end{tabular}
\end{center}
...
$\mathcal{C}_{ib}/\lesssim_{ib}$ is a posset-enriched cartesian closed category. The game semantics of the basic types and constants of \pcf\ can be transposed from $\mathcal{C}_{ib}$ to $\mathcal{C}_{ib}/\lesssim_{ib}$. Unfortunately it is not know whether $\mathcal{C}_{ib}/\lesssim_{ib}$
