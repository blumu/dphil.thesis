...
been used as tool to study formal models of programming languages.
The idea is to model the execution of a program as a game played by
two protagonists: the Opponent representing the environment and the
Proponent representing the system. The meaning of the program being
modeled by a strategy for the Proponent.


These game-based model were used to give a solution to the
long-standing problem of ``Full abstraction of PCF''
\citep{abramsky94full, hylandong_pcf,Nickau:lfcs94}.

Based on that major result, in a more applied direction, games have
been used as a new tool for software verification
...
\subsubsection{Call-by-Value first-order Idealized Algol}

Game semantics for call-by-value programming Language.



\section{Applications of game semantics}

\subsection{Algorithmic game semantics}

\subsubsection{First-order and second-order Idealized Algol}
...
representable by extended regular languages, hence program
approximation and equivalence is decidable (see work of Dan R. Ghica
and Guy McCusker).
