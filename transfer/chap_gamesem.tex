...
We call \emph{pending question} a question in a sequence that has not been answered.
A strategy $\sigma$ is well-bracketed if for every play $s \cdot m \in \sigma$:
\begin{itemize}
\item if $m$ is an answer then it points to the last pending question in $s$;
\item if $m$ is a question then it points to a pending question in $s$.
\end{itemize}

The second condition is usually omitted in the literature of game semantics. It is harmless to add it
since the well-bracketed strategies involved in the game model of PCF satisfy this condition. Moreover it will
be a necessary condition for some property that we are going to prove.
...
subsequence of $s$ obtain by keeping only the pending questions in $s$.

\begin{rem}
By definition of well-bracketing, for any well-bracketed sequence $s\cdot m$, the move $m$ points to a question in $?(s)$.
Moreover if $s \cdot m$ satisfies visibility and $m$ is a P-move (resp. O-move) then $m$ points in $?(\pview{m})$ (resp. $?(\oview{m})$).
\end{rem}

\begin{lem}
Let $s \cdot m$ be a legal well-bracketed sequence of moves.
\begin{enumerate}
\item If $m$ is not a P-answer then $?(\pview{s \cdot m}) = \pview{?(s \cdot m)}$;
\item if $m$ is not an O-answer then $?(\oview{s \cdot m}) = \oview{?(s \cdot m)}$.
\end{enumerate}
\end{lem}
\begin{proof}
By induction on the length of $s$. The base case is trivial. Suppose $s = s' \cdot q \cdot s'' \cdot q_2$

\end{proof}

The previous remark and lemma gives the following corollary:
\begin{cor}
\label{cor:pendingview}
Let $s \cdot m$ be a legal well-bracketed sequence of moves \emph{satisfying visibility}.
\begin{enumerate}
\item If $m$ is a P-move then it points in $?(\pview{s}) = \pview{?(s)}$;
\item if $m$ is an O-move then it points in $?(\oview{s}) = \oview{?(s)}$.
\end{enumerate}

\end{cor}


The height of an arena $\langle M, \lambda, \vdash \rangle$ is the length of the longest sequence of moves
...
On the diagram below we represent an arena $A_i$ where $i \in I_0$ on the left and an arena $A_j$ where $j \in I_1$ on the right:
\begin{center}
\
  \pstree[levelsep=6ex]
    {\TR{$q^i$}}
...
Let $s$ be a legal and well-bracketed \emph{justified} sequence of moves of the game $A$.
...
If $s = u \cdot m$ with $m \in M_A$
for some non empty legal well-bracketed justified sequence $u$.
By the induction hypothesis, $u$ corresponds to one of the cases 0,A,B,C or D.
...
\item[case D] $?(u) \in q \cdot L^* \cdot q^i$ where $i \in I_0$ and the move $m$ is played by $O$.
    ...
    $q^i$ since this is the only pending occurrence of its enabler.
