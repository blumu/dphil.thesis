...
\begin{proof}
Consider an arena of order 0. Such arena has the following flat structure:
\begin{center}
  \pstree[levelsep=6ex]
    {\TR[name=R]{q}}
...
When representing graphically the arena, we display the sub-arena by decreasing order of sub-arena order.
It is safe to do so since in the definition of the forest of tree of an arena, the children nodes
are not ordered.

Let $A$ be an arena of order 2. We assume that $A$ has only one root (initial question). We will
see that the proof can easily be generalized to arenas that are forest of trees.
The arena $A$ has therefore the following shape:
\begin{center}
\
  \pstree[levelsep=6ex]
    { \TR{$q$} }
...
where each triangle represents an arena of order 0 or 1.



%$T = T_1 \rightarrow T_2 \rightarrow \ldots T_n \rightarrow \typexp$.
%

Let $I_k = \{ i \in 1.. n\ |\ \order{T_i} = k \}$ for $k = 0..1$. The arenas $T_i$ for $i \in I_0$ and $T_j$ for $j \in I_1$ are given
below:
\begin{center}
\
  \pstree[levelsep=6ex]
