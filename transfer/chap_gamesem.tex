...
initial in A or B. And $m \vdash_{A\otimes B} n$  if and only if $m
\vdash_{A} n$ or $m \vdash_{B} n$.

The game $A \otimes B$ is defines as follows:
\begin{eqnarray*}
...
(i.e. there is a function $f$ from the set of odd length position to the set of moves $M$
 such that $f(s a) =b$). The first condition ensures that the strategy $\sigma$ only considers
positions that the strategy itself could have led to in a previous
move.
