...
Safety has been defined as a syntactical constraint. Interestingly,
this syntactic constraint has

In this chapter, we make use of the correspondence theorem in order
to give a game-semantic account of safety.

We start by introducing the notion of \emph{incrementally-justified
strategy} and prove that safe terms in $\beta$-normal form are
exactly the simply-typed terms denoted by such strategies. A nice
property of incrementally-justified strategy is that pointers can be
reconstructed uniquely from the sequences of moves.

We prove that pointers in the game semantics of safe terms can be
uniquely recovered. We establish a characterisation of the game
semantics of safe terms :


The first section of this chapter is concerned only with the pure
Safe $\lambda$-Calculus without interpreted constants.

In the next section we extend the result by taking into account the
interpreted constants of \pcf and \ialgol. We define the language
Safe \ialgol\ (resp. Safe \pcf) to be the fragment of \ialgol\
(resp. \pcf) where the application and abstraction rules are
constrained the same way as in the Safe $\lambda$-Calculus. We show
that Safe \pcf\ terms are denoted by incrementally-justified
strategies and we give the key elements for a possible extension of
the result to Safe Idealized Algol.
...
\upharpoonright r) \jseq s$. We assume that the last node $n$ of $t$
is hereditarily justified by $r$ (otherwise we replace $t$ by its
...
gives: $ \pview{?(t \upharpoonright r)} \jseq \pview{?(t)}
...
Pointers in the game semantics of safe terms are uniquely
recoverable.
\end{thm}
\begin{proof}
...
We need to complete the definition of the interaction semantics of
section \ref{sec:interaction_semantics} to take into account the
...
v_{\iaseq} \in \travset } \bkptr{20}{m-}{seq-} \bklabelc{2}
$$
...
\bkptr{30}{n}{d} \bklabelc{1} \hspace{1cm} (\mbox{mkvar}_r')
