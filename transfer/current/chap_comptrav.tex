...
\label{def:nodeorder}
Suppose $\Gamma \vdash M : A$. The node-order function $\textsf{ord}$ is defined on nodes of $\tau(M)$ as follows:
\begin{eqnarray*}
\ord{r} &=& \ord{\Gamma \rightarrow A} \\
\ord{\alpha:T} &=& \ord{T}, \mbox{ where $\alpha$ is a variable or constant} \\
\ord{n} &=& \ord{T}, \hbox{ where 
$\kappa(n) : T$ for $n \in (N_\lambda \union N_@) \setminus \{r \}$\ .}
\end{eqnarray*}
\end{dfn}
...
\end{align*}
where $q^0_A$ denotes the only initial question of the arena $A$ (arenas involved in the game semantics of pure simply-typed $\lambda$-calculus have a single  root).
...
Moreover if $M$ is in $\beta$-normal form, for any traversal $t$, if $\varphi_M(t)$ is a \emph{maximal} play then $t$ is a \emph{maximal} traversal.
\end{thm}
