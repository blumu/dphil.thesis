...
\begin{dfn}[P-incrementally-justified strategy]
A strategy $\sigma : A$ is said to be \emph{P-incrementally-justified}
if for any sequence of moves $s q \in P_A$ we have:
\begin{eqnarray*}
...
\begin{enumerate}
\item[(i)] If a term in $\beta$-normal form has an incrementally-bound computation tree then it is denoted by a P-incrementally-justified strategy.
\item[(ii)] In the $\lambda$-calculus without interpreted constants, conversely, if a term is denoted by a P-incrementally-justified strategy then
the computation tree of its $\beta$-normal form is incrementally-bound.
\end{enumerate}
\end{prop}
...
\begin{thm}[P-pointers are superfluous for safe \pcf]
Pointers emanating from P-moves in the game denotation of safe PCF terms are uniquely recoverable.
\end{thm}
\begin{proof}
...
by P-incrementally-justified strategies.
...
Hence P-pointers are superfluous in the game denotation of safe \pcf\
terms.
\end{proof}
