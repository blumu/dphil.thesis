...
Let $T$ be a PCF type of order at most 2. $T = T_1 \rightarrow T_2 \rightarrow \ldots T_n \rightarrow \typexp$.
Let $I_k = \{ 1.. n\ |\ \order{T_i} = k \}$ for $k = 0..1$.

The arenas $T_i$ for $i \in I_0$ and $T_j$ for $j \in I_1$ are given below:
\begin{center}
\
  \pstree[levelsep=6ex]
    {\TR{$q_i$}}
...
We recall the definition of the safe $\lambda$-calculus given in \cite{Ong2005}.
