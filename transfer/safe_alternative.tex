...
From the second assertion of the previous lemma we deduce the following property:
\begin{property}[Context reduction]
If $\Gamma \vdash M : B$ then $fv(M) \vdash M : B$.

where $fv(M) \subseteq \Gamma$ denotes the context constituted of the variables
in the $\Gamma$ occurring free in $M$.
\end{property}
...
$\lambda \overline{y}. P$. Thus, lemma \ref{lem:safe_basic_prop} (ii) gives:
