...
when performing the following capture permitting simultaneous substitution on safe terms
$\Gamma,\overline{x} \vdash M$, $\Gamma \vdash  N_1, \ldots \Gamma \vdash  N_n$:
...
provided that the substitution is performed simultaneously on
\emph{all} free variables of the same order in $M$.
\end{lem}
\begin{proof}
...
We now prove that reducing a safe redex $(\lambda \overline{x} . M) N_1 \ldots N_l$ produces a safe term.
...
\todobox{TO FINISH: Unfortunately, we cannot use directly lemma \ref{lem:subst_preserve_safety} because the substitution on $T$
is performed on the variables $x_1, \ldots x_l$ and not on all the free variables of order $\ord{x_1}$.}

%However, since the variable $x_1, \ldots x_n$ are abstracted together at once, we have:
%$$\Gamma \vdash \lambda \overline{x} . T : (\overline{B_1}|\ldots|\overline{B_m}|o)$$
%where $\overline{x}$ constitutes the lowest partition of the context $\Gamma, \overline{x}$. i.e.
%$\forall z \in \Gamma : \ord{z} \geq \ord{\overline{x}}$.
%
%Therefore for all $j \in (l+1)\ldots n$, we can derive using the rules \textbf{(var)} and \textbf{(wk)} the sequent
%  $\Gamma \vdash x_j$.
%

%We now perform a capture permitting substitution of the safe terms
%$\Gamma \vdash N_1$, \ldots, $\Gamma \vdash N_l$, $\Gamma \vdash x_{l+1}$, \ldots, $\Gamma \vdash x_n$ for the variables
%$x_1 \ldots x_n$ in the safe term $\Gamma \vdash T$, and we obtain by lemma \ref{lem:subst_preserve_safety}:
%$$\Gamma \vdash T [N_1/x_1 \ldots N_l/x_l \ x_{l+1}/x_{l+1} \ldots x_n/x_n] = T[N_1/x_1 \ldots N_l/x_l]$$

%Finally, by applying the rule $\mathbf{(abs)}$ we prove the safety of $t$.
