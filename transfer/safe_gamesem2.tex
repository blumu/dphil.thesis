...
By property \ref{proper:phi_pview}, the P-view of $\tilde{s}$ and the P-view of $r$ are computed
similarly and have the same pointers. This means that
node $n$ and  move $q$ point to the same position in
the justified sequence $\pview{r}$ and $\pview{\tilde{s}}$ respectively. Therefore, to conclude, it suffices to retrieve
the position in $\pview{r}$ where the node $n$ points to.


By the induction hypothesis, the pointers of $s$ can been recovered up to the last move $q$,
therefore the P-view $\pview{\tilde{s}}$ can be computed.
...
The regularity of $\tau(M)$ (property \ref{proper:regularity}) ensures that $n$ points to the
...
labelled $\lambda$ and therefore no node points to them. Hence
