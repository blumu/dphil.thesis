...
\begin{dfn}[Safely-justified strategy]
A strategy $\sigma : A$ is said to be \emph{safely-justified} if for any sequence of moves $s q \in P_A$
...
\begin{lem}
\label{lem:safejustified_pointers_uniqu_recover}
Pointers are superfluous for safely-justified strategies.
\end{lem}
\begin{proof}
Suppose $\sigma$ is a safely-justified strategy. We prove that pointers in a play $s\in \sigma$ are uniquely recoverable by induction on the length of $s$.
...
\begin{exmp}
The denotation of the evaluation map $ev$ is not safely-justified. Indeed consider the play $s = q_0 q_1 q_2 q_3 \in \sem{ev}$
...
The last O-move in $?(\pview{s})= s$ with order strictly greater than $\ord{q_3}$ is $q_1$.
 However $q_3$ points to $q_0$ therefore $\sem{ev}$ is not safely-justified.
\end{exmp}
...
\begin{dfn}[Safely-bound computation tree]
A variable node $x$ of a computation tree is said to be \emph{safe} if either:
\begin{enumerate}
...
A computation tree is said to be \emph{safely-bound} if all the variable nodes are safe.
\end{dfn}

\begin{prop}[Safely-bound trees coincide with safely-justified strategies] \
\label{prop:safebound_imp_safejustified}
\begin{enumerate}
\item[(i)] If a term in $\beta$-normal form has a safely-bound computation tree then it is denoted by a safely-justified strategy.
\item[(ii)] in the pure $\lambda$-calculus ($\Sigma=\emptyset$), reciprocally if a term is denoted by a safely-justified strategy then
the computation tree of its $\beta$-normal is safely-bound.
\end{enumerate}
\end{prop}
...
(i) Suppose that $\tau(M)$ is safely-bound.
...
$\tau(M)$ is safely-bound, $m$ is the last $\lambda$-node in $[r,n]$ of order strictly greater than $\ord{n}$.
...
Since the computation tree is safely-bound, all the $\lambda$-nodes in $]r,n]$ have order smaller than $\ord{n}$. Therefore by the correspondence theorem,
...
(ii) Suppose that the strategy $\sem{M}$ is safely-justified.
...
Since $\sem{M}$ is safely-justified, $\varphi(x)$ must point to the last move in $\pview{?(\varphi(t \upharpoonright r))}$ with
...
Since node $x$ of order $0$ is not bound by the order 1 node $\lambda y$, $\tau(M)$ is not safely-bound and by proposition
\ref{prop:safebound_imp_safejustified} $\sem{\lambda x . f (\lambda y .x)}$ is not safely-justified.
Similarly we can check that the denotation of $f (\lambda y .x)$ is not safely-justified whereas $\lambda y. x$ has a safely-justified denotation.
Also for any higher-order variable $x:A$, the computation tree $\tau(x)$ is safely-bound, therefore
the projection strategies $\pi_i$ are safely-justified.
From these examples we observe that application does not preserve safe-justification: $\sem{f}$ and $\sem{\lambda y. x}$ are safely-justified whereas
...
\begin{lem}[Safe terms have safely bound computation trees]
\label{lem:safe_imp_safebound}
Let $\Gamma \vdash M$ be a simply-typed term.
\begin{itemize}
\item[(i)] If $M$ is a safe term then $\tau(M)$ is safely bound ;
\item[(ii)] reciprocally, if $M$ is \emph{closed} and $\tau(M)$ is safely-bound then the $\eta$-normal form of $M$ is safe.
\end{itemize}
\end{lem}
\begin{proof}
...
Hence $\tau$ is safely-bound.

(ii) We assume that $M$ is already in $\eta$-normal form. Suppose $M$ is closed and $\tau(M)$ is safely-bound,
...
If it is bound by $\lambda \overline{\xi}$ then since $\tau(M)$ is safely-bound we have $\ord{z} \geq \ord{\lambda \overline{\eta_1}} = \ord{N_i}$.
...
$\lambda x y .x$ and $\lambda y . x$ where $x,y:o$ have (isomorphic) safely-bound computation trees. However $\lambda x y .x$ is safe
whereas $\lambda y . x$ is not.



Putting proposition \ref{prop:safebound_imp_safejustified} and lemma \ref{lem:safe_imp_safebound} together we
obtain a game semantics characterization of safe terms:
\begin{cor}[Safely-justified strategies characterize closed safe terms]
Let $M$ be a closed pure simply-typed term (with no constants) then:
$$ \sem{M} \mbox{ is safely-justified if and only if $\etabetanf{M}$ is safe,} $$
...
By lemma \ref{lem:safe_imp_safebound} (i), $\tau(M')$ is safely-bound and
by proposition \ref{prop:safebound_imp_safejustified},
$\sem{M'}$ is a safely-justified strategy.
By lemma \ref{lem:safejustified_pointers_uniqu_recover}, the pointers in $\sem{M'}$ are uniquely recoverable.
...
Despite this particularity, lemma \ref{lem:safe_imp_safebound} still holds in the \pcf\ setting:
\begin{lem}[Safe terms have safely-bound computation tree]
\label{lem:pcf_safe_imp_safebound}
If $\Gamma \vdash M$ is a safe \pcf\ term then $\tau(M)$ is safely-bound.
\end{lem}
\begin{proof}
...
Clearly, lemma \ref{lem:safe_imp_safebound} is still valid in $\pcf_1$ (the subterms of the form $\Omega$ are just
represented by the constant $\bot$ in the computation tree).
Therefore since $M_k$ is in $\pcf_1$, $\tau(M_k)$ must be safely-bound.
...
proposition \ref{prop:safebound_imp_safejustified}(i) also holds: terms in $\beta$-nf
with a safely-bound computation tree are denoted by safely-justified strategies.

Safety is preserved by reduction (lemma \ref{lem:ia_safety_preserved}), therefore
by lemma \ref{lem:pcf_safe_imp_safebound} and soundness of the game denotation, any Safe \pcf\ term must be
denoted by a safely-justified strategy.
...
\item prove that computation tree of safe term are safely-bound;
\item prove that safely-bound computation trees are denoted by safely-justified strategies.
\end{enumerate}
...
check that computation DAG of Safe \ialgol\ terms are safely-bound;
consequently by lemma \ref{lem:safejustified_pointers_uniqu_recover}, pointers in the strategy are superfluous for Safe IA terms.
}
...
An example of such language is the Serially Re-entrant Idealized Algol, or SRIA for short, proposed
...
SRIA and Safe \ialgol\ are therefore two different examples of languages with pointer-less game semantics. It
would be interesting to compare the computational power of both languages.
