...
%      $M'$ is an order 0 constant: same treatment as the (constant) case above.
  ...
  \item (abstraction of a variable): $M \equiv \lambda \overline{\xi} . x$
    ...
    Let $q_o$ be the initial question of the arena $\sem{M}$.
...
of $\intersem{N_0}$ but not to \sem{N_0}, otherwise the evaluation. Therefore the
las mnode
preceding

It is indeed a traversal of $\tau(M)$ since the last node in $t$

we can therefore just copy
...
\end{enumerate}
    ...
    For the converser, we proceed by induction on the structure of the traversal and case analysis on the rule used:
    $s =\varphi_M(t-@)$
