...
\begin{eqnarray}
 \sem{\Gamma \vdash M : T} = \intersem{\Gamma \vdash M : T} \upharpoonright root(\Gamma),
 root(T) \label{eqn:int_std_gamsem}
\end{eqnarray}
...
$$ \varphi_M(\travset^{-@}) \upharpoonright root(\Gamma), root(T) = \varphi_M(\travset^{\upharpoonright r}) $$
In other words, for any traversal $t$ of the computation tree of $\Gamma \vdash M :T$ we have:
$$\varphi_M(t-@) \upharpoonright root(\Gamma), root(T) = \varphi_M(t\upharpoonright r)$$
\end{lem}
\begin{proof}
...
\begin{align*}
\sem{M} &= \intersem{M} \upharpoonright root(\Gamma), root(T) & \mbox{(eq. \ref{eqn:int_std_gamsem})} \\
        &= \varphi_M(\travset^{-@}(M)) \upharpoonright root(\Gamma), root(T) & \mbox{(by (i))}\\
