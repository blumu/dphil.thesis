...
\begin{property}[Traversal of $\beta$-normal forms]
...
If $n$ is labelled with a free variable $x$ then $n$ points to $r$, the first node of the traversal.
Therefore by definition of $\varphi$, $\varphi(n) = x$ is a child node of some root $q_0 = \varphi(r)$ of the arena $\sem{\Gamma \rightarrow A}$
and $\ord{x} = \ord{q_0} - 1$. The game $\sem{\Gamma \rightarrow A}$ is well-opened therefore $x$ is the only move in $s$ that
has order strictly greater than $\ord{x}$.


If $n$ is labelled with a variable $x$ bound by a node $m$ occuring in $t$ then
since $t \upharpoonright r$ is a reduced-traversal of $\tau(M)$ and $M$ is in $\beta$-normal form,
