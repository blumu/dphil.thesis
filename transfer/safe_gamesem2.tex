...
Suppose $t = t_1 \cdot p \cdot t_2$ is a traversal where $p$ is the pending $\lambda$-node in $t$ then:
\begin{itemize}
...
%\item the subsequence obtained from a traversal by keeping only unmatched nodes is such that
%        $\lambda$-nodes and non $\lambda$-nodes alternate;
%\item if the last two unmatched nodes are $y$ and $\lambda \overline{\xi}$ in that order, for some non-input variable node $y$ then necessary
...
\subsection{Property of traversals}
...
The cartesian closed category of games $\mathcal{C}$ defined in the first chapter is used as a
