...
\begin{property}[Traversal of closed terms in $\beta$-normal form]
\label{proper:betaeta_trav}
Suppose $M$ is a closed term in $\beta$-normal form then
...
Here the term $M$ is in $\beta$-normal form therefore there is no node of type $@$ in the computation tree.
...
\label{lem:redtrav_trav} Suppose $M$ is a closed term in $\beta$-normal form and
let $r$ denotes the the root of $\tau(M)$.
\begin{itemize}
...
This definition implies that $\varphi$ regarded as a function from sequences of nodes without
pointers to sequences of moves without pointers is a monoid homomorphism:
\begin{lem}
\label{lem:varphi_monoidhomom}
$\varphi : (N\setminus\{@\})^* \rightarrow \mathcal{M}^*$ is a monoid homomorphism.
\end{lem}
...
%$\tilde{s}$ is also a justified sequence of move (there is no dangling pointer since questions-moves points to question-moves).
...
\begin{eqnarray*}
s q \in \sigma \wedge |s| \mbox{ even } \implies q \mbox{ points to the last P-move in $\oview{?(s)}$ with order greater or equal to $\ord{q}$} \\
s q \in \sigma \wedge |s| \mbox{ odd } \implies q \mbox{ points to the last O-move in $\pview{?(s)}$ with order greater or equal to $\ord{q}$}
\end{eqnarray*}
\end{dfn}
...
$m$ points to the last O-move in $\pview{?(s)}$ with order greater or equal to $\ord{q}$.
Similarly, if $m$ is an O-move then $|s|$ is even and by regularity
$m$ points to the last P-move in $\oview{?(s)}$ with order greater or equal to $\ord{q}$.
...
A simply-type term in $\beta$-normal form whose computation tree is regular is denoted by a regular strategy.
\end{prop}

\begin{proof}
Let $\Gamma \vdash M : A$ be a simply-typed term in $\beta$-normal form such that $\tau(M)$ is regular.
Consider a justified sequence of move $s \in \sem{\Gamma \vdash M}$
and let $q$ denotes the last move in $s$.
Proposition \ref{prop:rel_gamesem_trav} tells us that there
is a reduced-traversal $r$ of $\tau(M)$ such that $\varphi_{M}(r) \jseq s.$
Let us write $n$ for the last node in the sequence $r$
and $root$ to denote the root of $\tau(M)$.
By remark \ref{rem:redtrav}, $ r \jseq t \upharpoonright root$
...
If $M$ is a safe term then $\tau(M)$ is regular.
\end{lem}
\begin{proof}
If $M$ is safe then its $\eta$-normal form is also safe. Hence we can assume that $M$ is in $\eta$-normal form.
First we remark that in the computation tree, the binder of a node $x$ always occurs in the path from $x$ to the root.
...
\begin{cor}[Pointers are superfluous for safe terms]
...
By lemma \ref{lem:regularity}, $\tau(M')$ is regular.
Consequently, by lemma \ref{lem:regular_pointers_uniqu_recover}, the pointers in $\sem{M'}$ are uniquely recoverable.
Finally, the soundness of the game model ensures that $\sem{M} = \sem{M'}$ which permits to conclude.
\end{proof}
...
If our conjecture is true then by following the framework of \cite{ghicamccusker00} we can obtain a characterization of strategies
by extended regular expressions for terms of finitary higher-order recursion-free fragments of Safe Idealized Algol.
Subsequently, this leads to the decidability of program equivalence
for the considered higher-order fragments of Safe \ialgol.

\subsection{Overview of the main steps of the proof}
The idea is to apply the same recipe used for the Safe $\lambda$-calculus.
Let us recapitulates the main ingredients:
\begin{enumerate}
\item define a unique computation tree associated to a given term;
...
\item prove a theorem showing correspondence between traversals of the computation tree and plays in the game semantics;
...
\subsection{Computation tree and traversals}
...
for $\lambda x . 5$ is:
$$\tree{\lambda x}{  \TR{5} }$$
The node labelled $5$ has, like any other node, value-leaves which are not represented on the tree for simplicity.
