...
This chapter is concerned about the study of game semantics of
...
We will then apply the correspondence theorem to establish a
characterization of game semantics of safe terms. We will introduce
the notion of \emph{incrementally-justified strategy} and prove that safe
terms in $\beta$-normal form are exactly the simply-typed terms that
are denoted by such strategies. A nice property of such strategy
...
%traversals of the computation tree.
...
%of safe term and prove that safety leads to a certain economy of
...
application rule and abstraction rule are constrained the same way as in the Safe
...
\item prove that computation tree of safe term are incrementally-bound;
\item Using 2. and 5., prove that incrementally-justified strategies characterize incrementally-bound computation trees;
\item prove that pointers are uniquely recoverable in incrementally-justified strategies.
\end{enumerate}
...
We work in the general setting of the simply-typed $\lambda$ calculus extended with
a fixed set $\Sigma$ of higher-order constants.
...
We sometime add a label to the edge to specify that either node $n$
is labelled by the $i$th variable abstracted in the lambda node
$m$ or that $n$ is the $i$th child of the non-lambda node $m$.
The children nodes are numbered using the following convention:
\begin{itemize}
...
and for some variables $x,\xi_1, \ldots \xi_n$, $\Sigma$-constant $f$, $i \in 1..n$, $j \in 0..1$, $k \in 1 ..arity(m)$,
variable $z$ free in $M$.



We write $s \jseq t$ to denote that the two justified sequences $t$ and $s$ have
the same nodes and same pointers.
Justified sequence of nodes can be ordered using the prefix ordering: $t \sqsubseteq t'$ if and only if $t=t'$ or the sequence of nodes
$t$ is a finite prefix of $t'$ (and the pointers of $t$ are the same as the pointers of the corresponding prefix of $t'$).
Note that this definition also applies in the case of infinite justified sequences of nodes.
This ordering gives rise to a complete partial order.
...
$$t \cdot \rnode{m}{@} \cdot
\rnode{n}{n} \bkptra[nodesep=1pt]{60}{n}{m} \bklabelc{0}
$$
...
\rnode{n}{n} \bkptra[nodesep=1pt]{60}{n}{m} \bklabelc{i} \qquad
\mbox{ for } 1 \leq i \leq arity(x).$$
Moreover for any $v\in \mathcal{D}$ the sequence $t_1 \cdot \rnode{x}{x} \cdot t_2 \cdot \rnode{xv}{x^v}
\bkptra[nodesep=1pt]{50}{xv}{x}$ is also a traversal.
\end{itemize}
...
\begin{itemize}
  \item (CCAnswer-$@$) If $t \cdot \lambda \overline{\xi} \cdot \rnode{app}{@} \cdot \rnode{lz}{\lambda \overline{z}} \cdot \ldots \cdot  \rnode{lzv}{\lambda \overline{z}^v}
              \bkptra[nodesep=1pt]{30}{lzv}{lz} \bklabelc{i}
              ...
              \bkptra[nodesep=1pt]{30}{lzv}{lz} \bklabelc{i}
              \bkptra[nodesep=1pt]{40}{lz}{app} \bklabelc{0}
                \bkptra[nodesep=1pt]{30}{lmdv}{lmd} \bklabelc{i}$.

  \item (CCAnswer-$\lambda$) If $t \cdot \lambda \overline{\xi} \cdot \rnode{x}{x} \cdot \ldots \cdot  \rnode{xv}{x^v}
              \bkptra[nodesep=1pt]{30}{xv}{x} \bklabelc{i}$
              is a traversal then so is:
              $t \cdot \rnode{lmd}{\lambda \overline{\xi}} \cdot \rnode{x}{x} \cdot \ldots \cdot \rnode{xv}{x^v} \cdot
              \rnode{lmdv}{\lambda \overline{\xi}^v}
              \bkptra[nodesep=1pt]{20}{xv}{x} \bklabelc{i}
                \bkptra[nodesep=1pt]{20}{lmdv}{lmd} \bklabelc{i}$.

     \item (CCAnswer-var) If $t \cdot y \cdot \rnode{lmd}{\lambda \overline{\xi}}
                   \cdot \ldots
                   \cdot \rnode{lmdv}{\lambda \overline{\xi}^v} \bkptra[nodesep=1pt]{30}{lmdv}{lmd} \bklabelc{i}$ is a traversal,
        ...
        $$t \cdot \rnode{y}{y}
            ...
            \cdot \rnode{yv}{y^v}
                \bkptra[nodesep=3pt]{35}{yv}{y} \bklabelc{i}
                \bkptra[nodesep=1pt]{30}{lmdv}{lmd} \bklabelc{i}.$$
...
\cdot n \cdot x_i$. We do a case analysis on $n$:
    ...
    \end{itemize}
\end{itemize}
If $T$ denotes a computation tree then we write $\travset(T)$ to denote the set of traversals of $T$.
We also use the abbreviation $\travset(M)$ for $\travset(\tau(M))$.
\end{dfn}
...
\begin{enumerate}
  ...
  \item $m$ is the $i$th child of $n$;
  \item $i$ is uniquely determined by $t$ (i.e. the rule is deterministic).
\end{enumerate}
Note that since $m$ points to $n$, necessarily $m$ is a $\lambda$-node and $n$ is the parent node of $m$.
\end{dfn}
...
\section{Relationship between computation trees and arenas}
...
\subsection{Computation tree-arena correspondence}
...
\section{Category of interaction games}
...
\subsection{Modeling the $\lambda$-calculus in $\mathcal{I}$}
...
\subsection{From interaction semantics to standard semantics and vice-versa}
...
\section{Some properties of $\varphi$}
...
\section{The correspondence theorem for the pure simply-typed $\lambda$-calculus}
...
The result extends immediately to the simply-typed $\lambda$-calculus with \emph{uninterpreted} constants by interpreting constants as
free variables.
...
\section{Game semantics characterization of safety}
...
This section is concerned only with Safe simply typed $\lambda$-term.
In the following section we will extend the result to \pcf.
...
\end{lem}
\begin{proof}
Suppose $\sigma$ is an incrementally-justified strategy. We prove that pointers in a play $s\in \sigma$ are uniquely recoverable by induction on the length of $s$.
...
If $m$ is a P-move then $|s|$ is odd and by safe-justification
$m$ points to the last O-move in $\pview{?(s)}$ with order strictly greater than $\ord{q}$.
Similarly, if $m$ is an O-move then $|s|$ is even and by safe-justification
...
The last O-move in $?(\pview{s})= s$ with order strictly greater than $\ord{q_3}$ is $q_1$.
 However $q_3$ points to $q_0$ therefore $\sem{ev}$ is not incrementally-justified.
\end{exmp}
...
\begin{prop}[Incrementally-binding coincides with incrementally-justification] \
...
(ii) Suppose that the strategy $\sem{M}$ is incrementally-justified.
...
there exists a traversal of the form $t \cdot x$ for some $t \in \travset(M)$.
The correspondence theorem gives $\varphi((t \cdot x) \upharpoonright r) = \varphi((t \upharpoonright r) \cdot x) \in \sem{M}$.
Since $\sem{M}$ is incrementally-justified, $\varphi(x)$ must point to the last move in $\pview{?(\varphi(t \upharpoonright r))}$ with
...
Since node $x$ of order $0$ is not bound by the order 1 node $\lambda y$, $\tau(M)$ is not incrementally-bound and by proposition
\ref{prop:incrbound_imp_incrjustified} $\sem{\lambda x . f (\lambda y .x)}$ is not incrementally-justified.
Similarly we can check that the denotation of $f (\lambda y .x)$ is not incrementally-justified whereas $\lambda y. x$ has an incrementally-justified denotation.
Also for any higher-order variable $x:A$, the computation tree $\tau(x)$ is incrementally-bound, therefore
the projection strategies $\pi_i$ are incrementally-justified.
From these examples we observe that application does not preserve safe-justification: $\sem{f}$ and $\sem{\lambda y. x}$ are incrementally-justified whereas
$\sem{f (\lambda y .x)}$ is not.

%In fact safe-justification is not preserved by composition. Indeed,
...
(ii) We assume that $M$ is already in $\eta$-normal form. Suppose $M$ is closed and $\tau(M)$ is incrementally-bound,
...
If it is bound by $\lambda \overline{\xi}$ then since $\tau(M)$ is incrementally-bound we have $\ord{z} \geq \ord{\lambda \overline{\eta_1}} = \ord{N_i}$.
...
\section{Game semantics of Safe PCF and Safe Idealized Algol}
...
A computation tree is a tree representing the $\eta$-normal form of some (potentially infinite) \pcf\ term. In
other words a tree is a computation tree if it can be written $\tau(M)$ for some infinite \pcf\ term $M$.
The set $L$ of labels is constituted of the $\Sigma$-constants, $@$, the special constant $\bot$, variables and abstractions of any sequence of variable.
...
We can now define the computation tree of a \pcf\ term $M$ to be the
the least upper-bound of the chain of computation trees of its approximants:
$$\tau(M) = \Union_{n\in\omega}(\tau(M_n))_{n\in\omega}.$$
It is easy to check that the sequence of computation trees $(\tau(M_n))_{n\in\omega}$ is indeed a chain.
...
constructed from the $\eta$-normal form of the term in the standard way. For instance the diagram below shows
...
\begin{lem}
\label{lem:travred_continuous}
Let $(S,\subseteq)$ denotes the set of sets of justified sequence of nodes ordered by subset inclusion.
...
\begin{proof} \
    ...
    at the last position in a traversal. Let us prove the following two properties:
        ...
        (i) By induction on the length of $t$: it is trivial for the empty traversal.
            suppose $t = t_1 \cdot n$ with $n \neq \bot$.

            For all traversal rules the node $n$ is defined to be the $i$th node
            of some node $n$ occurring in $t_1$ for some $i\in 0..arity(n)$.
            Moreover for all rules, the exact value of $i$ depends only on the traversal $t_1$
            and not from any other property of the computation
            tree \footnote{In fact for the rules (InputVar$^0$) and (InputVar$^{\geq 1}$), the value of $i$ is chosen
            non-deterministically and therefore does not even depend on $t_1$.}.

            Suppose that $n$ is justified by $m$ in $t_1$.
            Since $T \sqsubseteq T'$ and $m\neq\bot$, $m$ belongs to $T'$
            and all the children nodes of $m$ in $T$ also belong to $T'$.

            By the induction hypothesis, $t_1$ is a traversal of $T'$. Therefore by the previous two remarks we can
            apply the traversal rule that has been used to produced $t$ of $T$ to produce the traversal $t$ of $T'$.
...
\mbox{(by (i) and (ii))} \quad        & \subseteq  \{ (t\cdot n) \upharpoonright r \ | \ t\cdot n \in \travset(T') \wedge n \neq \bot \}
        ...
        The set of traversal is prefix-closed therefore $t_i \in \travset \left( \Union_{n\in\omega} T_n \right)$ for any $i$.
...
by the induction hypothesis $N_k$ is safe. Consequently the term $Y_A^k N_k = \underbrace{N_k ( \ldots ( N_k}_{k \mbox{ times}} \Omega ) \ldots )$ is also safe.
Hence by compositionality $M_k$ must be safe.

Clearly, lemma \ref{lem:safe_imp_incrbound} is still valid in $\pcf_1$ (the subterms of the form $\Omega$ are just
represented by the constant $\bot$ in the computation tree).
Therefore since $M_k$ is in $\pcf_1$, $\tau(M_k)$ must be incrementally-bound.
Consequently, the node $z$ is safe in $\tau(M_k)$ and since $[r,z]=[r_k,z]$, the corresponding node $z$ in
the tree $\tau(M)$ is also safe.
...
We now consider Safe Idealized Algol. First we concentrate on the \ialgolmnew\ fragment. Then we can make use of the factorization
theorem to extend the result to full Safe \ialgol.

\subsubsection{\ialgolmnew fragment}

The extension to \ialgolmnew fragment is straightforward. It follows the same recipe as for Safe \pcf.
However there are some differences due to the introduction of new base type.
Indeed, the language \ialgol\ has two new base types: \iavar\ and \iacom. In the game semantics model, the base type
\iavar\ is represented by the infinite product of games $\iacom^{\nat} \times \iaexp$. This game has infinitely many
initial moves. This contrasts with the arenas of \pcf\ which all have a single initial move.
Since we are aiming at establishing a relationship between arenas and computation trees, we need to update the definition of
computation tree to take into account this new property of arenas.


\subsubsection{Computation DAG and traversals for \ialgolmnew }
In \ialgol, ``computation trees'' becomes ``computation directed acyclic graphs (DAG)''. This means that
a computation DAG may have (possibly infinitely) many roots and that two nodes of a given level can share children at the next level.
...
It is then straightforward to defined the traversals rules for \ialgolmnew\ as we did for \pcf.
...
%and that the O-view of a traversal is the O-view of its reduction (lemma \ref{lem:redtrav_trav});
%\item prove that computation tree of safe term are incrementally-bound;
%\item prove that incrementally-bound computation trees are denoted by incrementally-justified strategies.
%\end{enumerate}
...
\todobox{TODO: Theorem showing the correspondence  between game plays and traversals of the computation DAG;
prove the counterpart of lemma \ref{lem:redtrav_trav} for traversal of \ialgol\ terms;
check that computation DAG of Safe \ialgol\ terms are incrementally-bound;
consequently by lemma \ref{lem:incrjustified_pointers_uniqu_recover}, pointers in the strategy are superfluous for Safe IA terms.
}
...
\begin{enumerate}
\item prove that observational equivalence is decidable for Safe \ialgol;
...
In a more general direction of research, we would like to study the class of languages
for which pointers are uniquely recoverable. We name this class PUR for ``Pointer Uniquely Recoverable''.

Safe $\lambda$-calculus is an example of PUR language. Another example is
the Serially Re-entrant Idealized Algol, or SRIA for short, proposed
by Abramsky  in \cite{abramsky:mchecking_ia}.
This language allows multiple occurrences or uses of arguments, as long
as they do not overlap in time. In the game semantics denotation of a SRIA term there
...
be ignored.
Safe \ialgol\ is not a sublanguage of SRIA. One reason for this is that none of the two Kierstead terms
$\lambda f . f (\lambda x . f (\lambda y .y ))$ and
$\lambda f . f (\lambda x . f (\lambda y .x ))$ are Serially Re-entrant whereas the first one is safe.
Conversely, SRIA is not a sublanguage of Safe \ialgol\ since the term
$\lambda f g. f (\lambda x . g (\lambda y .x ))$ where $f,g:((o,o),o)$ belongs to SRIA but not to Safe \ialgol.
SRIA and Safe \ialgol\ are therefore two different examples of languages with pointer-less game semantics.

Another example of PUR language is Finitary $\ialgol_2$. For this language, not only pointers can be uniquely recovered but
also observational equivalence is decidable. How does this property extend to other PUR languages?
