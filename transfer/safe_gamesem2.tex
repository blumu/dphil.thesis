...
any \emph{bound} variable node $x$ occurring in the computation tree is bound by the first $\lambda$-node in the path to the root that has
...
there exists a traversal $t \cdot x \in \travset(M)$.
...
\item[(ii)] reciprocally, if $M$ is \emph{closed} and $\tau(M)$ regular then the $\eta$-normal form of $M$ is safe.
\end{itemize}
\end{lem}
\begin{proof}
