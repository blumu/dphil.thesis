...
\begin{proof}
    ...
    First we observe that traversals ending with a node labelled $\bot$ are maximal.
    Hence $\bot$ can only occur at the last position in a traversal.
    There are two cases:
        \begin{enumerate}
            \item Suppose that $t$ does not end with a $\bot$-labelled node.
 %           that does not contain any node labelled with $\bot$.

            We prove that $t$ is also a traversal of $T'$ by induction on the length of $t$.
            It is trivial for the empty traversal.
            \emph{Step case:} suppose $t = t_1 \cdot n$ with $n \neq \bot$ then by induction, $t_1$ is a traversal of $T'$.
            Let us do a case analysis on the rule used to obtain $t$ from $t_1$:
            \begin{itemize}
                \item if it is the (Var) rule:
                \item  if it is a constant rule: since constant rules are well-behaved, $n$
                must be the child node of some non $\lambda$-node occurring in $t_1$.

                \item for any other rule: by inspection of the rules we observed that $n$
                must be the child of some node occurring in $t_1$. Therefore
                the proof is similar to the previous case.
            \end{itemize}
            %For all the traversal rules except Var
            %therefore all the node of $t_1$ exist since $T \sqsubseteq T'$, the node
            Consequently, any reduced traversa


            \item Suppose that $t_1 \cdot \bot$. Then $t_1$ does not end with $\bot$ and
            therefore from the previous case we have $t_1\in \travset(T')$.
        \end{enumerate}
...
\mbox{(by 1. and 2.)} \quad        & \subseteq  \{ (t\cdot n) \upharpoonright r \ | \ t\cdot n \in \travset(T') \wedge n \neq \bot \}
            \union \{ t \upharpoonright r \ | \ t \in \travset(T')  \} \\
        & = \travset(T')^{\upharpoonright r}
        \end{align*}

        \item[Continuity:]
        $$\travset^{\upharpoonright r} \left(\Union_{n\in\omega} T_n \right) = \Union_{n\in\omega} \travset^{\upharpoonright r}(T_n)$$
    \end{description}
\end{proof}
...
&= \travset^{\upharpoonright r}\left(\Union_{n\in\omega} \tau(M_n) \right) & (\mbox{by continuity of $\travset^{\upharpoonright r}$, lemma \ref{lem:travred_continuous}}) \\
...
Despite this particularity, lemma \ref{lem:regularity} still holds in the \pcf\ setting:
\begin{lem}[Safety--regularity]
