...
%%  \item (application)
%%  Suppose that $\ord{M} = 0$ then the arena is of the following form (only question moves are represented):
%%    $$ \tree{ q }
%%        {   \tree[linestyle=dotted]{q^1}{\TR{} \TR{} }
%%            \tree[linestyle=dotted]{q^2}{\TR{} \TR{} }
%%            \TR{\ldots}
...
%%        }$$
%%        \begin{itemize}
%%        \item (variable) Suppose that $M = \lambda. y_i : o$. Then $\sem{M} = \pi_i$ and the only question move that can occur
%%                in $s \in \pi_i$ is the initial question $q$ (with no pointer).
%%                Take $t = \tilde{s}[\lambda/q]$ where $\lambda$ denotes the root of the tree $\tau(M)$.
%%                It is then clear that $\varphi^{-1}(t) = \tilde{s}$.
%%
