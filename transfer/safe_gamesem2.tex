...
A \emph{traversal} is defined to be the prefix of a maximal-traversal.
...
\item ($\mbox{Sig}^{\geq 1}$) After a node labelled $f@$ where $f$ is a $\Sigma$-constant of order greater or equal to $1$,
a maximal-traversal must visit one of the children nodes of $f@$.

\item ($\mbox{Var}^0$) After a node labelled $x$ where $x$ is a variable of order $0$ either occurring free in $M$
or bound by the root of the tree $\tau(M)$, a maximal-traversal
stops.

\item ($\mbox{Var}^{\geq 1}$) After a node labelled $x@$ where $x$ is a variable of order greater or equal to $1$ either occurring free in $M$
or bound by the root of the tree $\tau(M)$, a maximal-traversal must
visit one of the children nodes.
...
\epsilon$. Let $s$ be the reduced-traversal $s = t \upharpoonright
\{ r \}$
\begin{itemize}
...
move. Suppose that property (i) is verified for traversal $t'$. Let
$s'$ be the reduced-traversal $s' = t' \upharpoonright \{ r \}$:
    \begin{equation}
...
\item Consider the traversal:
    ...
    and suppose that $t''$ verifies property (i). Let
    $s''$ be the reduced-traversal $s'' = t'' \upharpoonright \{ r \}$.
