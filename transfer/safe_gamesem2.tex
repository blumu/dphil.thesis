...
%  \item Suppose $t \cdot n' \cdot n \cdot \rnode{lmd}{\lambda \overline{\xi}}
%                   \cdot \ldots
%                   \cdot \rnode{lmdv}{\lambda \overline{\xi}^v} \bkptra[nodesep=1pt]{40}{lmdv}{lmd}$.
%        is a traversal where $n$ and $n'$ are nodes.
%        Since traversals are well-bracketed (this can be proved by induction)
%        $n$ must be the last pending node.
%
%        Moreover the subsequence of pending nodes of a traversal is such that
%        $\lambda$-nodes and non $\lambda$-nodes alternate (proof by induction),
%        therefore $n$ must be a non $\lambda$-node:
%        \begin{itemize}
%        \item if $n=@$ then necessarily $\lambda \overline{\xi}$ must be the $0th$ child node of $@$ in the computation tree and
%        \vspace{5pt}
%        $$t \cdot \rnode{app}{@}
...
%                \bkptra[arcangleA=-30,nodesep=0pt]{30}{appv}{app}
%                \bkptra[nodesep=1pt]{20}{lmdv}{lmd}$$ is also a traversal;
%        \item if $n$ is a non-input variable node $y$ then the following is also a traversal:
%        $$t \cdot \rnode{y}{y}
%            \cdot \rnode{lmd}{\lambda \overline{\xi}}
%            \cdot \ldots
%            \cdot \rnode{lmdv}{\lambda \overline{\xi}^v}
...
Nodes labelled $\fatsemi$, $\times$ or $\otimes$ are binary nodes, nodes labelled $!$, $\dagger$ or $\Lambda$ are unary nodes.
...
\end{itemize}
\end{dfn}
A $;$-node with children of type $!A\multimap B$ and $!B\multimap C$ is annotated with a set of index $U$ which indicates
...
From now on we will write $\Sigma_1 \fatsemi^U \Sigma_2$ for $\Sigma_1^\dagger ; ^U \Sigma_2$.
...
\begin{align*}
  \intersem{M}
    ...
    &=  \underbrace{\underbrace{\langle \varphi_{M} (\travset^{-@}(N_0)), \varphi_{M}(\travset^{-@}(N_1)) \rangle}_{\sigma} \parallel ev}_L
           & \mbox{($\fatsemi^{0..1}$ and $\parallel$ are the same operator)}
%    &=  \varphi_{M}( \langle \travset^{-@}(N_0), \travset^{-@}(N_1) \rangle \fatsemi^{0..1}
%    ev )
...
We now prove that
$\L = \overbrace{\varphi_{M}( \travset^{-@}(M) )}^{R}$.

It is enough to consider only maximal sequences.
Let $q_o$ be the initial question of the arena $\sem{M}$ and $q_1$ the initial question of $\sem{N_0}$.

The maximal sequences in $L$ all starts with $q_0 \cdot q_1$. The move $q_0$ is the initial move in $C$ and
$q_1$ is the copy of $q_0$ replicated by $ev$ in the component $o'$.

In $R$, the maximal sequences all starts with $t_p = \lambda \overline{\xi} \cdot @ \cdot \lambda y_1\ldots y_p$
and we have $\varphi_M(t_p-@) = q_0 \cdot q_1$.




%%
%%We first prove that $\langle \varphi_{M} (\travset^{-@}(N_0)), \varphi_{M}(\travset^{-@}(N_1)) \rangle \fatsemi^{0..1} ev \subseteq \varphi_{M}( \travset^{-@}(M) )$.
%%Suppose $ u \in \langle \varphi_{M} (\travset^{-@}(N_0)), \varphi_{M}(\travset^{-@}(N_1)) \rangle \fatsemi^{0..1} ev$
%%we prove by induction on the size of $u$ that $u \in \varphi_{M}( \travset^{-@}(M)$.
%%Base cases:
%%\begin{itemize}
%%\item $u=\epsilon$ then $\varphi(\epsilon) = u$.
%%\item $u=m$ then $m=q_0$ is the initial move in $C$ and $\varphi(\lambda \overline{\xi}) = u$.
%%\item $u=q_0 \cdot m$ then since $u \upharpoonright B, C \in ev$, $m$ must be the copy of $q_0$ replicated by $ev$ in the $B_1$ component of $B$.
%%$m$ is therefore the initial question of the arena $\sem{N_0}$. Hence $\varphi(\lambda \overline{\xi} \cdot \lambda y_1\ldots y_p ) = u$.
%%\end{itemize}
%%Step cases, if $u = u' \cdot m$ with $u' = \varphi(t')$ for some traversal $t'$ of $\tau(M)$:
%%\begin{enumerate}
%%
%%\item If $m \not\in A \union B \union C$ then $m$ is an internal move. Suppose that $m$ belongs to an internal arena
%%of the interaction strategy $\intersem{N_0}$, then $m$ does not play any role during the composition of $\sigma$ with $ev$.
%%$m$ must be hereditarily justified by an initial node $m^0$ in $\sem{B_1'\rightarrow o}$. We have
%%$(u \upharpoonright m^0) \upharpoonright \Gamma, (B_1'\rightarrow o) = \varphi_{N_0}(t_0-@)$ for some traversal $t_0$ of $N_0$.
%%$m$ is the last node in $u$ therefore it is also the last node in $\varphi_{N_0}(t_0-@)$.
%%
%%Take the traversal $t = t' \cdot n$ where $n$ is the last node in $t_0-@$.
%%
%%Since $m$ is an internal move of $\intersem{N_0}$, the move preceding $m$ which is also the last move of $u'$ must belong to an arena
%%of $\intersem{N_0}$ but not to $\sem{N_0}$, otherwise the evaluation.
%%
%%It is indeed a traversal of $\tau(M)$ since the last node in $t$.
%%
%%
%%
%%The proof is the similar if $m$ belongs to an internal arena of the interaction strategy $\intersem{N_1}$.
%%
%%\item $m \in C$. If $m = q_0$ then this is the second base case.
%%If $m = q_0^v$ for some value $v\in \mathcal{D}$ then take $t=t'\cdot \lambda \overline{\xi}^v$ where $\lambda \overline{\xi}$ points to the first node in $t$.
%%
%%\item $m \in B_1$. Let us write $u = \ldots \cdot m'' \cdot m' \cdot \ldots \cdot m_l \cdot m$ and
%%$t' = \ldots \cdot n'' \cdot n' \cdot \ldots \cdot n_l$.
%%
%%    \begin{itemize}
%%    \item If $m_l$ is an O-move then $m$ is a P-move and therefore $m$ is the copy of $m_l$ duplicated in $B_1$ by the evaluation strategy.
%%    Consequently, $m_l$ points to $m'$ and $m$ points to $m''$.
%%
%%    $m_l$ is hereditarily justified by some initial move $m_l^0$ in $B_1'$
%%    \end{itemize}
%%
%%\end{enumerate}
%%    For the converse, we proceed by induction on the structure of the traversal and case analysis on the rule used:
%%    $s =\varphi_M(t-@)$
%%
%%    \begin{itemize}
%%    \item The base case, rule ($\epsilon$), is trivial : $t = s = \epsilon$.
%%    \item Rule (Root), $t=r$ then $s = q_0 \sem{M}$.
%%    \item Rule (App) then $t=t' \cdot @$ and $t-@=t'-@$ the induction hypothesis permits to conclude
%%    \item Rule (Lmd) then $t= t' \cdot \lambda \overline{\xi} \cdot n$
%%    \end{itemize}

    We have $\sigma : !A \multimap B$ and $ev : !B \multimap C$, the arena $A,B,C$ being defined as follows:
    \begin{eqnarray*}
    ...
    $u \in \intersem{M} \cong \sigma^{\dag} \parallel ev$ if and only if
    $$
        \left\{
          \begin{array}{ll}
            u \in int(!A,!B,C)\\
            w \defeq u \upharpoonright !A,!B  \in \sigma^\dagger \\
            u \upharpoonright !B,C  \in  ev
          \end{array}
        \right.
    $$

    Because of this there must be a sequence of nodes $t$ in $N\setminus\{@\}$ such that $\varphi_M{t} = u$.

    Suppose $u=u_0 u_1 \ldots$.


    Therefore for any $u \in \intersem{M}$,


    $w \in \sigma^\dagger$ means that $(w \upharpoonright m) \in \sigma$ for any initial $m$ in $w$. In other words for any
    initial $m$ in $w$ there is $j \in 0..p$ such that:
    \begin{eqnarray}
        z_m \defeq w \upharpoonright !A, B_j, m &\in& \varphi_{M} (\travset^{-@}(N_0)) \label{eq:def_z} \\ % \intersem{\Gamma \vdash N_j}  \label{eq:def_z} \\
        (w \upharpoonright m) \upharpoonright B_k &=& \epsilon \quad \mbox{ for every } k\neq j \label{eq:b}
    \end{eqnarray}

%    From equation \ref{eq:def_z} we have $z_m \in \intersem{N_j}$. Since the term $N_j$ is a closed term of height strictly smaller than the height
%    of $M$, we can apply the induction hypothesis:
%    there is a traversal $t^j_m$ of the tree $\tau(N_j)$
%    such that $\varphi_{N_j}(t^j_m - @) \jseq z_m$.

    From equation \ref{eq:b}, for $k\neq j : (w \upharpoonright m) \upharpoonright B_k = \epsilon$. Since $B =
    B_0 \times B_1$ this implies that moves in $(w \upharpoonright m)$ hereditarily justified by moves in $B$ are in fact all
    hereditarily justified by moves in $B_j$.  Hence $z_m = (w \upharpoonright m) \upharpoonright !A, B_j = (w \upharpoonright m) \upharpoonright !A, B = w \upharpoonright !A, B, m$ and
    $\varphi_{N_j}(t^j_m - @) \jseq w \upharpoonright !A, B, m$.
...
\travset(M) \ar@/_/[ur]_{-@}^{\cong} \ar[dr]^{\upharpoonright r}  \\
                                           & \travset(M)^{\upharpoonright r} \ar[r]^{\varphi_M}_\cong & \sem{M} \ar@/_/[uu]^{\cong}_{\mbox{full uncover.}}
}
$$
...
the subterm $f (\lambda y . x)$, the free variable $x$ has the same
