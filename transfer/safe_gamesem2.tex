...
of section \ref{sec:safe_homog} leads to am economy of pointer in the game
...
$nd$, the index number of a $x@$-labelled node or a $x$-labelled
node in the computation tree and $q$, a question move of the arena
...
We consider the cartesian closed category $\mathcal{C}$ as a model
of the simply-typed lambda calculus. We note $\sem{\Gamma \vdash M :
A}$ for the strategy denoting the simply-type term $\Gamma \vdash M
: A$ as it has been defined in section \ref{subsec:pcfgamemodel} of
the first chapter.


\begin{prop}[Relation between game semantics and reduced-traversals]
\label{prop:rel_gamesem_redtrav_closed} Let $\emptyset \vdash M : A$
be a closed term. Let $r$ denotes the root of $\tau(M)$. If $s$ is a
justified sequence of moves such that $s \in \sem{\Gamma \vdash
M:A}$ then there is a traversal of $\tau(M)$ $t = n_0 n_1 \ldots$
such that:
 $$\tilde{s} \jseq  \varphi_M(t \upharpoonright  r  )$$
\end{prop}
\begin{proof}
Let us assume that $M$ is already in $\eta$-long normal form.
Let $s$ be as justified sequence of moves in $\sem{M}$.

We proceed by induction on the height of of the tree $\tau(M)$ and
by case analysis on the structure of the computation tree.
...
\begin{itemize}
  ...
  \item[(order 0 variable)] $M = x$. This case does not happen since $M$ is a closed term.
...
%    $$ \varphi_{M} (t \upharpoonright \lambda) \jseq \varphi_{M} (t) \jseq  \tilde{s}$$
  ...
  \item[(abstraction)]
    ...
    \begin{itemize}
      \item $M'$ is an order 0 constant: same treatment as the (constant) case above.
      \item $M'$ is a variable: then the variable must be of ground type $o$ and since $M$ is
      closed it must be $\xi_i \in \overline{\xi}$ for some $i$.
      Hence $M = \lambda \overline{\xi} . \xi_i$ and  $\tau(M)$ has the following shape:
        ...
        Therefore the only question-moves occurring in $s \in \sem{M}$ are the initial question $q$ (which occurs only once)
        and $q'$.
        We take $t \jseq \tilde{s} [\lambda \overline{\xi}/q, \xi_i/q']$ which is a valid justified sequence of nodes since
        the node $\xi_i$ is bound by the root node $\lambda \overline{\xi}$. It is also a valid
        traversal and clearly we have $\varphi(t\upharpoonright \lambda \overline{\xi} ) \jseq \varphi(t) \jseq  \tilde{s}$.

    \item $M'$ is an application: $M = \lambda \overline{\xi} . N_0 N_1 \ldots N_p$ where
    \begin{eqnarray*}
    \emptyset &\vdash& M : (X_1,\ldots,X_n,o) \\
    \Gamma &\vdash& N_0 N_1 \ldots N_p : o \\
    \Gamma &\vdash& N_0 : (A_1,\ldots,A_p,o) \\
    \Gamma &\vdash& N_i : A_i \mbox{ for } i \in 1..p \\
    \Gamma &=& \overline{\xi} : \overline{X}
    \end{eqnarray*}

    In the case $N_0 \equiv \alpha$ where $\alpha$ is either a $\Sigma$-constant or a variable in $\overline{\xi}$, the tree has the following form:
    $$ \tree[levelsep=6ex]{\lambda \overline{\xi}^{[0]}}
        { \tree[levelsep=6ex]{\alpha@^{[1]}}
            {   \TR{\tau(N_1)} \TR{\ldots} \TR{\tau(N_p)}}}
    $$
    In the other cases, when $N_0$ is neither a variable nor a constant, the tree $\tau(M)$ has the following form:
    $$ \tree[levelsep=6ex]{\lambda \overline{\xi}^{[0]}}
        { \tree[levelsep=6ex]{@^{[1]}}
            {   \TR{\tau(N_0)} \TR{\ldots} \TR{\tau(N_p)}}}
    $$
    ...
    \sem{\Gamma \vdash N_0  N_1 \ldots N_p : o} &=& \overbrace{\langle \sem{\Gamma \vdash N_0}, \sem{\Gamma \vdash N_1}, \ldots, \sem{\Gamma \vdash N_p} \rangle}^\sigma \fatsemi\ ev_{A_1,\ldots,A_p,o}
    \end{eqnarray*}

    with $\sigma : !A \multimap B$ and $ev_{A_1,\ldots,A_p,o} : !B \multimap C$, the arena being defined as follows:
    \begin{eqnarray*}
    ...
    We have $s \in \sem{M} \cong \sem{\Gamma \vdash N_0 N_1 \ldots N_p : o} = \sigma \fatsemi ev_{A_1,\ldots,A_p,o}$ therefore:
    \begin{equation}
        s = u \upharpoonright !A, C   \quad \mbox{ for some } u \in \sigma^{\dag} \parallel ev_{A_1,\ldots,A_p,o}
    \end{equation}
    ...
    For any initial move $m$ in $w$ we note $w_m = w \upharpoonright
    m$. $w \in \sigma^\dagger$ means that for all initial $m$ in $w$, $w_m \in \sigma$. Hence for
    all initial $m$ in $w$ there is $j \in 1..p$ such that:
    \begin{eqnarray}
        z_m \defeq w_m \upharpoonright !A, H_j &\in& \sem{\Gamma \vdash N_j}  \label{eq:def_z} \\
        \mbox{ and for every } k\neq j &:& w_m \upharpoonright H_k = \epsilon \label{eq:b}
    \end{eqnarray}

    We cannot use the induction hypothesis on $\Gamma \vdash N_j : A_j$ because it is not a closed terms!
    Instead we consider the closed term $N'_j = \lambda \overline{\xi} . N_j$:
    ...
    Equation \ref{eq:def_z} gives $z_m \in \sem{N_j} \cong \Lambda^n(\sem{\Gamma \vdash N_j}) = \sem{N'_j}$.
    The term $N'_j$ is a closed term of same height as $N_j$ which is strictly smaller than the height
    of $M$ therefore we can apply the induction hypothesis to $N'_j$:
    there is a traversal $t^j_m$ of the tree $\tau(N'_j)$
    such that $\varphi_{N'_j}(t^j_m \upharpoonright \overline{y_j} ) \jseq \widetilde{z_m}$.
    ...
    From equation \ref{eq:b}, for $k\neq j : w_m \upharpoonright H_k = \epsilon$. Since $B =
    H_0 \times H_1 \times \ldots \times H_p$ this implies that moves in $w_m$ hereditarily justified by moves in $B$ are in fact all
    hereditarily justified by moves in $H_j$.  Hence $z_m = w_m \upharpoonright !A, H_j = w_m \upharpoonright !A, B$ and:
    \begin{eqnarray*}
    ...
    Hence $\varphi_{N'_j}(t^j_m \upharpoonright \overline{y_j} ) \jseq \widetilde{w_m}$.
    ...
    (Root)) and then passes the node $x@$. It then proceeds by traversing the term $N_0$
    ...
    It becomes clear that the trajectory of the traversal $t$ behave like the composition of the strategy $\sigma$
    ...
    We process by ``reading'' the sequence of moves $s$ move by move, uncovering on-the-fly the moves that have been hidden by
    composition. We describe in parallel how the traversal progresses.
    ...
    $H_0$ and the game continues in $H_0$. We reflect this in the traversal : we make $t$ follow
    ...
    of the game $H_0$ in which case the evaluation map copies the move to an initial move $m_1$ in the corresponding component $H_k = \sem{A_k}$.
    ...
    back to the component $A_k$ of the game $H_0$.

    The traversal now resumes the simulation of $t^0_{m_0}$. And the
    process goes continuously.

    \begin{itemize}
    \item[Remark 1:] When the traversal pauses a simulation thread and
    ...
    \item[Remark 2:] When simulating a traversal $t^k_{m}$, if a variable
     ...
     altered. The reason is that $t^k_{m}$ is a traversal of $N'_k$, a term which was
    introduced to close the term $N_k$. We therefore need to change the pointer
    to make it point to the root of $\tau(M)$ instead.
    ...
    By construction of the traversal $t$, the nodes of $t$ that are hereditarily justified
    by the root correspond exactly to the question moves of the arena $A = \sem{X_1} \times \ldots \times \sem{X_n}$ and $C = \sem{o}$ that have been
    copied by the strategy $\sigma$ and the copy-cat evaluation strategy respectively. Moreover for each node $\alpha$ in $t$, this corresponding question move is given by
    $\varphi_M(\alpha)$. Hence $\varphi_M(t \upharpoonright r) = \tilde{u} \upharpoonright !A, C$ ($u$ is the interaction
    sequence).


    Also, the sequence of moves $s$ can be obtained by hiding the internal moves in the interaction sequence of moves between
    the strategy $\sigma$ and the copy-cat evaluation  strategy $ev$. But all the hiding can be performed at once by conserving only the moves occurring in the arena $A$ or
    $C$:
    $$ \tilde{s} = \tilde{u} \upharpoonright !A, C$$
    Hence we have:
    $$\varphi_M(t\upharpoonright r) = s$$
