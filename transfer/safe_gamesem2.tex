...
We note $\mathcal{TRAV}(M)$ for the set of traversals of $\tau(M)$.
\end{dfn}
...
\label{prop:rel_gamesem_redtrav_closed} Let $\Gamma \vdash M : T$
be a term of the pure simply-typed $\lambda$-calculus. Let $r$ denotes the root of $\tau(M)$.

Consider a play $s$ on the game $\sem{T}$. If $s \in \sem{\Gamma \vdash
M:T}$ then there is a computable traversal $t$ of $\tau(M)$ such that:
 $$\tilde{s} \jseq  \varphi_M(t \upharpoonright r )$$

Moreover if $s$ is a \emph{maximal} play then the traversal $t$ is \emph{maximal} and \emph{uniquely defined}.
In other words, the restriction of the function $\varphi_M : \sem{M} \rightarrow \mathcal{TRAV}(M)$ to the
set of maximal plays defines a bijection from the set of maximal plays to the set of maximal traversals of $M$.
...
We then have:
$$\sem{\Gamma \vdash M:T} = \Lambda^n( \sem{\emptyset \vdash \lambda \xi_1\ldots \xi_n . M: (X_1,\ldots,X_n,T) } ) \simeq \sem{\emptyset \vdash \lambda \xi_1\ldots \xi_n . M: (X_1,\ldots,X_n,T) }.$$
Similarly $\tau(M)$ is isomorphic to
...
Hence we can make the assumption that $M$ is a closed term.
...
%      $M'$ is an order 0 constant: same treatment as the (constant) case above.
  ...
  \item (abstraction of a variable): $M \equiv \lambda \overline{\xi} . x$
    ...
    with $\sigma : !A \multimap B$ and $ev_{B_1,\ldots,B_p,o} : !B \multimap C$, the arena being defined as follows:
    \begin{eqnarray*}
