...
$t$.
%It is straightforward to check that the sequence obtained is a
...
Consider the $\beta$-normal term $M = \lambda x . f (\lambda y .x)$ where $x,y:o$ and $f:(o,o),o$. Its computation tree is
represented on the right. The exponent in each label gives the order of the node.
The node $x$ of order $0$ is not bound by the order 1 node $\lambda y$ therefore $\tau(M)$ is not regular. Hence by proposition
\ref{prop:regular_comp_imp_regular_strat},
$\sem{\lambda x . f (\lambda y .x)}$ is not regular.
Similarly, the denotation of $f (\lambda y .x)$ is not regular, but the denotation of $\lambda y. x$ is regular.
...
We also remark that the evaluation strategy does not preserve regularity since $\sem{f}$ and $\sem{\lambda y. x}$ are regular but
...
In a computation tree, a binder node always occurs in the path from the bound node to the root,
therefore by the previous observation a bound variable $x$ must be bound by the first $\lambda$-node occurring in the path to the root
that has order strictly greater than $\ord{x}$. Hence $\tau$ is regular.
