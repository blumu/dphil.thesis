...
\paragraph{Category of interaction game semantics}

We call \emph{type-tree} a binary tree labelled with simple types : each nodes and leaves is labelled with a simple type such that the type
associated to a node is equal to the composition of type of its two children.

Let $T$ be type-tree. We define an \emph{interaction arena} of type $T$ to be
a binary tree of arenas isomorphic to $T$ such that each leaf or node of the interaction arena tree is the arena (in the sense of the standard game semantics)
of the type of the corresponding node in the tree $T$.
The interaction tree of a type-tree $T$ is written $\intersem{T}$.
We write $root(\intersem{T})$ to denote the arena located at the root of the interaction arena $\intersem{T}$.

\begin{dfn}[Function space construction]
Let $\intersem{T_1}$ and $\intersem{T_2}$ be two interaction arenas. Then the function space interaction arena
$\intersem{T_1 \rightarrow T_2}$ is obtained by replacing every node-arena or leaf-arena $\sem{A}$ of the tree $\intersem{T_2}$ by
the arena $\sem{root(\intersem{T_1}) \rightarrow A}$.
\end{dfn}

\begin{dfn}[Tree-type of a term]
Let $M$ be a term. The \emph{tree-type of $M$} is a binary tree isomorphic to the tree obtained by collapsing
every maximal sub-term of the computation tree that is in $\beta$-normal form to a single leaf.
The nodes of the tree-type of $M$ correspond to $@$-nodes in the computation tree
of $M$, leaves correspond to maximal sub-terms in $\beta$-normal form.
Suppose that $n$ denotes an $@$-node in the computation tree then the corresponding node in the type-tree is
labelled with $type(\kappa(p))$ where $p$ denotes the parent node of $n$ in the computation tree.
\end{dfn}
The tree-type of $M$ is a well-defined tree-type (the type of a node is equal to the type composition of its two children nodes).

The interaction arena of a term $\Gamma \vdash M:A$ noted $\intersem{\Gamma \rightarrow A}_M$
is defined to be $\intersem{T}$ where $T$ denotes the tree-type of $M$.


\begin{exmp}
Consider the term $\lambda x . (\lambda f . f x) (\lambda y . y)$. Its computation tree and
...
An \emph{interaction strategy} $\Sigma$ on the interaction arena $\intersem{T}$ is a composition of several
standard strategies where the internal moves are not hidden. Suppose that the list of leaves of $T$ obtained by
depth-first search (i.e. appearing from left to right on in the tree) is: $A_1$, \ldots $A_p$ then
$\Sigma$ must be the composition of strategies $\sigma_1$, \ldots $\sigma_p$ on the games $\sem{A_1}$, \ldots $\sem{A_p}$ respectively.

by is the composition $\

An interaction strategy on the interaction arena $\intersem{T}$ is obtained by composing


A play of the interaction semantics is called \emph{interaction play}.
In an interacting play, the moves are implicitly tagged so that it is possible to retrieve
which arena-node and which component of the arena the move belongs to.
The internal moves of an interaction play are those
which do not belong to the root arena of $\intersem{T_1 \rightarrow T_2}$.

We define the category $\mathcal{I}$ corresponding to the interaction game semantics. The objects of
$\mathcal{I}$ are \emph{interaction-arenas}.
The morphisms of the category are the interaction strategies: a morphism from $T_1$ to $T_2$ is an interaction strategy on
the interaction arena $\intersem{T_1 \rightarrow T_2}$.
...
$\mathcal{I}$ is a cartesian close category.
