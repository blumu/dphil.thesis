...
In a reduced-traversal, application nodes are all labelled
$\psi @$ for some variable $\psi$ \emph{bound} in $M$. There are
no node labelled $@$, $u@$ or $u$ where $u$ is a free variable in $M$ or a $\Sigma$-constant.
This is because in a justified sequence, nodes of these types have no pointer.
...
any for any reduced-traversal $s \jseq t \upharpoonright \{ r \}$:
$$ \pview{s} \jseq \pview{t} \setminus \left( N_\Sigma \union Succ(N_{fv(M)}) \right)$$

where $N_\Sigma$ denotes nodes lablelled $u$ or $u@$ for some
$\Sigma$-constant $u$, $N_{fv(M)}$ denotes nodes lablelled $u$ or $u@$ for some
variable $x$ free in $M$ and $Succ(N)$ denotes the set of child nodes of
$N$ in the tree $\tau(M)$.
\end{lem}
\todomargin{proof}
