...
%We recall that from the definition of $\vdash$ that if $n$ is a $\lambda$-node then:
%$$n\vdash x \iff ( x \mbox{ bound by } n ) \vee \mbox{ or } ( x \mbox{ is free in } M  \mbox{ and } n \mbox{ is the root of } \tau(M) ).$$
%Therefore an equivalent definition of tree regularity is: for any $\lambda$-node $n$ different from the root of $\tau(M)$ and any variable node $x$:
%$$n \vdash x \quad \iff \quad n \mbox{ is the first $\lambda$-node in the path from $x$ to the root with order $>\ord{x}$} $$
...
%\begin{floatingfigure}[r]{0.5cm}
%\begin{wrapfigure}{r}{0.5cm}
\parpic[r]{
%\pspicture(0,0)(1cm,1cm)
%\psgrid[subgriddiv=0, gridcolor=lightgray, gridlabelcolor=lightgray](0,0)(1cm,2cm)
%\rput[t](0.5cm,2cm)
{\psset{levelsep=4ex} \pstree{\TR{$\lambda x^3$}}{\pstree{\TR{$f^2$}}{ \pstree{\TR{$\lambda y^1$}}{ \TR{$x^0$} }}} }}
%\endpspicture
%\end{floatingfigure}
%\end{wrapfigure}
...
The properties of the Safe $\lambda$-calculus can be transposed straightforwardly to Safe \ialgol. In particular, it can be shown that
...
The $Y$ combinators needs a special treatment.
...
Y^{n+1}_A M &=& M( Y^n M )
\end{eqnarray*}
where $\Omega$ denotes the term $Y(\lambda x .x)$.
For any term $M:A$ we have $\sem{Y_A M} = \Union_{n\in\omega} \sem{Y_A^n M}$ (\cite{abramsky:game-semantics-tutorial}, lemma 16).

We add the constant $\bot$ to the set $\Sigma$ which represents the infinite computation $Y(\lambda x .x)$. Then it is possible to
compute the computation tree $\tau(Y_A^n M)$ for each approximant $Y_A^n M$.
We now use the standard approximation ordering on trees defined in \cite{KNU02}, section 1.
$\tau(Y_A M)$ is then defined to be the least upper bound of the sequence of computation trees $(\tau(Y_A^n M))_{n\in\omega}$.
...
It is straightforward to check that these traversal rules are well-behaved and that the system $\Sigma$-rules is deterministic.
Therefore conditions (C1) and (C2) of section \ref{subsec:traversal} are satisfied.

Similarly the reduction rules associated to $\pcfsucc$, $\pcfpred$, $\pcfcond$ preserves safety and their game denotation
are all regular strategies. Hence assumption (A1) and (A2) of section \ref{sec:assumptions} are satisfied.
