...
Before looking at each individual rule we prove a useful lemma:
\begin{lem}
...
\ord{B} -1$. Therefore the premise of the rule verifies $P_{-1}$:
...
We are now ready to look at each rule individually and try to specialize them to
...
$$ \rulename{abs^i} \quad  \rulef{\Gamma \union \overline{x} : \overline{A} \vdash M : B}
                                   {\Gamma  \vdash^{0} \lambda \overline{x} : \overline{A} . M : (\overline{A},B)} \qquad
                                   \forall y \in \Gamma : \ord{y} \geq \ord{\overline{A},B}$$

We now partition the context $\Gamma$ is according to the order of
...
used repeatedly if further partitions need to be abstracted. We
...
Table \ref{tab:homosafelmd_rules} recapitulates the entire set of rules. We observe that these rules correspond exactly to the rules given in the previous section
