...
We can make the assumption that $\ord{A_1} = \ldots = \ord{A_l} = \ord{\overline{A}}$ (if it is not the case,
we can replace this instance of the application rule by several consecutive instances of the
application rule of this kind).
...
$$fv(M) \vdash M : (A,\ldots,A_l,B) \mbox{ is valid and satisfies } P_0$$

Suppose $z \in \Sigma$ then for some $i \in 1..p$ $z \in fv(N_i)$, hence  $\ord{z} \geq \ord{A_i} \geq \ord{B} - 1$.
Therefore by applying the weakening rule we get:
\begin{equation}
fv(M) \union \Sigma \vdash M : (A,\ldots,A_l,B) \mbox{ is valid and satisfies } P_{-1} \label{eq:prop:nonhomogsafe_homog_restriction:m}
\end{equation}

Suppose $z \in fv(M)$ then for all $i\in 1..p$, $\ord{z} \geq \ord{M} \geq \ord{A_i}$ therefore:
\begin{equation}
...
$$
\Sigma' \vdash^{-1} M : (A,\ldots,A_l,B) \qquad \mbox{and} \qquad \Sigma' \vdash^0 N_i : A_i \mbox{ for all } i \in 1..l
$$
...
Otherwise, $\Sigma' \subset \Gamma$. The side condition in the original instance of the application rule says that all
