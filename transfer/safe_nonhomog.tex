...
Let $\Sigma$ be a set of higher-order constants. We call safe terms any simply typed lambda term that is
...
is identical.
Consequently, in the safe lambda calculus setting, we can omit to rename
...
restrictive since whenever $l>n$ it means that the application rule is ``wider'' than the abstraction
...
Then lemma \ref{lem:nonhomosafe_basic_prop} can then be restated as follows:
\begin{lem}[Context reduction]
...
lambda-calculus, the term $E$ is safe for any types $\mu$ and
$\tau$.
