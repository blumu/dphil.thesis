...
Hence, in the safe lambda calculus setting, we can simplify
...
%%%% note for the homogeneous case: (property (i) in lemma \ref{lem:safe_basic_prop}) is preserved
% by substitution
...
``simultaneous substitution'', a generalization of the standard substitution given in definition \ref{dfn:subst}.
...
\ldots x_n}$:
\begin{eqnarray*}
c \subst{\overline{N}}{\overline{x}} &=& c \quad \mbox{for $x\neq y$}\\
...
where $M[N/x]$ denotes the single capture permitting substitution. Equivalently it can be defined by induction as follow:
...
\todobox{To be proved: in the homogeneous case, there is a reduction
strategy performing reduction in a certain way (reduces simultaneously variable of the same order)
that preserves safety and that is normalizing .}
